...
FCITX_DEFINE_PLUGIN(fcitx_pinyin, ime2, FcitxIMClass2) = {
    PYCreate, PYDestroy, ReloadConfigPY, NULL, NULL, NULL, NULL, NULL};
...
void *PYCreate(FcitxInstance *instance) {
    ...
    FcitxInstanceRegisterIM(instance, pystate, "pinyin", _("Pinyin"), "pinyin",
                            PYInit, ResetPYStatus, DoPYInput, PYGetCandWords,
                            NULL, SavePY, NULL, NULL, 5, "zh_CN");
    FcitxInstanceRegisterIM(instance, pystate, "shuangpin", _("Shuangpin"),
                            "shuangpin", SPInit, ResetPYStatus, DoPYInput,
                            PYGetCandWords, NULL, SavePY, NULL, NULL, 5,
                            "zh_CN");
...
void PYDestroy(void *arg) {
    ...
    for (i = 0; i < pystate->iPYFACount; i++) {
        for (j = 0; j < PYFAList[i].iBase; j++) {
            PyPhrase *phrase =
                USER_PHRASE_NEXT(PYFAList[i].pyBase[j].userPhrase);
...
boolean LoadPYBaseDict(FcitxPinyinState *pystate) {
    ...
    pystate->PYFAList =
        (PYFA *)fcitx_utils_malloc0(sizeof(PYFA) * pystate->iPYFACount);
    PYFA *PYFAList = pystate->PYFAList;
    for (i = 0; i < pystate->iPYFACount; i++) {
        ...
        PYFAList[i].pyBase =
            (PyBase *)fcitx_utils_malloc0(sizeof(PyBase) * PYFAList[i].iBase);
        for (j = 0; j < PYFAList[i].iBase; j++) {
            int8_t len;
            ...
            PYFAList[i].pyBase[j].userPhrase->next =
                PYFAList[i].pyBase[j].userPhrase;
        }
    }
...
void LoadPYPhraseDict(FcitxPinyinState *pystate, FILE *fp, boolean isSystem,
    ...
    while (!feof(fp)) {
        ...
        for (k = 0; k < count; k++) {
            ...
            if (isSystem) {
                phrase->strMap = (char *)fcitx_memory_pool_alloc(
                    pystate->pool, sizeof(char) * (iLen + 1));
            } else {
                phrase->strMap =
                    (char *)fcitx_utils_malloc0(sizeof(char) * (iLen + 1));
            }
            ...
            if (isSystem) {
                phrase->strPhrase = (char *)fcitx_memory_pool_alloc(
                    pystate->pool, sizeof(char) * (iLen + 1));
            } else {
                phrase->strPhrase =
                    (char *)fcitx_utils_malloc0(sizeof(char) * (iLen + 1));
        ...
        if (isSystem) {
            if (PYFAList[i].pyBase[j].iPhrase == 0) {
                ...
                if (stripDup) {
                    for (m = 0; m < count; m++) {
                        for (n = 0; n < PYFAList[i].pyBase[j].iPhrase; n++) {
                            int result =
                                strcmp(PYFAList[i].pyBase[j].phrase[n].strMap,
                                       phrase[m].strMap);
                            if (result == 0) {
                                if (strcmp(PYFAList[i]
                                               .pyBase[j]
                                               .phrase[n]
                                               .strPhrase,
                                           phrase[m].strPhrase) == 0)
                                    break;
                            }
                        }
                ...
                if (left >= 0) {
                    ...
                    PYFAList[i].pyBase[j].phrase = realloc(
                        PYFAList[i].pyBase[j].phrase,
                        sizeof(PyPhrase) * PYFAList[i].pyBase[j].iPhrase);
                    // memcpy(phrase, oldph, sizeof(PyPhrase) * old);
                }
                for (m = 0; m < count; m++) {
                    if (!flag[m]) {
                        memcpy(&PYFAList[i].pyBase[j].phrase[orig], &phrase[m],
                               sizeof(PyPhrase));
                        orig++;
                    } else {
    ...
    if (!fp)
        ...
        while (curStr) {
            if (strcmp(curStr->name, PY_PHRASE_FILE) != 0 &&
                ...
                fp = FcitxXDGGetFileWithPrefix("pinyin", curStr->name, "r",
                                               NULL);
                if (fp) {
...
INPUT_RETURN_VALUE DoPYInput(void *arg, FcitxKeySym sym, unsigned int state) {
    ...
    if (!pystate->bIsPYAddFreq && !pystate->bIsPYDelFreq &&
        ...
        if ((FcitxHotkeyIsHotKeyLAZ(sym, state) ||
             ...
             (pystate->bSP && FcitxInputStateGetRawInputBufferSize(input) > 0 &&
              pystate->bSP_UseSemicolon &&
              FcitxHotkeyIsHotKey(sym, state, FCITX_SEMICOLON)))) {
            FcitxInputStateSetIsInRemind(input, false);
            FcitxInputStateSetShowCursor(input, true);

            /* we cannot insert seperator in the first, nor there is a existing
             * separator */
            if (FcitxHotkeyIsHotKey(sym, state, FCITX_SEPARATOR)) {
                if (!pystate->iPYInsertPoint)
                    return IRV_TO_PROCESS;
                if (pystate->strFindString[pystate->iPYInsertPoint - 1] ==
                    PY_SEPARATOR)
            ...
            if (!pystate->bSP && pystate->pyconfig.bUseVForQuickPhrase &&
                i == 0 &&
                FcitxHotkeyIsKey(sym, state, FcitxKey_v, FcitxKeyState_None)) {
                int key = sym;
                boolean useDup = false;
                boolean append = true;
                if (FcitxQuickPhraseLaunch(pystate->owner, &key, &useDup,
                                           &append))
            ...
            if (pystate->iPYSelected) {
                ...
                strcpy(strTemp,
                       pystate->pySelected[pystate->iPYSelected - 1].strPY);
            ...
            if (pystate->iPYInsertPoint <= 0) {
                if (pystate->iPYSelected) {
                    ...
                    strcpy(pystate->strFindString, strTemp);
                    pystate->iPYInsertPoint =
                        strlen(pystate->strFindString) - val;
                    pystate->iPYSelected--;
                    ParsePY(&pystate->pyconfig, pystate->strFindString,
                            &pystate->findMap, PY_PARSE_INPUT_USER,
                            pystate->bSP);
            ...
            retVal = IRV_COMMIT_STRING;
                                       ...
                                       pystate->pyconfig.hkPYDelUserPhr)) {
            if (!pystate->bIsPYDelUserPhr) {
                ...
                if (!candWord) {
                    ...
                    FcitxMessagesAddMessageStringsAtLast(
                        ...
                        _("Press index to delete user phrase (ESC for "
                          "cancel)"));
                    ...
                    return IRV_DISPLAY_MESSAGE;
                                       ...
                                       pystate->pyconfig.hkPYAddFreq)) {
            if (!pystate->bIsPYAddFreq && pystate->findMap.iHZCount == 1 &&
            ...
            if (!pystate->bIsPYDelFreq) {
                ...
                if (val == 1) {
                    FcitxMessagesAddMessageAtLast(
                        FcitxInputStateGetAuxUp(input), MSG_TIPS,
                        _("Press 1 to delete %s in frequent list (ESC for "
                          "cancel)"),
                        pystate->strFindString);
                } else {
                    FcitxMessagesAddMessageAtLast(
                        FcitxInputStateGetAuxUp(input), MSG_TIPS,
                        _("Press 1-%d to delete %s in frequent list (ESC for "
                          "cancel)"),
                        val, pystate->strFindString);
                }
        ...
        if (iKey >= 0) {
            ...
            if (!FcitxInputStateGetIsInRemind(input)) {
                if (!FcitxInputStateGetRawInputBufferSize(input))
                    ...
                    if (candWord->owner == pystate &&
                        (pystate->bIsPYAddFreq || pystate->bIsPYDelFreq ||
                         pystate->bIsPYDelUserPhr)) {
                        PYCandWord *pycandWord = candWord->priv;
                        if (pystate->bIsPYAddFreq) {
                            ...
                            if (pycandWord->iWhich == PY_CAND_USERPHRASE)
                                PYDelUserPhrase(
                                    pystate, pycandWord->cand.phrase.iPYFA,
                                    pycandWord->cand.phrase.iBase,
                                    (PyUsrPhrase *)
                                        pycandWord->cand.phrase.phrase);
                            pystate->bIsPYDelUserPhr = false;
                        }
...
INPUT_RETURN_VALUE PYGetCandWords(void *arg) {
    ...
    if (pystate->iPYSelected) {
        ...
        for (i = 0; i < pystate->iPYSelected; i++) {
            FcitxMessagesMessageConcat(
                msgPreedit, FcitxMessagesGetMessageCount(msgPreedit) - 1,
                pystate->pySelected[i].strHZ);
            FcitxMessagesMessageConcat(
                msgClientPreedit,
                FcitxMessagesGetMessageCount(msgClientPreedit) - 1,
                pystate->pySelected[i].strHZ);
        }
    }
    ...
    if (FcitxCandidateWordPageCount(candList) != 0) {
        FcitxCandidateWord *candWord =
            FcitxCandidateWordGetCurrentWindow(candList);
...
void PYCreateAuto(FcitxPinyinState *pystate) {
    ...
    while (fcitx_utf8_strlen(pystate->strPYAuto) < pystate->findMap.iHZCount) {
        ...
        if ((pystate->findMap.iHZCount - iCount) > 1) {
            for (candPos.iPYFA = 0; candPos.iPYFA < pystate->iPYFACount;
                 candPos.iPYFA++) {
                if (!Cmp2Map(pyconfig, PYFAList[candPos.iPYFA].strMap, str,
                             pystate->bSP)) {
                    for (candPos.iBase = 0;
                         candPos.iBase < PYFAList[candPos.iPYFA].iBase;
                         candPos.iBase++) {
                        phrase = USER_PHRASE_NEXT(PYFAList[candPos.iPYFA]
                                                      .pyBase[candPos.iBase]
                                                      .userPhrase);
                        for (candPos.iPhrase = 0;
                             candPos.iPhrase < PYFAList[candPos.iPYFA]
                                                   .pyBase[candPos.iBase]
                                                   .iUserPhrase;
                             candPos.iPhrase++) {
                            val = CmpMap(pyconfig, phrase->strMap, strMap,
                                         &iMatchedLength, pystate->bSP);
                            if (!val && iMatchedLength ==
                                            (pystate->findMap.iHZCount - 1) * 2)
                                return;
                            if (!val || (val && (strlen(phrase->strMap) ==
                                                 iMatchedLength))) {
                                if (!phraseSelected) {
                                    baseSelected =
                                        &(PYFAList[candPos.iPYFA]
                                              .pyBase[candPos.iBase]);
                                    pPYFA = &PYFAList[candPos.iPYFA];
                                    phraseSelected = phrase;
                                } else if (strlen(phrase->strMap) <=
                                           (pystate->findMap.iHZCount - 1) *
                                               2) {
                                    if (strlen(phrase->strMap) ==
                ...
                if (!Cmp2Map(pyconfig, PYFAList[candPos.iPYFA].strMap, str,
                    ...
                    for (candPos.iBase = 0;
                        ...
                        for (candPos.iPhrase = 0;
                            ...
                            if (!val ||
                                (val &&
                                 (strlen(PYFAList[candPos.iPYFA]
                                             .pyBase[candPos.iBase]
                                             .phrase[candPos.iPhrase]
                                             .strMap) == iMatchedLength))) {
                                if (!phraseSelected) {
                                    baseSelected =
                                        &(PYFAList[candPos.iPYFA]
                                              .pyBase[candPos.iBase]);
                                    ...
                                    phraseSelected =
                                        &(PYFAList[candPos.iPYFA]
                                              .pyBase[candPos.iBase]
                                              .phrase[candPos.iPhrase]);
                                } else if (strlen(PYFAList[candPos.iPYFA]
                                                      .pyBase[candPos.iBase]
                                                      .phrase[candPos.iPhrase]
                                                      .strMap) <=
                                           (pystate->findMap.iHZCount - 1) *
                                               2) {
                                    if (strlen(PYFAList[candPos.iPYFA]
                                                   .pyBase[candPos.iBase]
                                                   .phrase[candPos.iPhrase]
                                                   .strMap) ==
                                        strlen(phraseSelected->strMap)) {
                                        //先看词频，如果词频一样，再最近优先
                                        if ((PYFAList[candPos.iPYFA]
                                                 .pyBase[candPos.iBase]
                                                 .phrase[candPos.iPhrase]
                                                 .iHit >
                                             phraseSelected->iHit) ||
            ...
            for (candPos.iPYFA = 0; candPos.iPYFA < pystate->iPYFACount;
                ...
                if (!Cmp2Map(pyconfig, PYFAList[candPos.iPYFA].strMap, str,
                    ...
                    for (candPos.iBase = 0;
                         candPos.iBase < PYFAList[candPos.iPYFA].iBase;
                         candPos.iBase++) {
                        if ((int)(PYFAList[candPos.iPYFA]
                                      .pyBase[candPos.iBase]
                                      .iHit) > val) {
                            val = PYFAList[candPos.iPYFA]
                                      .pyBase[candPos.iBase]
                                      .iHit;
                            baseSelected = &(
                                PYFAList[candPos.iPYFA].pyBase[candPos.iBase]);
                            pPYFA = &PYFAList[candPos.iPYFA];
                        }
                    }
                }
...
INPUT_RETURN_VALUE PYGetCandWord(void *arg, FcitxCandidateWord *candWord) {
    ...
    case PY_CAND_AUTO:
        ...
        bAddNewPhrase =
            (pystate->iPYSelected > 0) || pystate->pyconfig.bPYSaveAutoAsPhrase;
        break;
    case PY_CAND_BASE: //是系统单字
        pBase = PYFAList[pycandWord->cand.base.iPYFA]
                    .pyBase[pycandWord->cand.base.iBase]
                    .strHZ;
        pBaseMap = PYFAList[pycandWord->cand.base.iPYFA].strMap;
        pIndex = &(PYFAList[pycandWord->cand.base.iPYFA]
                       .pyBase[pycandWord->cand.base.iBase]
                       .iIndex);
        PYFAList[pycandWord->cand.base.iPYFA]
            .pyBase[pycandWord->cand.base.iBase]
            .iHit++;
        pystate->iOrderCount++;
        break;
    ...
    case PY_CAND_SYSPHRASE:           //是系统词组
        pBase = PYFAList[pycandWord->cand.phrase.iPYFA]
                    .pyBase[pycandWord->cand.phrase.iBase]
                    .strHZ;
    ...
    case PY_CAND_FREQ: //是常用字
        ...
        break;
    case PY_CAND_REMIND: {
        strcpy(pystate->strPYRemindSource,
               pycandWord->cand.remind.phrase->strPhrase +
                   pycandWord->cand.remind.iLength);
        strcpy(pystate->strPYRemindMap, pycandWord->cand.remind.phrase->strMap +
                                            pycandWord->cand.remind.iLength);
    ...
    if (iLen == pystate->findMap.iHZCount) {
        ...
        ParsePY(&pystate->pyconfig, FcitxInputStateGetRawInputBuffer(input),
                &pystate->findMap, PY_PARSE_INPUT_USER, pystate->bSP);
        strHZString[0] = '\0';
        for (i = 0; i < pystate->iPYSelected; i++)
        ...
        if (bAddNewPhrase &&
            (fcitx_utf8_strlen(pystate->strPYAuto) <= (MAX_PY_PHRASE_LENGTH)))
    ...
    for (i = 0; i < iLen; i++)
        strcat(pystate->pySelected[pystate->iPYSelected].strPY,
               pystate->findMap.strPYParsed[i]);
    if (pBaseMap)
...
void PYGetPhraseCandWords(FcitxPinyinState *pystate) {
    ...
    for (candPos.iPYFA = 0; candPos.iPYFA < pystate->iPYFACount;
        ...
        if (!Cmp2Map(pyconfig, PYFAList[candPos.iPYFA].strMap, str,
            ...
            for (candPos.iBase = 0;
                 candPos.iBase < PYFAList[candPos.iPYFA].iBase;
                 candPos.iBase++) {
                phrase = USER_PHRASE_NEXT(
                    PYFAList[candPos.iPYFA].pyBase[candPos.iBase].userPhrase);
                for (candPos.iPhrase = 0;
                     candPos.iPhrase <
                     PYFAList[candPos.iPYFA].pyBase[candPos.iBase].iUserPhrase;
                     candPos.iPhrase++) {
                    val = CmpMap(pyconfig, phrase->strMap, strMap,
                                 &iMatchedLength, pystate->bSP);
                    if (!val ||
                        (val && (strlen(phrase->strMap) == iMatchedLength))) {
                        PYCandWord *pycandWord = fcitx_utils_new(PYCandWord);
                        PYAddPhraseCandWord(pystate, candPos, phrase, false,
                                            pycandWord);
    ...
    for (candPos.iPYFA = 0; candPos.iPYFA < pystate->iPYFACount;
        ...
        if (!Cmp2Map(pyconfig, PYFAList[candPos.iPYFA].strMap, str,
            ...
            for (candPos.iBase = 0;
                ...
                for (candPos.iPhrase = 0;
                     candPos.iPhrase <
                     PYFAList[candPos.iPYFA].pyBase[candPos.iBase].iPhrase;
                     candPos.iPhrase++) {
                    val = CmpMap(pyconfig,
                                 PYFAList[candPos.iPYFA]
                                     .pyBase[candPos.iBase]
                                 ...
                                 strMap, &iMatchedLength, pystate->bSP);
                    if (!val ||
                        (val && (strlen(PYFAList[candPos.iPYFA]
                                            .pyBase[candPos.iBase]
                                            .phrase[candPos.iPhrase]
                                            .strMap) == iMatchedLength))) {
                        PYCandWord *pycandWord = fcitx_utils_new(PYCandWord);
                        PYAddPhraseCandWord(pystate, candPos,
                                            &(PYFAList[candPos.iPYFA]
                                                  .pyBase[candPos.iBase]
                                                  .phrase[candPos.iPhrase]),
                                            true, pycandWord);
    ...
    for (pcand = (PYCandWord **)utarray_front(&candtemp); pcand != NULL;
        ...
        if ((*pcand)->iWhich == PY_CAND_USERPHRASE)
            ...
            candWord.wordType = MSG_OTHER;
        const char *pBase = PYFAList[(*pcand)->cand.phrase.iPYFA]
                                .pyBase[(*pcand)->cand.phrase.iBase]
                                .strHZ;
 ...
 */
boolean PYAddPhraseCandWord(FcitxPinyinState *pystate, PYCandIndex pos,
                            PyPhrase *phrase, boolean b,
                            PYCandWord *pycandword) {
    PYFA *PYFAList = pystate->PYFAList;
...
void PYGetBaseCandWords(FcitxPinyinState *pystate, PyFreq *pCurFreq) {
    ...
    for (candPos.iPYFA = 0; candPos.iPYFA < pystate->iPYFACount;
        ...
        if (!Cmp2Map(pyconfig, PYFAList[candPos.iPYFA].strMap, str,
            ...
            for (candPos.iBase = 0;
                ...
                if (!PYIsInFreq(
                        pCurFreq,
                        PYFAList[candPos.iPYFA].pyBase[candPos.iBase].strHZ)) {
                    PYCandWord *pycandWord = fcitx_utils_new(PYCandWord);
    ...
    for (pcand = (PYCandWord **)utarray_front(&candtemp); pcand != NULL;
        ...
        candWord.priv = *pcand;
        candWord.strExtra = NULL;
        candWord.strWord = strdup(PYFAList[(*pcand)->cand.base.iPYFA]
                                      .pyBase[(*pcand)->cand.base.iBase]
                                      .strHZ);
        candWord.wordType = MSG_OTHER;
 ...
 * 将一个常用字加入到候选列表的合适位置中
 */
void PYAddFreqCandWord(PyFreq *pyFreq, HZ *hz, char *strPY,
                       PYCandWord *pycandWord) {
...
boolean PYAddUserPhrase(FcitxPinyinState *pystate, const char *phrase,
    ...
    newPhrase->phrase.strMap =
        (char *)fcitx_utils_malloc0(sizeof(char) * strlen(map + 2) + 1);
    newPhrase->phrase.strPhrase =
        (char *)fcitx_utils_malloc0(sizeof(char) * strlen(phrase + clen) + 1);
    ...
    userPhrase = PYFAList[i].pyBase[j].userPhrase->next;
    for (k = 0; k < PYFAList[i].pyBase[j].iUserPhrase; k++) {
        if (CmpMap(pyconfig, map + 2, userPhrase->phrase.strMap, &iTemp,
                   pystate->bSP) > 0)
            break;
        temp = userPhrase;
        userPhrase = userPhrase->next;
...
void PYDelUserPhrase(FcitxPinyinState *pystate, int32_t iPYFA, int iBase,
                     PyUsrPhrase *phrase) {
    PyUsrPhrase *temp;
    PYFA *PYFAList = pystate->PYFAList;
...
void PYAddFreq(FcitxPinyinState *pystate, PYCandWord *pycandWord) {
    ...
    if (pCurFreq) {
        ...
        if (pycandWord->iWhich != PY_CAND_FREQ) {
            ...
            for (i = 0; i < pCurFreq->iCount; i++) {
                if (!strcmp(PYFAList[pycandWord->cand.base.iPYFA]
                                .pyBase[pycandWord->cand.base.iBase]
                                .strHZ,
                            HZTemp->strHZ)) {
                    i = -1;
    ...
    HZTemp = fcitx_utils_new(HZ);
    strcpy(HZTemp->strHZ, PYFAList[pycandWord->cand.base.iPYFA]
                              .pyBase[pycandWord->cand.base.iBase]
                              .strHZ);
...
INPUT_RETURN_VALUE PYGetRemindCandWords(void *arg) {
    ...
    for (i = 0; i < pystate->iPYFACount; i++) {
        if (!strncmp(pystate->strPYRemindMap, PYFAList[i].strMap, 2)) {
            for (j = 0; j < PYFAList[i].iBase; j++) {
                if (!fcitx_utf8_strncmp(pystate->strPYRemindSource,
                                        PYFAList[i].pyBase[j].strHZ, 1)) {
...
_HIT:
    ...
    for (i = 0; i < pyBaseForRemind->iPhrase; i++) {
        ...
        if (bDisablePagingInRemind &&
            utarray_len(&candtemp) >=
                FcitxCandidateWordGetPageSize(
        ...
        if (fcitx_utf8_strlen(pystate->strPYRemindSource) == 1) {
            if (fcitx_utf8_strlen(pyBaseForRemind->phrase[i].strPhrase) == 1) {
                PYCandWord *pycandWord = fcitx_utils_new(PYCandWord);
                PYAddRemindCandWord(pystate, &pyBaseForRemind->phrase[i],
                                    pycandWord);
                utarray_push_back(&candtemp, &pycandWord);
            }
        } else if (strlen(pyBaseForRemind->phrase[i].strPhrase) ==
                   strlen(pystate->strPYRemindSource)) {
            if (!strncmp(
                    pystate->strPYRemindSource +
                        fcitx_utf8_char_len(pystate->strPYRemindSource),
                    pyBaseForRemind->phrase[i].strPhrase,
                    strlen(pystate->strPYRemindSource +
                           fcitx_utf8_char_len(pystate->strPYRemindSource)))) {
                PYCandWord *pycandWord = fcitx_utils_new(PYCandWord);
                PYAddRemindCandWord(pystate, &pyBaseForRemind->phrase[i],
    ...
    for (i = 0; i < pyBaseForRemind->iUserPhrase; i++) {
        ...
        } else if (strlen(phrase->strPhrase) ==
                   strlen(pystate->strPYRemindSource)) {
            if (!strncmp(
                    pystate->strPYRemindSource +
                        fcitx_utf8_char_len(pystate->strPYRemindSource),
    ...
    utarray_foreach(pcand, &candtemp, PYCandWord *) {
        ...
        candWord.priv = *pcand;
        candWord.strExtra = NULL;
        candWord.strWord = strdup((*pcand)->cand.remind.phrase->strPhrase +
                                  (*pcand)->cand.remind.iLength);
        candWord.wordType = MSG_OTHER;
...
void PinyinMigration() {
    ...
    FcitxXDGGetFileUserWithPrefix("pinyin", PY_USERPHRASE_FILE, NULL,
                                  &newuserphrase);
    FcitxXDGGetFileUserWithPrefix("pinyin", PY_INDEX_FILE, NULL, &newpyindex);

    struct stat olduserphrasestat, oldpyindexstat, newuserphrasestat,
        newpyindexstat;

    /* check old file are all not exist */
    if (stat(newpyindex, &newpyindexstat) == -1 &&
        stat(newuserphrase, &newuserphrasestat) == -1) {
        if (stat(oldpyindex, &oldpyindexstat) == 0 ||
            stat(olduserphrase, &olduserphrasestat) == 0) {
...
int PYCandWordCmp(const void *b, const void *a, void *arg) {
    ...
    case PY_CAND_BASE: {
        ...
        case AD_NO:
            return 0;
        case AD_FAST: {
            int delta = context->pystate->PYFAList[canda->cand.base.iPYFA]
                            .pyBase[canda->cand.base.iBase]
                            .iIndex -
                        context->pystate->PYFAList[candb->cand.base.iPYFA]
                            .pyBase[candb->cand.base.iBase]
                            .iIndex;
            if (delta != 0)
                return delta;

            delta = context->pystate->PYFAList[canda->cand.base.iPYFA]
                        .pyBase[canda->cand.base.iBase]
                        .iHit -
                    context->pystate->PYFAList[candb->cand.base.iPYFA]
                        .pyBase[candb->cand.base.iBase]
    ...
    case PY_CAND_USERPHRASE: {
        switch (context->order) {
        case AD_NO:
            return strlen(canda->cand.phrase.phrase->strPhrase) -
                   strlen(candb->cand.phrase.phrase->strPhrase);
            break;
        case AD_FAST: {
            int size = strlen(canda->cand.phrase.phrase->strPhrase) -
                       strlen(candb->cand.phrase.phrase->strPhrase);
            if (size != 0)
                return size;

            if (canda->cand.phrase.phrase->iIndex -
                    candb->cand.phrase.phrase->iIndex !=
                0)
                return canda->cand.phrase.phrase->iIndex -
                       candb->cand.phrase.phrase->iIndex;

            return canda->cand.phrase.phrase->iHit -
                   candb->cand.phrase.phrase->iHit;
        } break;
        case AD_FREQ: {
            int size = strlen(canda->cand.phrase.phrase->strPhrase) -
                       strlen(candb->cand.phrase.phrase->strPhrase);
            ...
            if (canda->cand.phrase.phrase->iHit -
                    candb->cand.phrase.phrase->iHit !=
                0)
                return canda->cand.phrase.phrase->iHit -
                       candb->cand.phrase.phrase->iHit;
    ...
    case PY_CAND_REMIND:
        return canda->cand.remind.phrase->iHit -
               canda->cand.remind.phrase->iHit;
    default:
        return 0;
    }
...
static char *PYSP2QP(FcitxPinyinState *pystate, const char *strSP) {
...
static void PYAddUserPhraseFromCString(FcitxPinyinState *pystate,
                                       const char *strHZ) {
    const char *pivot;
    char *sp;
