...
static void FcitxKeyboardLayoutCreate(FcitxKeyboard *keyboard, const char *name,
                                      const char *langCode,
                                      const char *layoutString,
                                      const char *variantString);
static boolean LoadKeyboardConfig(FcitxKeyboard *keyboard,
                                  FcitxKeyboardConfig *fs);
...
static uint32_t processCompose(FcitxKeyboardLayout *layout, uint32_t keyval,
                               uint32_t state);

CONFIG_DESC_DEFINE(GetKeyboardConfigDesc, "fcitx-keyboard.desc")

FCITX_DEFINE_PLUGIN(fcitx_keyboard, ime2,
                    FcitxIMClass2) = {FcitxKeyboardCreate,
                                      NULL,
...
void FcitxKeyboardLayoutCreate(FcitxKeyboard *keyboard, const char *name,
                               const char *langCode, const char *layoutString,
                               const char *variantString) {
    FcitxKeyboardLayout *layout =
        fcitx_utils_malloc0(sizeof(FcitxKeyboardLayout));
    ...
    if (strcmp(keyboard->initialLayout, layoutString) == 0 &&
        ...
        FcitxXkbLayoutExists(keyboard->owner, layoutString, variantString,
                             &result);
        if (result)
            iPriority = 50;
    }
    ...
    FcitxInstanceRegisterIMv2(keyboard->owner, layout, uniqueName, name, "kbd",
                              iface, iPriority, langCode);
    free(uniqueName);
}
...
#if defined(ENABLE_XKB)
static const char *FindBestLanguage(FcitxIsoCodes *isocodes, const char *hint,
                                    UT_array *languages) {
...
void *FcitxKeyboardCreate(FcitxInstance *instance) {
    ...
    keyboard->xkbComposeTable =
        keyboard->xkbContext
            ? xkb_compose_table_new_from_locale(keyboard->xkbContext, locale,
                                                XKB_COMPOSE_COMPILE_NO_FLAGS)
            : NULL;
    keyboard->xkbComposeState =
        keyboard->xkbComposeTable
            ? xkb_compose_state_new(keyboard->xkbComposeTable,
    ...
    if (rules && utarray_len(rules->layoutInfos)) {
        ...
        FcitxXDGGetFileUserWithPrefix("", "cached_layout_XXXXXX", NULL,
                                      &tempfile);
        ...
        FcitxIsoCodes *isocodes =
            FcitxXkbReadIsoCodes(ISOCODES_ISO639_JSON, ISOCODES_ISO3166_JSON);
        utarray_foreach(layoutInfo, rules->layoutInfos, FcitxXkbLayoutInfo) {
            {
                const char *lang = FindBestLanguage(
                    isocodes, layoutInfo->description, layoutInfo->languages);
                char *description;
                ...
                FcitxKeyboardLayoutCreate(keyboard, description, lang,
                                          layoutInfo->name, NULL);
                free(description);
                if (fp)
                    fprintf(fp, "%s:%s:%s\n", layoutInfo->description,
                            layoutInfo->name, lang ? lang : "");
            }
            FcitxXkbVariantInfo *variantInfo;
            for (variantInfo = (FcitxXkbVariantInfo *)utarray_front(
                     layoutInfo->variantInfos);
                 variantInfo != NULL;
                 variantInfo = (FcitxXkbVariantInfo *)utarray_next(
                     layoutInfo->variantInfos, variantInfo)) {
                const char *lang =
                    FindBestLanguage(isocodes, layoutInfo->description,
                                     utarray_len(variantInfo->languages) > 0
                ...
                if (fp)
                    fprintf(fp, "%s:%s:%s:%s:%s\n", layoutInfo->description,
                            layoutInfo->name, variantInfo->description,
                            variantInfo->name, lang ? lang : "");
                free(description);
            }
        }
        ...
        if (fp) {
            ...
            FcitxXDGGetFileUserWithPrefix("", "cached_layout", NULL,
                                          &layoutFileName);
...
#endif
    {
        /* though this is unrelated to libxml2, but can only generated with
         * libxml2 enabled */
#if defined(ENABLE_XKB)
        FILE *fp =
            FcitxXDGGetFileUserWithPrefix("", "cached_layout", "r", NULL);
        if (fp) {
            ...
            while (getline(&buf, &len, fp) != -1) {
                ...
                if (utarray_len(list) == 3) {
                    ...
                    fcitx_utils_alloc_cat_str(
                        description, _("Keyboard"), " - ",
                        dgettext("xkeyboard-config", *layoutDescription), " ",
                        _("(Unavailable)"));
                    ...
                    fcitx_utils_alloc_cat_str(
                        description, _("Keyboard"), " - ",
                        dgettext("xkeyboard-config", *layoutDescription), " - ",
                        dgettext("xkeyboard-config", *variantDescription), " ",
                        _("(Unavailable)"));
                    FcitxKeyboardLayoutCreate(keyboard, description,
                                              *variantLang, *layoutName,
                                              *variantName);
                }
...
#endif
        {
            ...
            keyboard->initialVariant = NULL;
            FcitxKeyboardLayoutCreate(keyboard, _("Keyboard"), "en", "us",
                                      NULL);
        }
    }
    ...
    keyboard->dataSlot =
        FcitxInstanceAllocDataForIC(instance, NULL, SimpleCopy, NULL, keyboard);
...
boolean FcitxKeyboardInit(void *arg) {
    ...
    boolean flag = true;
    FcitxInstanceSetContext(layout->owner->owner, CONTEXT_DISABLE_AUTOENG,
                            &flag);
    FcitxInstanceSetContext(layout->owner->owner, CONTEXT_DISABLE_QUICKPHRASE,
                            &flag);
    FcitxInstanceSetContext(layout->owner->owner, CONTEXT_DISABLE_FULLWIDTH,
                            &flag);
...
static void FcitxKeyboardSetBuff(FcitxKeyboard *keyboard, const char *str) {
    int len = strlen(str);
...
static INPUT_RETURN_VALUE FcitxKeyboardHandleFocus(FcitxKeyboard *keyboard,
                                                   FcitxKeySym sym,
                                                   unsigned int state) {
        ...
        if (!keyboard->cursor_moved) {
            ...
            if (!cand_word) {
                ...
                FcitxCandidateWordSetFocus(
                    cand_list,
                    FcitxCandidateWordGetIndex(cand_list, cand_word));
            }
        }
    ...
    if (!FcitxInstanceICSupportPreedit(instance,
...
INPUT_RETURN_VALUE FcitxKeyboardDoInput(void *arg, FcitxKeySym sym,
                                        unsigned int state) {
    FcitxKeyboardLayout *layout = (FcitxKeyboardLayout *)arg;
    FcitxKeyboard *keyboard = layout->owner;
    FcitxInstance *instance = keyboard->owner;
    ...
    const char *currentLang =
        FcitxInstanceGetContextString(instance, CONTEXT_IM_LANGUAGE);
    if (currentLang == NULL)
        currentLang = "";
    ...
    if (enableWordHint && IsDictAvailable(keyboard)) {
        ...
        if (FcitxHotkeyIsHotKey(sym, state, keyboard->config.hkAddToUserDict)) {
        ...
        if (IsValidChar(result) || FcitxHotkeyIsHotKeySimple(sym, state) ||
            ...
            if (IsValidChar(result) || FcitxHotkeyIsHotKeyLAZ(sym, state) ||
                ...
                strncpy(&keyboard->buffer[0][keyboard->cursorPos], buf,
                        charlen);
            ...
            if (FcitxHotkeyIsHotKey(sym, state, FCITX_BACKSPACE)) {
                ...
                if (slen > 0) {
                    if (keyboard->cursorPos > 0) {
                        ...
                        memset(keyboard->buffer[0] + keyboard->cursorPos, 0,
                               sizeof(char) * (slen - keyboard->cursorPos));
                        return IRV_DISPLAY_CANDWORDS;
                    } else {
                        return IRV_DO_NOTHING;
                    }
...
static INPUT_RETURN_VALUE FcitxKeyboardGetCandWordCb(void *arg,
                                                     const char *commit) {
    FcitxKeyboardLayout *layout = (FcitxKeyboardLayout *)arg;
    FcitxKeyboard *keyboard = layout->owner;
    FcitxInstance *instance = keyboard->owner;
...
INPUT_RETURN_VALUE FcitxKeyboardGetCandWords(void *arg) {
    ...
    static const unsigned int cmodtable[] = {
        FcitxKeyState_None, FcitxKeyState_Alt, FcitxKeyState_Ctrl,
        FcitxKeyState_Shift};
    ...
    memcpy(FcitxInputStateGetRawInputBuffer(input), keyboard->buffer[0],
           bufferlen + 1);
    ...
    if (im && strncmp(im->uniqueName, "fcitx-keyboard",
        ...
        if (need_notify) {
            FcitxFreeDesktopNotifyShowAddonTip(
                instance, "fcitx-keyboard-hint", "tools-check-spelling",
                _("Spell hint"),
                enableWordHint != PTR_FALSE ? _("Spell hint is enabled.")
                                            : _("Spell hint is disabled."));
        }

        FcitxInstanceSetICData(instance, currentIC, keyboard->dataSlot,
                               enableWordHint);
        return IRV_DO_NOTHING;
    } else {
        return IRV_TO_PROCESS;
...
uint32_t processCompose(FcitxKeyboardLayout *layout, uint32_t keyval,
                        uint32_t state) {
    FcitxKeyboard *keyboard = layout->owner;
    if (!keyboard->xkbComposeState) {
        return 0;
    ...
    enum xkb_compose_feed_result result =
        xkb_compose_state_feed(keyboard->xkbComposeState, keyval);
    if (result == XKB_COMPOSE_FEED_IGNORED) {
        return 0;
    }

    enum xkb_compose_status status =
        xkb_compose_state_get_status(keyboard->xkbComposeState);
    if (status == XKB_COMPOSE_NOTHING) {
        return 0;
    } else if (status == XKB_COMPOSE_COMPOSED) {
        char buffer[UTF8_MAX_LENGTH + 1] = {'\0', '\0', '\0', '\0',
                                            '\0', '\0', '\0'};
        int length = xkb_compose_state_get_utf8(keyboard->xkbComposeState,
