...
Bool XIMSetICValuesHandler(FcitxXimFrontend *xim, IMChangeICStruct *call_data) {
    ...
    FcitxInputContext *ic =
        FcitxInstanceFindIC(xim->owner, xim->frontendid, &call_data->icid);
...
void SetTrackPos(FcitxXimFrontend *xim, FcitxInputContext *ic,
    ...
    if (call_data) {
        ...
        for (i = 0; i < (int)((IMChangeICStruct *)call_data)->preedit_attr_num;
             i++, pre_attr++) {
    ...
    if (window != None) {
        ...
        if (ximic->offset_x < 0 && ximic->offset_y < 0) {
            ...
            XTranslateCoordinates(
                xim->display, window, RootWindow(xim->display, xim->iScreen), 0,
                attr.height, &ic->offset_x, &ic->offset_y, &dst);
        } else {
            XTranslateCoordinates(xim->display, window,
                                  RootWindow(xim->display, xim->iScreen),
                                  ximic->offset_x, ximic->offset_y,
                                  &ic->offset_x, &ic->offset_y, &dst);
        }
    }
...
void XIMProcessKey(FcitxXimFrontend *xim, IMForwardEventStruct *call_data) {
    ...
    if (ic->frontendid != xim->frontendid ||
        GetXimIC(ic)->id != call_data->icid) {
        ic = FcitxInstanceFindIC(xim->owner, xim->frontendid, &call_data->icid);
        if (ic == NULL)
            return;
        if (FcitxInstanceSetCurrentIC(xim->owner, ic))
    ...
    state = kev->state - (kev->state & FcitxKeyState_NumLock) -
            (kev->state & FcitxKeyState_CapsLock) -
            (kev->state & FcitxKeyState_ScrollLock);
    ...
    INPUT_RETURN_VALUE retVal =
        FcitxInstanceProcessKey(xim->owner, type, kev->time, sym, state);
...
void XimForwardKeyInternal(FcitxXimFrontend *xim, FcitxXimIC *ic,
                           XEvent *xEvent) {
...
void XimPreeditCallbackStart(FcitxXimFrontend *xim, const FcitxXimIC *ic) {
...
void XimPreeditCallbackDone(FcitxXimFrontend *xim, const FcitxXimIC *ic) {
    IMPreeditCBStruct *pcb = fcitx_utils_new(IMPreeditCBStruct);
...
void XimPreeditCallbackDraw(FcitxXimFrontend *xim, FcitxXimIC *ic,
    ...
    if (len + 1 > xim->feedback_len) {
        ...
        xim->feedback =
            realloc(xim->feedback, sizeof(XIMFeedback) * xim->feedback_len);
    }
    ...
    Xutf8TextListToTextProperty(xim->display, (char **)&preedit_string, 1,
