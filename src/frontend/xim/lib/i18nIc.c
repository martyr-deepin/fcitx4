...
/* Set IC values */
static void SetCardAttribute(XICAttribute *value_ret, char *p, XICAttr *ic_attr,
...
static void SetFontAttribute(XICAttribute *value_ret, char *p, XICAttr *ic_attr,
                             int value_length, int need_swap) {
    char *buf;
    char *base_name;
    CARD16 base_length;
...
static void SetPointAttribute(XICAttribute *value_ret, char *p,
                              XICAttr *ic_attr, int value_length,
                              int need_swap) {
    XPoint *buf;
    FrameMgr fm;
...
static void SetRectAttribute(XICAttribute *value_ret, char *p, XICAttr *ic_attr,
                             int value_length, int need_swap) {
    XRectangle *buf;
    FrameMgr fm;
...
/* get IC values */
static void GetAttrHeader(unsigned char *rec, XICAttribute *list,
                          int need_swap) {
    FrameMgr fm;
...
static int ReadICValue(Xi18n i18n_core, CARD16 icvalue_id, int value_length,
                       void *p, XICAttribute *value_ret, CARD16 *number_ret,
                       int need_swap) {
    ...
    *number_ret = (CARD16)0;
    ...
    case XimType_NEST: {
        ...
        while (total_length < value_length) {
            ...
            FrameMgrFree(fm);
            p1 += sizeof(CARD16) * 2;
            ReadICValue(i18n_core, attribute_ID, attribute_length, p1,
                        (value_ret + ic_len), &number, need_swap);
            ic_len++;
            *number_ret += number;
            p1 += attribute_length;
            p1 += IMPAD(attribute_length);
            total_length += (CARD16)sizeof(CARD16) * 2 +
                            (INT16)attribute_length + IMPAD(attribute_length);
        }
        /*endwhile*/
        return ic_len;
    }
...
static XICAttribute *CreateNestedList(CARD16 attr_id, XICAttribute *list,
...
static int GetICValue(Xi18n i18n_core, XICAttribute *attr_ret, CARD16 *id_list,
                      int list_num) {
...
void _Xi18nChangeIC(XIMS ims, IMProtocol *call_data, unsigned char *p,
    ...
    while (FrameMgrIsIterLoopEnd(fm, &status) == False) {
        ...
        FrameMgrGetToken(fm, value);
        attrib_list[attrib_num].value =
            fcitx_utils_set_str_with_len(NULL, value, value_length);
        attrib_num++;
    }
    /*endwhile*/
        ...
        if (IsNestedList(i18n_core, attrib_list[i].attribute_id)) {
            if (attrib_list[i].attribute_id ==
                ...
                ReadICValue(i18n_core, attrib_list[i].attribute_id,
                            attrib_list[i].value_length, attrib_list[i].value,
                            &pre_attr[preedit_ic_num], &number,
                            _Xi18nNeedSwap(i18n_core, connect_id));
                preedit_ic_num += number;
            } else if (attrib_list[i].attribute_id ==
                       i18n_core->address.statusAttr_id) {
                ReadICValue(i18n_core, attrib_list[i].attribute_id,
                            attrib_list[i].value_length, attrib_list[i].value,
    ...
    if (create_flag == True) {
        _Xi18nSendMessage(ims, connect_id, XIM_CREATE_IC_REPLY, 0, reply,
                          total_size);
    } else {
        _Xi18nSendMessage(ims, connect_id, XIM_SET_IC_VALUES_REPLY, 0, reply,
                          total_size);
    }
    /*endif*/
    if (create_flag == True) {
        ...
        if (on_key_num == 0 && off_key_num == 0) {
            ...
            _Xi18nSetEventMask(ims, connect_id, input_method_ID, changeic->icid,
                               mask, ~mask);
        }
        /*endif*/
    }
...
void _Xi18nGetIC(XIMS ims, IMProtocol *call_data, unsigned char *p) {
    ...
    while (i < number) {
        ...
        if (IsNestedList(i18n_core, attrID_list[i])) {
            if (attrID_list[i] == i18n_core->address.preeditAttr_id) {
                read_number = GetICValue(i18n_core, &pre_attr[pre_count],
                                         &attrID_list[i], number);
                i += read_number + 1;
                pre_count += read_number;
            } else if (attrID_list[i] == i18n_core->address.statusAttr_id) {
                read_number = GetICValue(i18n_core, &sts_attr[sts_count],
                                         &attrID_list[i], number);
                i += read_number + 1;
            ...
            read_number = GetICValue(i18n_core, &ic_attr[ic_count],
                                     &attrID_list[i], number);
            if (read_number == 0) {
                i += 1;
            } else {
    ...
    preedit_ret = CreateNestedList(i18n_core->address.preeditAttr_id,
                                   getic->preedit_attr, getic->preedit_attr_num,
                                   _Xi18nNeedSwap(i18n_core, connect_id));
    if (preedit_ret)
        iter_count++;
    /*endif*/
    status_ret = CreateNestedList(i18n_core->address.statusAttr_id,
                                  getic->status_attr, getic->status_attr_num,
                                  _Xi18nNeedSwap(i18n_core, connect_id));
    if (status_ret)
        iter_count++;
    /*endif*/
    ...
    /*endif*/
    _Xi18nSendMessage(ims, connect_id, XIM_GET_IC_VALUES_REPLY, 0, reply,
                      total_size);
    XFree(reply);
    XFree(attrID_list);
