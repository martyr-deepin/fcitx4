...
static void DebugLog(char *msg) { fprintf(stderr, msg); }
#endif

extern Xi18nClient *_Xi18nFindClient(Xi18n, CARD16);

static void GetProtocolVersion(CARD16 client_major, CARD16 client_minor,
                               CARD16 *server_major, CARD16 *server_minor) {
    *server_major = client_major;
    *server_minor = client_minor;
}

static void ConnectMessageProc(XIMS ims, IMProtocol *call_data,
                               unsigned char *p) {
    ...
    IMConnectStruct *imconnect = (IMConnectStruct *)&call_data->imconnect;
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_CONNECT_REPLY, 0, reply, total_size);
...
static void DisConnectMessageProc(XIMS ims, IMProtocol *call_data) {
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_DISCONNECT_REPLY, 0, reply, 0);
...
static void OpenMessageProc(XIMS ims, IMProtocol *call_data, unsigned char *p) {
    ...
    fm =
        FrameMgrInit(open_fr, (char *)p, _Xi18nNeedSwap(i18n_core, connect_id));
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_OPEN_REPLY, 0, reply, total_size);

    FrameMgrFree(fm);
    XFree(reply);
}

static void CloseMessageProc(XIMS ims, IMProtocol *call_data,
                             unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_CLOSE_REPLY, 0, reply, total_size);

    FrameMgrFree(fm);
    XFree(reply);
}

static XIMExt *MakeExtensionList(Xi18n i18n_core, XIMStr *lib_extension,
...
static void QueryExtensionMessageProc(XIMS ims, IMProtocol *call_data,
                                      unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    FmStatus status;
    ...
    /*endfor*/
    _Xi18nSendMessage(ims, connect_id, XIM_QUERY_EXTENSION_REPLY, 0, reply,
                      total_size);
    FrameMgrFree(fm);
    XFree(reply);
...
static void SyncReplyMessageProc(XIMS ims, IMProtocol *call_data,
                                 unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
...
static void GetIMValueFromName(Xi18n i18n_core, CARD16 connect_id, char *buf,
...
static XIMAttribute *MakeIMAttributeList(Xi18n i18n_core, CARD16 connect_id,
                                         CARD16 *list, int *number,
                                         int *length) {
    ...
    for (i = 0; i < *number; i++) {
        for (j = 0; j < list_len; j++) {
            if (attr[j].attribute_id == list[i]) {
                ...
                GetIMValueFromName(i18n_core, connect_id, NULL, attr[j].name,
                                   &value_length);
                ...
                GetIMValueFromName(i18n_core, connect_id,
                                   attrib_list[list_num].value, attr[j].name,
                                   &value_length);
...
static void GetIMValuesMessageProc(XIMS ims, IMProtocol *call_data,
                                   unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    FmStatus status;
    ...
    im_attribute_list = MakeIMAttributeList(i18n_core, connect_id,
                                            im_attrID_list, &number, &list_len);
    if (im_attrID_list)
    ...
    /*endfor*/
    _Xi18nSendMessage(ims, connect_id, XIM_GET_IM_VALUES_REPLY, 0, reply,
                      total_size);
    FrameMgrFree(fm);
    XFree(reply);
...
static void CreateICMessageProc(XIMS ims, IMProtocol *call_data,
                                unsigned char *p) {
    _Xi18nChangeIC(ims, call_data, p, True);
}

static void SetICValuesMessageProc(XIMS ims, IMProtocol *call_data,
                                   unsigned char *p) {
    _Xi18nChangeIC(ims, call_data, p, False);
}

static void GetICValuesMessageProc(XIMS ims, IMProtocol *call_data,
                                   unsigned char *p) {
    _Xi18nGetIC(ims, call_data, p);
}

static void SetICFocusMessageProc(XIMS ims, IMProtocol *call_data,
                                  unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
...
static void UnsetICFocusMessageProc(XIMS ims, IMProtocol *call_data,
                                    unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
...
static void DestroyICMessageProc(XIMS ims, IMProtocol *call_data,
                                 unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    ...
    register int total_size;
    unsigned char *reply = NULL;
    IMDestroyICStruct *destroy = (IMDestroyICStruct *)&call_data->destroyic;
    CARD16 connect_id = call_data->any.connect_id;
    CARD16 input_method_ID;
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_DESTROY_IC_REPLY, 0, reply,
                      total_size);
    XFree(reply);
    FrameMgrFree(fm);
}

static void ResetICMessageProc(XIMS ims, IMProtocol *call_data,
                               unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    ...
    register int total_size;
    unsigned char *reply = NULL;
    IMResetICStruct *resetic = (IMResetICStruct *)&call_data->resetic;
    CARD16 connect_id = call_data->any.connect_id;
    CARD16 input_method_ID;
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_RESET_IC_REPLY, 0, reply,
                      total_size);
    FrameMgrFree(fm);
    XFree(reply);
}

static int WireEventToEvent(Xi18n i18n_core, xEvent *event, CARD16 serial,
                            XEvent *ev) {
...
static void ForwardEventMessageProc(XIMS ims, IMProtocol *call_data,
                                    unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    ...
    if (WireEventToEvent(i18n_core, &wire_event, forward->serial_number,
                         &forward->event) == True) {
        if (i18n_core->address.improto) {
            if (!(i18n_core->address.improto(ims, call_data)))
                return;
...
static void ExtForwardKeyEventMessageProc(XIMS ims, IMProtocol *call_data,
                                          unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
...
static void ExtMoveMessageProc(XIMS ims, IMProtocol *call_data,
                               unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    extern XimFrameRec ext_move_fr[];
    IMMoveStruct *extmove = (IMMoveStruct *)&call_data->extmove;
    CARD16 connect_id = call_data->any.connect_id;
    CARD16 input_method_ID;
...
static void ExtensionMessageProc(XIMS ims, IMProtocol *call_data,
                                 unsigned char *p) {
...
static void TriggerNotifyMessageProc(XIMS ims, IMProtocol *call_data,
                                     unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    ...
    if (flag == 0) {
        /* on key */
        _Xi18nSendMessage(ims, connect_id, XIM_TRIGGER_NOTIFY_REPLY, 0, reply,
                          total_size);
...
static INT16 ChooseEncoding(Xi18n i18n_core,
    ...
    for (i = 0; i < (int)p->count_encodings; i++) {
        for (j = 0; j < (int)enc_nego->encoding_number; j++) {
            if (strcmp(p->supported_encodings[i], enc_nego->encoding[j].name) ==
                0) {
                enc_index = j;
                break;
            }
...
static void EncodingNegotiatonMessageProc(XIMS ims, IMProtocol *call_data,
                                          unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
    FmStatus status;
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_ENCODING_NEGOTIATION_REPLY, 0, reply,
                      total_size);
    XFree(reply);
...
void PreeditStartReplyMessageProc(XIMS ims, IMProtocol *call_data,
                                  unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
...
void PreeditCaretReplyMessageProc(XIMS ims, IMProtocol *call_data,
                                  unsigned char *p) {
    Xi18n i18n_core = ims->protocol;
    FrameMgr fm;
...
void StrConvReplyMessageProc(XIMS ims, IMProtocol *call_data,
                             unsigned char *p) {
    return;
}
...
static void ProcessQueue(XIMS ims, CARD16 connect_id) {
    Xi18n i18n_core = ims->protocol;
    Xi18nClient *client =
        (Xi18nClient *)_Xi18nFindClient(i18n_core, connect_id);
    if (!client)
        return;
...
void _Xi18nMessageHandler(XIMS ims, CARD16 connect_id, unsigned char *p,
                          Bool *delete) {
