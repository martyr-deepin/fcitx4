...
void _Xi18nDeleteClient(Xi18n i18n_core, CARD16 connect_id) {
    ...
    for (ccp = i18n_core->address.clients, ccp0 = NULL; ccp != NULL;
...
void _Xi18nSendTriggerKey(XIMS ims, CARD16 connect_id) {
    ...
    /*endfor*/
    _Xi18nSendMessage(ims, connect_id, XIM_REGISTER_TRIGGERKEYS, 0, reply,
                      total_size);
    FrameMgrFree(fm);
    XFree(reply);
}

void _Xi18nSetEventMask(XIMS ims, CARD16 connect_id, CARD16 im_id, CARD16 ic_id,
    ...
    _Xi18nSendMessage(ims, connect_id, XIM_SET_EVENT_MASK, 0, reply,
                      total_size);
