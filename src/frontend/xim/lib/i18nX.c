...
static XClient *NewXClient(Xi18n i18n_core, Window new_client) {
    ...
    x_client->accept_win =
        XCreateSimpleWindow(dpy, DefaultRootWindow(dpy), 0, 0, 1, 1, 1, 0, 0);
    client->trans_rec = x_client;
    return ((XClient *)x_client);
}

static unsigned char *ReadXIMMessage(XIMS ims, XClientMessageEvent *ev,
                                     int *connect_id) {
    ...
    if (ev->format == 8) {
        ...
        return_code = XGetWindowProperty(
            i18n_core->address.dpy, x_client->accept_win, atom, 0L, length,
...
static void ReadXConnectMessage(XIMS ims, XClientMessageEvent *ev) {
    ...
    /*endif*/
    _XRegisterFilterByType(dpy, x_client->accept_win, ClientMessage,
                           ClientMessage, WaitXIMProtocol, (XPointer)ims);
    ...
    XSendEvent(dpy, new_client, False, NoEventMask, &event);
    XFlush(dpy);
}
...
static Bool Xi18nXBegin(XIMS ims) {
    ...
    spec->xim_request =
        XInternAtom(i18n_core->address.dpy, _XIM_PROTOCOL, False);
    spec->connect_request =
        XInternAtom(i18n_core->address.dpy, _XIM_XCONNECT, False);

    _XRegisterFilterByType(dpy, i18n_core->address.im_window, ClientMessage,
                           ClientMessage, WaitXConnectMessage, (XPointer)ims);
    return True;
}
...
static Bool Xi18nXEnd(XIMS ims) {
    ...
    _XUnregisterFilter(dpy, i18n_core->address.im_window, WaitXConnectMessage,
                       (XPointer)ims);
    return True;
}
...
static Bool Xi18nXSend(XIMS ims, CARD16 connect_id, unsigned char *reply,
                       long length) {
    Xi18n i18n_core = ims->protocol;
    ...
    if (length > XCM_DATA_LIMIT) {
        ...
        return_code = XGetWindowProperty(
            i18n_core->address.dpy, x_client->client_win, atom, 0L, 10000L,
            False, XA_STRING, &actual_type_ret, &actual_format_ret, &nitems_ret,
        ...
        if (win_data)
            XFree((char *)win_data);
        /*endif*/
        XChangeProperty(i18n_core->address.dpy, x_client->client_win, atom,
                        XA_STRING, 8, PropModeAppend, (unsigned char *)reply,
                        length);
    ...
    XSendEvent(i18n_core->address.dpy, x_client->client_win, False, NoEventMask,
               &event);
...
static Bool Xi18nXWait(XIMS ims, CARD16 connect_id, CARD8 major_opcode,
                       CARD8 minor_opcode) {
    Xi18n i18n_core = ims->protocol;
    XEvent event;
    Xi18nClient *client = _Xi18nFindClient(i18n_core, connect_id);
    ...
    for (;;) {
        ...
        XIfEvent(i18n_core->address.dpy, &event, CheckCMEvent,
                 (XPointer)i18n_core);
        if (event.xclient.window == x_client->accept_win) {
            if ((packet = ReadXIMMessage(ims, (XClientMessageEvent *)&event,
                                         &connect_id_ret)) ==
                (unsigned char *)NULL) {
                return False;
            }
...
static Bool Xi18nXDisconnect(XIMS ims, CARD16 connect_id) {
    ...
    _XUnregisterFilter(dpy, x_client->accept_win, WaitXIMProtocol,
                       (XPointer)ims);
    XFree(x_client);
...
Bool _Xi18nCheckXAddress(Xi18n i18n_core, TransportSW *transSW, char *address) {
    XSpecRec *spec;
...
static Bool WaitXConnectMessage(Display *dpy, Window win, XEvent *ev,
...
static Bool WaitXIMProtocol(Display *dpy, Window win, XEvent *ev,
                            XPointer client_data) {
    ...
    if (((XClientMessageEvent *)ev)->message_type == spec->xim_request) {
        if ((packet = ReadXIMMessage(ims, (XClientMessageEvent *)ev,
