...
static void IPCForwardKey(void *arg, FcitxInputContext *ic,
                          FcitxKeyEventType event, FcitxKeySym sym,
                          unsigned int state);
static void IPCSetWindowOffset(void *arg, FcitxInputContext *ic, int x, int y);
static void IPCGetWindowRect(void *arg, FcitxInputContext *ic, int *x, int *y,
                             int *w, int *h);
static void IPCUpdatePreedit(void *arg, FcitxInputContext *ic);
static void IPCDeleteSurroundingText(void *arg, FcitxInputContext *ic,
                                     int offset, unsigned int size);
static boolean IPCGetSurroundingText(void *arg, FcitxInputContext *ic,
                                     char **str, unsigned int *cursor,
                                     unsigned int *anchor);
static void IPCUpdateClientSideUI(void *arg, FcitxInputContext *ic);
static DBusHandlerResult IPCDBusEventHandler(DBusConnection *connection,
                                             DBusMessage *message,
                                             void *user_data);
static DBusHandlerResult IPCICDBusEventHandler(DBusConnection *connection,
                                               DBusMessage *msg,
                                               void *user_data);
...
static void IPCICSetCursorRect(FcitxIPCFrontend *ipc, FcitxInputContext *ic,
                               int x, int y, int w, int h);
static int IPCProcessKey(FcitxIPCFrontend *ipc, FcitxInputContext *callic,
                         const uint32_t originsym, const uint32_t keycode,
                         const uint32_t originstate, uint32_t t,
                         FcitxKeyEventType type);
static boolean IPCCheckICFromSameApplication(void *arg,
                                             FcitxInputContext *icToCheck,
...
const FcitxDBusPropertyTable propertTable[] = {
    {FCITX_IM_DBUS_INTERFACE, "IMList", "a(sssb)", IPCGetPropertyIMList,
     IPCSetPropertyIMList},
    {FCITX_IM_DBUS_INTERFACE, "CurrentIM", "s", IPCGetPropertyCurrentIM,
     IPCSetPropertyCurrentIM},
...
FCITX_DEFINE_PLUGIN(fcitx_ipc, frontend,
                    FcitxFrontend) = {IPCCreate,
                                      IPCDestroy,
                                      IPCCreateIC,
                                      IPCCheckIC,
                                      IPCDestroyIC,
...
void *IPCCreate(FcitxInstance *instance, int frontendid) {
    ...
    DBusObjectPathVTable fcitxIPCVTable = {
        NULL, &IPCDBusEventHandler, NULL, NULL, NULL, NULL};

    if (ipc->_conn) {
        dbus_connection_register_object_path(ipc->_conn, FCITX_IM_DBUS_PATH,
                                             &fcitxIPCVTable, ipc);
    }

    if (ipc->_privconn) {
        dbus_connection_register_object_path(ipc->_privconn, FCITX_IM_DBUS_PATH,
                                             &fcitxIPCVTable, ipc);
    }
...
void IPCCreateIC(void *arg, FcitxInputContext *context, void *priv) {
    ...
    if (dbus_message_is_method_call(message, FCITX_IM_DBUS_INTERFACE,
                                    "CreateIC")) {
        ...
        dbus_message_append_args(reply, DBUS_TYPE_INT32, &ipcic->id,
                                 DBUS_TYPE_UINT32, &arg1, DBUS_TYPE_UINT32,
                                 &arg2, DBUS_TYPE_UINT32, &arg3,
                                 DBUS_TYPE_UINT32, &arg4, DBUS_TYPE_INVALID);
        ...
        char *appname = NULL;
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &appname,
                                   DBUS_TYPE_INVALID))
            appname = NULL;
        else {
            if (strlen(appname) == 0) {
        ...
        if (config->shareState == ShareState_PerProgram) {
            FcitxInstanceSetICStateFromSameApplication(
                ipc->owner, ipc->frontendid, context);
        }
        ...
        dbus_message_append_args(
            reply, DBUS_TYPE_INT32, &ipcic->id, DBUS_TYPE_BOOLEAN, &arg0,
            DBUS_TYPE_UINT32, &arg1, DBUS_TYPE_UINT32, &arg2, DBUS_TYPE_UINT32,
        ...
        char *appname = NULL;
        if (!dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &appname,
                                   DBUS_TYPE_INT32, &icpid, DBUS_TYPE_INVALID))
            appname = NULL;
        else {
            if (strlen(appname) == 0) {
                appname = NULL;
    ...
    DBusObjectPathVTable vtable = {
        NULL, &IPCICDBusEventHandler, NULL, NULL, NULL, NULL};
    if (!ipcic->isPriv) {
        if (ipc->_conn) {
            dbus_connection_register_object_path(ipc->_conn, ipcic->path,
                                                 &vtable, ipc);
            dbus_connection_flush(ipc->_conn);
        }
    } else {
        if (ipc->_privconn) {
            dbus_connection_register_object_path(ipc->_privconn, ipcic->path,
                                                 &vtable, ipc);
...
void IPCDestroyIC(void *arg, FcitxInputContext *context) {
    ...
    if (!ipcic->isPriv) {
        if (ipc->_conn)
            dbus_connection_unregister_object_path(ipc->_conn,
                                                   GetIPCIC(context)->path);
    } else {
        if (ipc->_privconn)
            dbus_connection_unregister_object_path(ipc->_privconn,
                                                   GetIPCIC(context)->path);
    }
...
void IPCReloadConfigUI(void *arg) {
    ...
    DBusMessage *msg = dbus_message_new_signal(
        FCITX_IM_DBUS_PATH,      // object name of the signal
...
void IPCEnableIM(void *arg, FcitxInputContext *ic) {
    FcitxIPCFrontend *ipc = (FcitxIPCFrontend *)arg;
    DBusMessage *msg = dbus_message_new_signal(
        GetIPCIC(ic)->path,      // object name of the signal
...
void IPCForwardKey(void *arg, FcitxInputContext *ic, FcitxKeyEventType event,
    ...
    dbus_message_append_args(msg, DBUS_TYPE_UINT32, &keyval, DBUS_TYPE_UINT32,
                             &keystate, DBUS_TYPE_INT32, &type,
                             DBUS_TYPE_INVALID);
    IPCSendSignal(ipc, GetIPCIC(ic), msg);
}
...
void IPCGetWindowRect(void *arg, FcitxInputContext *ic, int *x, int *y, int *w,
                      int *h) {
    FCITX_UNUSED(arg);
    *x = ic->offset_x;
    *y = ic->offset_y;
...
static DBusHandlerResult IPCDBusEventHandler(DBusConnection *connection,
    ...
    if (dbus_message_is_method_call(msg, DBUS_INTERFACE_INTROSPECTABLE,
        ...
        dbus_message_append_args(reply, DBUS_TYPE_STRING, &im_introspection_xml,
                                 DBUS_TYPE_INVALID);
        ...
        reply = FcitxDBusPropertySet(ipc, propertTable, msg);
                                           ...
                                           "GetAll")) {
        FcitxLog(DEBUG, "GetAll");
        reply = FcitxDBusPropertyGetAll(ipc, propertTable, msg);
    } else if (dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "CreateIC") ||
               dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "CreateICv2") ||
               dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "CreateICv3")) {
        FcitxLog(DEBUG, "CreateIC");
        ...
        if (dbus_message_get_args(msg, &error, DBUS_TYPE_INT32, &index,
            ...
            reply = FcitxDBusPropertyUnknownMethod(msg);
                                           ...
                                           "SetCurrentIM")) {
        FcitxLog(DEBUG, "SetCurrentIM");
        DBusError error;
        dbus_error_init(&error);
        char *imname = NULL;
        if (dbus_message_get_args(msg, &error, DBUS_TYPE_STRING, &imname,
                                  DBUS_TYPE_INVALID)) {
        ...
        if (dbus_message_get_args(msg, &error, DBUS_TYPE_STRING, &imname,
                                  DBUS_TYPE_INVALID)) {
            FcitxIM *im =
                FcitxInstanceGetIMFromIMList(instance, IMAS_Disable, imname);
            const char *name = "";
            if (im && im->owner)
                name = im->owner->name;
            reply = dbus_message_new_method_return(msg);
        ...
        fcitx_utils_launch_configure_tool();
                                           ...
                                           "ActivateIM")) {
        FcitxLog(DEBUG, "ActivateIM");
        FcitxInstanceEnableIM(ipc->owner, FcitxInstanceGetCurrentIC(ipc->owner),
                              false);
        reply = dbus_message_new_method_return(msg);
    } else if (dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "InactivateIM")) {
        FcitxLog(DEBUG, "InactivateIM");
        FcitxInstanceCloseIM(ipc->owner, FcitxInstanceGetCurrentIC(ipc->owner));
        reply = dbus_message_new_method_return(msg);
    } else if (dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "ToggleIM")) {
        FcitxLog(DEBUG, "ToggleIM");
        FcitxInstanceChangeIMState(ipc->owner,
                                   FcitxInstanceGetCurrentIC(ipc->owner));
        reply = dbus_message_new_method_return(msg);
    } else if (dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "SwitchIM")) {
        FcitxLog(DEBUG, "SwitchIM");
        FcitxInstanceSwitchIMByIndex(instance, -2);
        reply = dbus_message_new_method_return(msg);
    } else if (dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "ResetIMList")) {
        FcitxLog(DEBUG, "ResetIMList");
        ...
        reply = dbus_message_new_method_return(msg);
                                           ...
                                           "GetCurrentState")) {
        FcitxLog(DEBUG, "GetCurrentState");
        int r = FcitxInstanceGetCurrentState(ipc->owner);
        reply = dbus_message_new_method_return(msg);
        dbus_message_append_args(reply, DBUS_TYPE_INT32, &r, DBUS_TYPE_INVALID);
    } else if (dbus_message_is_method_call(msg, FCITX_IM_DBUS_INTERFACE,
                                           "ConfigureAddon")) {
        FcitxLog(DEBUG, "ConfigureAddon");
        DBusError error;
        dbus_error_init(&error);
        char *addonname = NULL;
        if (dbus_message_get_args(msg, &error, DBUS_TYPE_STRING, &addonname,
                                  DBUS_TYPE_INVALID)) {
            if (addonname) {
        ...
        dbus_error_free(&error);
                                           ...
                                           "ReloadAddonConfig")) {
        FcitxLog(DEBUG, "ReloadAddonConfig");
        DBusError error;
        dbus_error_init(&error);
        char *addonname = NULL;
        if (dbus_message_get_args(msg, &error, DBUS_TYPE_STRING, &addonname,
...
static DBusHandlerResult IPCICDBusEventHandler(DBusConnection *connection,
    ...
    FcitxInputContext *ic =
        FcitxInstanceFindIC(ipc->owner, ipc->frontendid, &id);
    DBusHandlerResult result = DBUS_HANDLER_RESULT_NOT_YET_HANDLED;
    DBusMessage *reply = NULL;
    boolean flush = false;
    ...
    if (dbus_message_is_method_call(msg, DBUS_INTERFACE_INTROSPECTABLE,
                                    "Introspect")) {
        reply = dbus_message_new_method_return(msg);
        dbus_message_append_args(reply, DBUS_TYPE_STRING, &ic_introspection_xml,
                                 DBUS_TYPE_INVALID);
    }
    ...
    if (!reply && ic) {
        ...
        if (strcmp(dbus_message_get_sender(msg), GetIPCIC(ic)->sender) != 0) {
            reply = dbus_message_new_error(msg, "org.fcitx.Fcitx.Error",
                                           "Invalid sender");
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "EnableIC")) {
            FcitxLog(DEBUG, "EnableIC");
            FcitxInstanceEnableIM(ipc->owner, ic, false);
            reply = dbus_message_new_method_return(msg);
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "CloseIC")) {
            FcitxLog(DEBUG, "CloseIC");
            FcitxInstanceCloseIM(ipc->owner, ic);
            reply = dbus_message_new_method_return(msg);
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "FocusIn")) {
            FcitxLog(DEBUG, "FocusIn");
            IPCICFocusIn(ipc, ic);
            reply = dbus_message_new_method_return(msg);
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "FocusOut")) {
            FcitxLog(DEBUG, "FocusOut");
            IPCICFocusOut(ipc, ic);
            reply = dbus_message_new_method_return(msg);
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "Reset")) {
            FcitxLog(DEBUG, "Reset");
            IPCICReset(ipc, ic);
            reply = dbus_message_new_method_return(msg);
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "MouseEvent")) {
            FcitxLog(DEBUG, "MouseEvent");
            reply = dbus_message_new_method_return(msg);
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "SetCursorLocation")) {
            FcitxLog(DEBUG, "SetCursorLocation");
            int x, y;
            if (dbus_message_get_args(msg, &error, DBUS_TYPE_INT32, &x,
                ...
                reply = FcitxDBusPropertyUnknownMethod(msg);
                                               ...
                                               "SetCursorRect")) {
            FcitxLog(DEBUG, "SetCursorRect");
            int x, y, w, h;
            if (dbus_message_get_args(msg, &error, DBUS_TYPE_INT32, &x,
                                      DBUS_TYPE_INT32, &y, DBUS_TYPE_INT32, &w,
                ...
                reply = FcitxDBusPropertyUnknownMethod(msg);
                                               ...
                                               "SetCapacity")) {
            FcitxLog(DEBUG, "SetCapacity");
            uint32_t flags;
            if (dbus_message_get_args(msg, &error, DBUS_TYPE_UINT32, &flags,
                                      DBUS_TYPE_INVALID)) {
            ...
            result = DBUS_HANDLER_RESULT_HANDLED;
                                               ...
                                               "SetSurroundingText")) {
            FcitxLog(DEBUG, "SetSurroundingText");
            char *text;
            uint32_t cursor, anchor;
            if (dbus_message_get_args(
                    msg, &error, DBUS_TYPE_STRING, &text, DBUS_TYPE_UINT32,
                    &cursor, DBUS_TYPE_UINT32, &anchor, DBUS_TYPE_INVALID)) {
                FcitxIPCIC *ipcic = GetIPCIC(ic);
                if (!ipcic->surroundingText ||
                    strcmp(ipcic->surroundingText, text) != 0 ||
                    cursor != ipcic->cursor || anchor != ipcic->anchor) {
                    fcitx_utils_free(ipcic->surroundingText);
            ...
            FcitxLog(DEBUG, "SetSurroundingTextPosition");
            uint32_t cursor, anchor;
            if (dbus_message_get_args(msg, &error, DBUS_TYPE_UINT32, &cursor,
                                      DBUS_TYPE_UINT32, &anchor,
                                      DBUS_TYPE_INVALID)) {
                FcitxIPCIC *ipcic = GetIPCIC(ic);
            ...
            result = DBUS_HANDLER_RESULT_HANDLED;
                                               ...
                                               "DestroyIC")) {
            FcitxLog(DEBUG, "DestroyIC");
            FcitxInstanceDestroyIC(ipc->owner, ipc->frontendid, &id);
            reply = dbus_message_new_method_return(msg);
            result = DBUS_HANDLER_RESULT_HANDLED;
        } else if (dbus_message_is_method_call(msg, FCITX_IC_DBUS_INTERFACE,
                                               "ProcessKeyEvent")) {
            FcitxLog(DEBUG, "ProcessKeyEvent");
            uint32_t keyval, keycode, state, t;
            int ret, itype;
            FcitxKeyEventType type;
            if (dbus_message_get_args(
                    msg, &error, DBUS_TYPE_UINT32, &keyval, DBUS_TYPE_UINT32,
                    &keycode, DBUS_TYPE_UINT32, &state, DBUS_TYPE_INT32, &itype,
                    DBUS_TYPE_UINT32, &t, DBUS_TYPE_INVALID)) {
                type = itype;
                ret = IPCProcessKey(ipc, ic, keyval, keycode, state, t, type);

                reply = dbus_message_new_method_return(msg);

                dbus_message_append_args(reply, DBUS_TYPE_INT32, &ret,
                                         DBUS_TYPE_INVALID);
                flush = true;
            } else {
                reply = FcitxDBusPropertyUnknownMethod(msg);
...
static int IPCProcessKey(FcitxIPCFrontend *ipc, FcitxInputContext *callic,
    ...
    if (ic == NULL || ic->frontendid != callic->frontendid ||
        GetIPCIC(ic)->id != GetIPCIC(callic)->id) {
    ...
    if (ic->state == IS_CLOSED && type == FCITX_PRESS_KEY &&
        FcitxHotkeyIsHotKey(sym, state, config->hkTrigger)) {
        FcitxInstanceEnableIM(ipc->owner, ic, false);
        return 1;
    ...
    INPUT_RETURN_VALUE retVal =
        FcitxInstanceProcessKey(ipc->owner, type, t, sym, state);
...
void IPCUpdatePreedit(void *arg, FcitxInputContext *ic) {
    ...
    if (ic->contextCaps & CAPACITY_FORMATTED_PREEDIT) {
        ...
        dbus_message_iter_open_container(&args, DBUS_TYPE_ARRAY, "(si)",
                                         &array);
        int i = 0;
        for (i = 0; i < FcitxMessagesGetMessageCount(clientPreedit); i++) {
            ...
            char *str = FcitxMessagesGetMessageString(clientPreedit, i);
            char *needtofree =
                FcitxInstanceProcessOutputFilter(ipc->owner, str);
            if (needtofree) {
                str = needtofree;
            }
        ...
        char *strPreedit =
            FcitxUIMessagesToCString(FcitxInputStateGetClientPreedit(input));
        ...
        dbus_message_append_args(msg, DBUS_TYPE_STRING, &strPreedit,
...
void IPCDeleteSurroundingText(void *arg, FcitxInputContext *ic, int offset,
                              unsigned int size) {
    FcitxIPCFrontend *ipc = (FcitxIPCFrontend *)arg;
    FcitxIPCIC *ipcic = GetIPCIC(ic);
     ...
     * itself would expect a up to date value after this call.
     *
    ...
    if (ipcic->surroundingText) {
        ...
        if (cursor_pos >= 0 && len - cursor_pos >= size) {
             ...
             */
            char *start =
                fcitx_utf8_get_nth_char(ipcic->surroundingText, cursor_pos);
    ...
    dbus_message_append_args(msg, DBUS_TYPE_INT32, &offset, DBUS_TYPE_UINT32,
                             &size, DBUS_TYPE_INVALID);

    IPCSendSignal(ipc, GetIPCIC(ic), msg);
}

boolean IPCGetSurroundingText(void *arg, FcitxInputContext *ic, char **str,
...
void IPCUpdateClientSideUI(void *arg, FcitxInputContext *ic) {
    ...
    char *strAuxDown =
        FcitxUIMessagesToCString(FcitxInputStateGetAuxDown(input));
    ...
    char *strPreedit =
        FcitxUIMessagesToCString(FcitxInputStateGetPreedit(input));
    ...
    dbus_message_append_args(msg, DBUS_TYPE_STRING, &strAuxUp, DBUS_TYPE_STRING,
                             &strAuxDown, DBUS_TYPE_STRING, &strPreedit,
                             DBUS_TYPE_STRING, &candidateword, DBUS_TYPE_STRING,
                             &imname, DBUS_TYPE_INT32, &iCursorPos,
                             DBUS_TYPE_INVALID);
...
boolean IPCCheckICFromSameApplication(void *arg, FcitxInputContext *icToCheck,
                                      FcitxInputContext *ic) {
    FCITX_UNUSED(arg);
...
void IPCGetPropertyIMList(void *arg, DBusMessageIter *args) {
    ...
    for (ime = (FcitxIM *)utarray_front(availimes); ime != NULL;
        ...
        if (!FcitxInstanceGetIMFromIMList(instance, IMAS_Enable,
            ...
            char *langCode = ime->langCode;
            dbus_message_iter_append_basic(&ssub, DBUS_TYPE_STRING, &name);
            dbus_message_iter_append_basic(&ssub, DBUS_TYPE_STRING,
                                           &uniqueName);
            dbus_message_iter_append_basic(&ssub, DBUS_TYPE_STRING, &langCode);
            dbus_message_iter_append_basic(&ssub, DBUS_TYPE_BOOLEAN, &enable);
            dbus_message_iter_close_container(&sub, &ssub);
...
void IPCUpdateIMList(void *arg) { IPCEmitPropertyChanged(arg, "IMList"); }
...
void IPCUpdateIMInfoForIC(void *arg) {
    ...
    if (ic && (ic->contextCaps & CAPACITY_GET_IM_INFO_ON_FOCUS) &&
        ic->frontendid == ipc->frontendid) {
        FcitxIM *im = FcitxInstanceGetCurrentIM(ipc->owner);
        const char *name =
            (im && im->strName && fcitx_utf8_check_string(im->strName))
                ? im->strName
                : "";
        const char *uniqueName =
            (im && im->uniqueName && fcitx_utf8_check_string(im->uniqueName))
                ? im->uniqueName
                : "";
        const char *langCode =
            (im && fcitx_utf8_check_string(im->langCode)) ? im->langCode : "";

        if (fcitx_utils_strcmp0(GetIPCIC(ic)->lastSentIMInfo.name, name) == 0 &&
            fcitx_utils_strcmp0(GetIPCIC(ic)->lastSentIMInfo.uniqueName,
                                uniqueName) == 0 &&
            fcitx_utils_strcmp0(GetIPCIC(ic)->lastSentIMInfo.langCode,
                                langCode) == 0) {
            return;
        }
        ...
        fcitx_utils_string_swap(&GetIPCIC(ic)->lastSentIMInfo.uniqueName,
                                uniqueName);
        fcitx_utils_string_swap(&GetIPCIC(ic)->lastSentIMInfo.langCode,
                                langCode);

        dbus_message_append_args(msg, DBUS_TYPE_STRING, &name, DBUS_TYPE_STRING,
                                 &uniqueName, DBUS_TYPE_STRING, &langCode,
                                 DBUS_TYPE_INVALID);
        IPCSendSignal(ipc, GetIPCIC(ic), msg);
    }
}
