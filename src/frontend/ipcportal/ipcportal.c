...
static void PortalCommitString(void *arg, FcitxInputContext *ic,
                               const char *str);
static void PortalForwardKey(void *arg, FcitxInputContext *ic,
                             FcitxKeyEventType event, FcitxKeySym sym,
                             unsigned int state);
static void PortalSetWindowOffset(void *arg, FcitxInputContext *ic, int x,
                                  int y);
static void PortalGetWindowRect(void *arg, FcitxInputContext *ic, int *x,
                                int *y, int *w, int *h);
static void PortalUpdatePreedit(void *arg, FcitxInputContext *ic);
static void PortalDeleteSurroundingText(void *arg, FcitxInputContext *ic,
                                        int offset, unsigned int size);
static boolean PortalGetSurroundingText(void *arg, FcitxInputContext *ic,
                                        char **str, unsigned int *cursor,
                                        unsigned int *anchor);
static DBusHandlerResult PortalDBusEventHandler(DBusConnection *connection,
                                                DBusMessage *message,
                                                void *user_data);
static DBusHandlerResult PortalICDBusEventHandler(DBusConnection *connection,
                                                  DBusMessage *msg,
                                                  void *user_data);
...
static void PortalICSetCursorRect(FcitxPortalFrontend *ipc,
                                  FcitxInputContext *ic, int x, int y, int w,
                                  int h);
static int PortalProcessKey(FcitxPortalFrontend *ipc, FcitxInputContext *callic,
                            const uint32_t originsym, const uint32_t keycode,
                            const uint32_t originstate, uint32_t t,
                            FcitxKeyEventType type);
static boolean PortalCheckICFromSameApplication(void *arg,
...
FCITX_DEFINE_PLUGIN(fcitx_ipcportal, frontend,
                    FcitxFrontend) = {PortalCreate,
                                      PortalDestroy,
                                      PortalCreateIC,
                                      PortalCheckIC,
                                      PortalDestroyIC,
...
void *PortalCreate(FcitxInstance *instance, int frontendid) {
    ...
    int ret = dbus_bus_request_name(ipc->_conn, FCITX_PORTAL_SERVICE, 0, NULL);
    if (DBUS_REQUEST_NAME_REPLY_PRIMARY_OWNER != ret) {
        FcitxLog(INFO, "Portal Service exists.");
    }

    DBusObjectPathVTable fcitxPortalVTable = {
        NULL, &PortalDBusEventHandler, NULL, NULL, NULL, NULL};
    dbus_connection_register_object_path(ipc->_conn, FCITX_IM_DBUS_PORTAL_PATH,
                                         &fcitxPortalVTable, ipc);
    dbus_connection_register_object_path(
        ipc->_conn, FCITX_IM_DBUS_PORTAL_PATH_OLD, &fcitxPortalVTable, ipc);
...
void PortalCreateIC(void *arg, FcitxInputContext *context, void *priv) {
    ...
    if (config->shareState == ShareState_PerProgram) {
        FcitxInstanceSetICStateFromSameApplication(ipc->owner, ipc->frontendid,
                                                   context);
    }
    ...
    DBusObjectPathVTable vtable = {
        NULL, &PortalICDBusEventHandler, NULL, NULL, NULL, NULL};
...
void PortalCommitString(void *arg, FcitxInputContext *ic, const char *str) {
    ...
    DBusMessage *msg = dbus_message_new_signal(
        GetPortalIC(ic)->path,   // object name of the signal
...
void PortalForwardKey(void *arg, FcitxInputContext *ic, FcitxKeyEventType event,
    ...
    dbus_message_append_args(msg, DBUS_TYPE_UINT32, &keyval, DBUS_TYPE_UINT32,
                             &keystate, DBUS_TYPE_BOOLEAN, &type,
                             DBUS_TYPE_INVALID);
    PortalSendSignal(ipc, msg);
}
...
void PortalGetWindowRect(void *arg, FcitxInputContext *ic, int *x, int *y,
                         int *w, int *h) {
    FCITX_UNUSED(arg);
    *x = ic->offset_x;
    *y = ic->offset_y;
...
static DBusHandlerResult PortalICDBusEventHandler(DBusConnection *connection,
    ...
    if (!reply && ic) {
        ...
        if (strcmp(dbus_message_get_sender(msg), GetPortalIC(ic)->sender) !=
            0) {
            ...
            if (dbus_message_get_args(msg, &error, DBUS_TYPE_INT32, &x,
                ...
                reply = dbus_message_new_method_return(msg);
                                               ...
                                               "SetCapability")) {
            uint64_t flags;
            if (dbus_message_get_args(msg, &error, DBUS_TYPE_UINT64, &flags,
                                      DBUS_TYPE_INVALID)) {
            ...
            if (dbus_message_get_args(
                    ...
                    &keycode, DBUS_TYPE_UINT32, &state, DBUS_TYPE_BOOLEAN,
                    &itype, DBUS_TYPE_UINT32, &t, DBUS_TYPE_INVALID)) {
                type = itype;
                ret = PortalProcessKey(ipc, ic, keyval, keycode, state, t,
                                       type) > 0;
...
static int PortalProcessKey(FcitxPortalFrontend *ipc, FcitxInputContext *callic,
    ...
    if (ic == NULL || ic->frontendid != callic->frontendid ||
        GetPortalIC(ic)->id != GetPortalIC(callic)->id) {
    ...
    FcitxLog(DEBUG, "KeyRelease=%d  state=%d  KEYCODE=%d  KEYSYM=%u ",
...
void PortalDeleteSurroundingText(void *arg, FcitxInputContext *ic, int offset,
                                 unsigned int size) {
    FcitxPortalFrontend *ipc = (FcitxPortalFrontend *)arg;
    FcitxPortalIC *ipcic = GetPortalIC(ic);
...
boolean PortalGetSurroundingText(void *arg, FcitxInputContext *ic, char **str,
...
boolean PortalCheckICFromSameApplication(void *arg,
                                         FcitxInputContext *icToCheck,
                                         FcitxInputContext *ic) {
    FCITX_UNUSED(arg);
...
void PortalUpdateIMInfoForIC(void *arg) {
    ...
    if (ic && (ic->contextCaps & CAPACITY_GET_IM_INFO_ON_FOCUS) &&
        ...
        if (fcitx_utils_strcmp0(GetPortalIC(ic)->lastSentIMInfo.name, name) ==
                0 &&
            fcitx_utils_strcmp0(GetPortalIC(ic)->lastSentIMInfo.uniqueName,
                                uniqueName) == 0 &&
            fcitx_utils_strcmp0(GetPortalIC(ic)->lastSentIMInfo.langCode,
                                langCode) == 0) {
            return;
        }
        ...
        fcitx_utils_string_swap(&GetPortalIC(ic)->lastSentIMInfo.uniqueName,
                                uniqueName);
        fcitx_utils_string_swap(&GetPortalIC(ic)->lastSentIMInfo.langCode,
                                langCode);
