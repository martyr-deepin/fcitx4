...
static inline FcitxXkbLayoutInfo *FindByName(FcitxXkbRules *rules,
                                             const char *name);
static void MergeRules(FcitxXkbRules *rules, FcitxXkbRules *rulesextra);

static const UT_icd layout_icd = {
    sizeof(FcitxXkbLayoutInfo), FcitxXkbLayoutInfoInit, FcitxXkbLayoutInfoCopy,
    FcitxXkbLayoutInfoFree};
...
static const UT_icd model_icd = {sizeof(FcitxXkbModelInfo),
                                 FcitxXkbModelInfoInit, FcitxXkbModelInfoCopy,
                                 FcitxXkbModelInfoFree};
...
static const UT_icd option_icd = {
    sizeof(FcitxXkbOptionInfo), FcitxXkbOptionInfoInit, FcitxXkbOptionInfoCopy,
    FcitxXkbOptionInfoFree};
...
void MergeRules(FcitxXkbRules *rules, FcitxXkbRules *rulesextra) {
    ...
    for (layoutInfo =
             (FcitxXkbLayoutInfo *)utarray_front(rulesextra->layoutInfos);
         layoutInfo != NULL; layoutInfo = (FcitxXkbLayoutInfo *)utarray_next(
                                 rulesextra->layoutInfos, layoutInfo)) {
    ...
    for (i = 0; i < utarray_len(&toAdd); i++) {
        FcitxXkbLayoutInfo *p =
            *(FcitxXkbLayoutInfo **)utarray_eltptr(&toAdd, i);
...
char *FcitxXkbRulesToReadableString(FcitxXkbRules *rules) {
    ...
    for (layoutInfo = (FcitxXkbLayoutInfo *)utarray_front(rules->layoutInfos);
         layoutInfo != NULL; layoutInfo = (FcitxXkbLayoutInfo *)utarray_next(
                                 rules->layoutInfos, layoutInfo)) {
        fcitx_utils_string_list_printf_append(list, "\tLayout Name: %s",
                                              layoutInfo->name);
        fcitx_utils_string_list_printf_append(list, "\tLayout Description: %s",
                                              layoutInfo->description);
        char *languages =
            fcitx_utils_join_string_list(layoutInfo->languages, ',');
        fcitx_utils_string_list_printf_append(list, "\tLayout Languages: %s",
                                              languages);
        free(languages);
        for (variantInfo =
                 (FcitxXkbVariantInfo *)utarray_front(layoutInfo->variantInfos);
             variantInfo != NULL;
             variantInfo = (FcitxXkbVariantInfo *)utarray_next(
                 layoutInfo->variantInfos, variantInfo)) {
            fcitx_utils_string_list_printf_append(list, "\t\tVariant Name: %s",
                                                  variantInfo->name);
            fcitx_utils_string_list_printf_append(
                list, "\t\tVariant Description: %s", variantInfo->description);
            char *languages =
                fcitx_utils_join_string_list(variantInfo->languages, ',');
            fcitx_utils_string_list_printf_append(
                list, "\t\tVariant Languages: %s", languages);
            free(languages);
        }
    }

    for (modelInfo = (FcitxXkbModelInfo *)utarray_front(rules->modelInfos);
         modelInfo != NULL; modelInfo = (FcitxXkbModelInfo *)utarray_next(
                                rules->modelInfos, modelInfo)) {
        fcitx_utils_string_list_printf_append(list, "\tModel Name: %s",
                                              modelInfo->name);
        fcitx_utils_string_list_printf_append(list, "\tModel Description: %s",
                                              modelInfo->description);
        fcitx_utils_string_list_printf_append(list, "\tModel Vendor: %s",
                                              modelInfo->vendor);
    }

    for (optionGroupInfo =
             (FcitxXkbOptionGroupInfo *)utarray_front(rules->optionGroupInfos);
         optionGroupInfo != NULL;
         optionGroupInfo = (FcitxXkbOptionGroupInfo *)utarray_next(
             rules->optionGroupInfos, optionGroupInfo)) {
        fcitx_utils_string_list_printf_append(list, "\tOption Group Name: %s",
                                              optionGroupInfo->name);
        fcitx_utils_string_list_printf_append(list,
                                              "\tOption Group Description: %s",
                                              optionGroupInfo->description);
        fcitx_utils_string_list_printf_append(
            list, "\tOption Group Exclusive: %d", optionGroupInfo->exclusive);
        for (optionInfo = (FcitxXkbOptionInfo *)utarray_front(
                 optionGroupInfo->optionInfos);
             optionInfo != NULL;
             optionInfo = (FcitxXkbOptionInfo *)utarray_next(
                 optionGroupInfo->optionInfos, optionInfo)) {
            fcitx_utils_string_list_printf_append(list, "\t\tOption Name: %s",
                                                  optionInfo->name);
            fcitx_utils_string_list_printf_append(
                list, "\t\tOption Description: %s", optionInfo->description);
        }
    }
...
void RulesHandlerStartElement(void *ctx, const xmlChar *name,
    ...
    if (StringEndsWith(strPath, "layoutList/layout/configItem")) {
    ...
    } else if (StringEndsWith(strPath,
                              "layoutList/layout/variantList/variant")) {
        FcitxXkbLayoutInfo *layoutInfo =
            (FcitxXkbLayoutInfo *)utarray_back(rules->layoutInfos);
        ...
        FcitxXkbOptionGroupInfo *optionGroupInfo =
            (FcitxXkbOptionGroupInfo *)utarray_back(rules->optionGroupInfos);
        int i = 0;
        while (atts && atts[i * 2] != 0) {
            if (strcmp(XMLCHAR_CAST atts[i * 2], "allowMultipleSelection") ==
                0) {
                optionGroupInfo->exclusive =
                    (strcmp(XMLCHAR_CAST atts[i * 2 + 1], "true") != 0);
            }
            i++;
        }
        ...
        int i = 0;
        while (atts && atts[i * 2] != 0) {
            if (strcmp(XMLCHAR_CAST atts[i * 2], "version") == 0 &&
                strlen(XMLCHAR_CAST atts[i * 2 + 1]) != 0) {
...
void RulesHandlerCharacters(void *ctx, const xmlChar *ch, int len) {
    FcitxXkbRulesHandler *ruleshandler = (FcitxXkbRulesHandler *)ctx;
    FcitxXkbRules *rules = ruleshandler->rules;
    ...
    if (strlen(trimmed) != 0) {
        ...
        FcitxXkbLayoutInfo *layoutInfo =
            (FcitxXkbLayoutInfo *)utarray_back(rules->layoutInfos);
        FcitxXkbModelInfo *modelInfo =
            (FcitxXkbModelInfo *)utarray_back(rules->modelInfos);
        FcitxXkbOptionGroupInfo *optionGroupInfo =
            (FcitxXkbOptionGroupInfo *)utarray_back(rules->optionGroupInfos);
        if (StringEndsWith(strPath, "layoutList/layout/configItem/name")) {
            if (layoutInfo != NULL)
                layoutInfo->name = strdup(trimmed);
                                  ...
                                  "layoutList/layout/configItem/description")) {
            layoutInfo->description = strdup(trimmed);
        } else if (StringEndsWith(
                       strPath,
                       "layoutList/layout/configItem/languageList/iso639Id")) {
            utarray_push_back(layoutInfo->languages, &trimmed);
        } else if (StringEndsWith(strPath, "layoutList/layout/variantList/"
                                           "variant/configItem/name")) {
            FcitxXkbVariantInfo *variantInfo =
                (FcitxXkbVariantInfo *)utarray_back(layoutInfo->variantInfos);
            variantInfo->name = strdup(trimmed);
        } else if (StringEndsWith(strPath, "layoutList/layout/variantList/"
                                           "variant/configItem/description")) {
            FcitxXkbVariantInfo *variantInfo =
                (FcitxXkbVariantInfo *)utarray_back(layoutInfo->variantInfos);
            ...
            modelInfo->name = strdup(trimmed);
                                  ...
                                  "modelList/model/configItem/description")) {
            modelInfo->description = strdup(trimmed);
        } else if (StringEndsWith(strPath,
                                  "modelList/model/configItem/vendor")) {
            modelInfo->vendor = strdup(trimmed);
        } else if (StringEndsWith(strPath,
                                  "optionList/group/configItem/name")) {
            optionGroupInfo->name = strdup(trimmed);
        } else if (StringEndsWith(strPath,
                                  "optionList/group/configItem/description")) {
            optionGroupInfo->description = strdup(trimmed);
        } else if (StringEndsWith(strPath,
                                  "optionList/group/option/configItem/name")) {
            FcitxXkbOptionInfo *optionInfo = (FcitxXkbOptionInfo *)utarray_back(
                optionGroupInfo->optionInfos);
            optionInfo->name = strdup(trimmed);
        } else if (StringEndsWith(
                       strPath,
                       "optionList/group/option/configItem/description")) {
            FcitxXkbOptionInfo *optionInfo = (FcitxXkbOptionInfo *)utarray_back(
                optionGroupInfo->optionInfos);
...
void FcitxXkbOptionGroupInfoCopy(void *dst, const void *src) {
    FcitxXkbOptionGroupInfo *optionGroupInfoDst =
        (FcitxXkbOptionGroupInfo *)dst;
    FcitxXkbOptionGroupInfo *optionGroupInfoSrc =
        (FcitxXkbOptionGroupInfo *)src;
    optionGroupInfoDst->name = COPY_IF_NOT_NULL(optionGroupInfoSrc->name);
    optionGroupInfoDst->description =
        COPY_IF_NOT_NULL(optionGroupInfoSrc->description);
    optionGroupInfoDst->exclusive = optionGroupInfoSrc->exclusive;
    optionGroupInfoDst->optionInfos =
        utarray_clone(optionGroupInfoSrc->optionInfos);
}
