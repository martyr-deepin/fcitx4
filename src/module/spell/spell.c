...
FCITX_DEFINE_PLUGIN(fcitx_spell, module,
                    FcitxModule) = {.Create = SpellCreate,
                                    .Destroy = SpellDestroy,
                                    .SetFD = NULL,
                                    .ProcessEvent = NULL,
                                    .ReloadConfig = SpellReloadConfig};

static void SaveSpellConfig(FcitxSpellConfig *fs) {
    FcitxConfigFileDesc *configDesc = GetSpellConfigDesc();
    FILE *fp =
        FcitxXDGGetFileUserWithPrefix("conf", "fcitx-spell.config", "w", NULL);
    FcitxConfigSaveConfigFileFp(fp, &fs->gconfig, configDesc);
    if (fp)
        fclose(fp);
}

static boolean LoadSpellConfig(FcitxSpellConfig *config) {
    FcitxConfigFileDesc *configDesc = GetSpellConfigDesc();
    if (configDesc == NULL)
        return false;

    FILE *fp =
        FcitxXDGGetFileUserWithPrefix("conf", "fcitx-spell.config", "r", NULL);
...
static int SpellHintStrLengths(int count, char **strs, int sizes, int *lens) {
    int i;
    int total = 0;
    if (!strs) {
    ...
    for (i = 0; i < count; i++)
        total += lens[i] =
            ((GET_NTH(strs, i, sizes)) ? (strlen(GET_NTH(strs, i, sizes)) + 1)
                                       : 0);
    return total;
}
...
static const char *SpellParseNextProvider(const char *str, const char **name,
                                          int *len) {
    char *p;
    if (!name || !len)
        return str;
...
static const SpellHintProvider *SpellFindHintProvider(const char *str,
                                                      unsigned int len) {
    int i;
    if (!str || !len)
        return NULL;
...
static boolean SpellAddPersonal(FcitxSpell *spell, const char *new_word,
                                const char *lang) {
...
static boolean FcitxSpellDictAvailable(FcitxSpell *spell, const char *lang,
                                       const char *providers) {
    const char *iter = providers ? providers : spell->provider_order;
    const SpellHintProvider *hint_provider;
    const char *name = NULL;
...
static const char *FcitxSpellCandWordGetCommit(FcitxSpell *spell,
...
static INPUT_RETURN_VALUE FcitxSpellGetCandWord(void *arg,
                                                FcitxCandidateWord *candWord) {
...
static void *SpellNewGetCandWordArgs(FcitxSpellGetCandWordCb cb, void *arg,
                                     const char *commit) {
    int len;
    void *res;
...
static FcitxCandidateWordList *SpellGetCandWords(
    FcitxSpell *spell, const char *before_str, const char *current_str,
    const char *after_str, unsigned int len_limit, const char *lang,
    ...
    FcitxCandidateWord candWord = {.callback = FcitxSpellGetCandWord,
                                   .owner = arg,
                                   .strExtra = NULL,
                                   .wordType = MSG_OTHER};
