...
static int (*_presage_new)(const char *(*past_stream_cb)(void *),
                           ...
                           void *future_stream_cb_arg, void **result) = NULL;
static int (*_presage_config_set)(void *prsg, const char *variable,
...
static boolean SpellPresageLoadLib() {
    if (_presage_handle)
        return true;
    _presage_handle = dlopen(PRESAGE_LIBRARY_FILENAME,
                             RTLD_NOW | RTLD_NODELETE | RTLD_GLOBAL);
    if (!_presage_handle)
        goto fail;
#define PRESAGE_LOAD_SYMBOL(sym)                                               \
    do {                                                                       \
...
static const char *FcitxSpellGetPastStream(void *arg) {
...
static const char *FcitxSpellGetFutureStream(void *arg) {
    FcitxSpell *spell = (FcitxSpell *)arg;
    return spell->after_str;
}

static SpellHint *SpellPresageResult(FcitxSpell *spell, char **suggestions) {
    int i;
    int count = 0;
...
boolean SpellPresageInit(FcitxSpell *spell) {
    ...
    if (!SpellPresageLoadLib())
        return false;
    _presage_new(FcitxSpellGetPastStream, spell, FcitxSpellGetFutureStream,
                 spell, &spell->presage);
...
SpellHint *SpellPresageHintWords(FcitxSpell *spell, unsigned int len_limit) {
    ...
    do {
        ...
        _presage_config_set(spell->presage, "Presage.Selector.SUGGESTIONS",
                            buf);
