...
static boolean
LoadFcitxNotificationItemConfig(FcitxNotificationItem2 *notificationitem2);
static void
SaveFcitxNotificationItemConfig(FcitxNotificationItem2 *notificationitem2);

static void FcitxNotificationItemDestroy(void *arg);

FCITX_DEFINE_PLUGIN(fcitx_notificationitem, module,
...
static boolean
FcitxNotificationItemEnable(FcitxNotificationItem *notificationitem,
                            FcitxNotificationItemAvailableCallback callback,
                            void *data);
static void
FcitxNotificationItemDisable(FcitxNotificationItem *notificationitem);
static void
FcitxNotificationItemSetAvailable(FcitxNotificationItem *notificationitem,
                                  boolean available);
...
void *FcitxNotificationItemCreate(FcitxInstance *instance) {
    ...
    do {
        ...
        int id = FcitxDBusWatchName(
            instance, NOTIFICATION_WATCHER_DBUS_ADDR, notificationitem,
        ...
        dbus_bool_t reply = dbus_connection_send_with_reply(
            notificationitem->conn, message, &call, DBUS_TIMEOUT_USE_DEFAULT);
        if (reply == TRUE) {
            dbus_pending_call_set_notify(
                call, NotificationWatcherServiceExistCallback, notificationitem,
                NULL);
...
LoadFcitxNotificationItemConfig(FcitxNotificationItem2 *notificationitem2) {
    ...
    if (configDesc == NULL)
        return false;
    FILE *fp;
    fp = FcitxXDGGetFileUserWithPrefix("conf", "fcitx-notificationitem.config",
                                       "r", NULL);
    if (!fp) {
        if (errno == ENOENT)
...
void SaveFcitxNotificationItemConfig(
    FcitxNotificationItem2 *notificationitem2) {
    FcitxConfigFileDesc *configDesc = GetFcitxNotificationItemConfig();
    FILE *fp = FcitxXDGGetFileUserWithPrefix(
        "conf", "fcitx-notificationitem.config", "w", NULL);
    ...
    if (notificationitem->conn) {
        if (notificationitem->callback) {
            dbus_connection_unregister_object_path(
                notificationitem->conn, NOTIFICATION_ITEM_DEFAULT_OBJ);
        }
        dbus_connection_unregister_object_path(notificationitem->conn,
                                               "/MenuBar");
    }
...
void FcitxNotificationItemRegister(FcitxNotificationItem *notificationitem) {
    ...
    dbus_message_append_args(message, DBUS_TYPE_STRING,
                             &notificationitem->serviceName, DBUS_TYPE_INVALID);
...
DBusHandlerResult FcitxNotificationItemEventHandler(DBusConnection *connection,
    ...
    if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE,
        ...
        FcitxInstanceChangeIMState(
            ...
            FcitxInstanceGetCurrentIC(notificationitem->owner));
        reply = dbus_message_new_method_return(message);
        ...
        reply = FcitxDBusPropertySet(notificationitem, propertTable, message);
                                           ...
                                           "GetAll")) {
        reply =
            FcitxDBusPropertyGetAll(notificationitem, propertTable, message);
    }
...
char *FcitxNotificationItemGetIconNameString(
    ...
    char *iconName = NULL;
    FcitxIM *im =
        FcitxInstanceGetIM(notificationitem->owner,
                           FcitxInstanceGetLastIC(notificationitem->owner));
...
const char *
FcitxNotificationItemGetLabel(FcitxNotificationItem *notificationitem) {
    const char *label = "";
    ...
    if (ic) {
        ...
        if (im) {
            if (strncmp(im->uniqueName, "fcitx-keyboard-",
                        strlen("fcitx-keyboard-")) == 0) {
                strncpy(notificationitem->layoutNameBuffer,
...
FcitxNotificationItemEnable(FcitxNotificationItem *notificationitem,
    ...
    dbus_connection_register_object_path(notificationitem->conn,
                                         NOTIFICATION_ITEM_DEFAULT_OBJ,
                                         &fcitxIPCVTable, notificationitem);
    notificationitem->callback = callback;
    notificationitem->data = data;
    asprintf(&notificationitem->serviceName, "org.kde.StatusNotifierItem-%u-%d",
             getpid(), ++notificationitem->index);
...
void FcitxNotificationItemDisable(FcitxNotificationItem *notificationitem) {
    ...
    if (notificationitem->serviceName) {
        dbus_bus_release_name(notificationitem->conn,
