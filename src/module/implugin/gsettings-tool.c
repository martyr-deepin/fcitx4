...
static void get_file_path() {
    FILE *conf_fp =
        FcitxXDGGetFileUserWithPrefix("", "config", "r", &global_conf_fp);
    if (!conf_fp) {
        global_conf_fp = NULL;
    }
    fclose(conf_fp);
...
static void fcitxconfig_value_changed(GSettings *settings, const gchar *key,
                                      gpointer user_data) {
    GVariant *value;
    gchar *printed;
    ...
    //  char *commod[] =
    //  {"/home/uos/Desktop/fcitx/build/src/module/dbus/fcitx-remote", "-r"};
...
static void gsettings_fcitxconfig_monitor(void) {
    if (global_key) {
        gchar *name;

        name = g_strdup_printf("changed::%s", global_key);
        g_signal_connect(global_settings, name,
                         G_CALLBACK(fcitxconfig_value_changed), NULL);
    } else
        g_signal_connect(global_settings, "changed",
                         G_CALLBACK(fcitxconfig_value_changed), NULL);
...
int gsetting_tool(int argc, char **argv) {
    void (*function)(void);
...
//  bindtextdomain (GETTEXT_PACKAGE, GLIB_LOCALE_DIR);
    ...
    if (strcmp(argv[1], "help") == 0)
        ...
        function = gsettings_monitor;
             ...
             strcmp(argv[1], "fcitxconfig-monitor") == 0)
