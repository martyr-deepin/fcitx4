...
char *global_conf_fp = NULL;
char *global_desc_fp = NULL;

static void
get_file_path()
{
    FILE *conf_fp = FcitxXDGGetFileUserWithPrefix("", "config", "r", &global_conf_fp);
    if(!conf_fp){
        global_conf_fp = NULL;
    }
    fclose(conf_fp);
  ...
  if (command == NULL)
    {
      ...
      _("Usage:\n"
        ...
        "  fcitxconfig-monitor       Watch for changes\n"
        "\n"
 ...
 * 文件注释检查并修改
 */
static void file_comment_modify(char *filename) {
    int fd = 0;
    int len = 0;
    char str[BUFSIZ] = {0};
...
static void
fcitxconfig_value_changed (GSettings   *settings,
               const gchar *key,
               gpointer     user_data)
{
  GVariant *value;
  ...
  if(strcmp(key, "shortcut-switch-to-first") == 0)
  {
    ...
    if(strcmp(printed,"\'Disabled\'")==0)
    {
        if(global_conf_fp != NULL){
         ...
         ini_puts("Hotkey/TriggerKey","DefaultValue", "", global_desc_fp);
    }
...
static void
gsettings_fcitxconfig_monitor (void)
{
  if (global_key)
    {
      gchar *name;
      ...
      name = g_strdup_printf ("changed::%s", global_key);
      g_signal_connect (global_settings, name, G_CALLBACK (fcitxconfig_value_changed), NULL);
    }
  else
    g_signal_connect (global_settings, "changed", G_CALLBACK (fcitxconfig_value_changed), NULL);
...
int
gsetting_tool (int argc, char **argv)
{
  void (* function) (void);
...
//  bindtextdomain (GETTEXT_PACKAGE, GLIB_LOCALE_DIR);
  ...
  else if ((argc == 3 || argc == 4) && strcmp (argv[1], "fcitxconfig-monitor") == 0)
    function = gsettings_fcitxconfig_monitor;
...
{
    argc = 3;
    argv[1]="fcitxconfig-monitor";
    argv[2]="com.deepin.fcitx-config";

    get_file_path();

    return gsetting_tool (argc, argv);
}
