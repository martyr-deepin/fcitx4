...
UnicodeSet *CharSelectDataGetMatchingChars(CharSelectData *charselect,
                                           const char *s);
...
CharSelectData *CharSelectDataCreate() {
    ...
    do {
        ...
        FILE *fp =
            FcitxXDGGetFileWithPrefix("data", "charselectdata", "r", NULL);
        if (!fp)
            break;
    ...
    do {
        if ((unicode >= 0x3400 && unicode <= 0x4DB5) ||
            ...
            fcitx_utils_alloc_cat_str(
                result, "HANGUL SYLLABLE ", JAMO_L_TABLE[LIndex],
...
UT_array *CharSelectDataFind(CharSelectData *charselect, const char *needle) {
    ...
    utarray_foreach(s2, searchStrings, char *) {
        UnicodeSet *partResult =
            CharSelectDataGetMatchingChars(charselect, *s2);
        if (firstSubString) {
            result = partResult;
            firstSubString = 0;
        } else {
    ...
    // results while letting these characters stay at the beginning
...
UnicodeSet *CharSelectDataGetMatchingChars(CharSelectData *charselect,
    ...
    last =
        utarray_custom_bsearch(s, charselect->indexList, 0, index_search_a_cmp);
    if (!pos)
        return NULL;
    if (!last)
...
UT_array *SplitString(const char *s) {
    ...
    while (end < length) {
        while (end < length &&
               (isdigit(s[end]) || isalpha(s[end]) || s[end] == '+')) {
            end++;
        }
        if (start != end) {
...
void CharSelectDataAppendToIndex(CharSelectData *charselect, uint32_t unicode,
                                 const char *str) {
    ...
    utarray_foreach(s, strings, char *) {
        ...
        if (!item) {
            ...
            HASH_ADD_KEYPTR(hh, charselect->index, item->key, strlen(item->key),
                            item);
        }
...
void CharSelectDataCreateIndex(CharSelectData *charselect) {
    ...
    for (pos = 0; pos <= max; pos++) {
        ...
        for (j = 0; j < aliasCount; j++) {
            CharSelectDataAppendToIndex(charselect, unicode,
                                        data + aliasOffset);
