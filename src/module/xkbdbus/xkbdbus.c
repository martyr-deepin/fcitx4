...
static void FcitxXkbDBusHelperOwnerChanged(void *user_data, void *arg,
                                           const char *serviceName,
                                           const char *oldName,
                                           const char *newName);
static void FcitxXkbDBusHelperServiceExistCallback(DBusPendingCall *call,
                                                   void *data);
...
FCITX_DEFINE_PLUGIN(fcitx_xkbdbus, module, FcitxModule) = {
    FcitxXkbDBusCreate, NULL, NULL, FcitxXkbDBusDestroy, NULL};
...
void *FcitxXkbDBusCreate(FcitxInstance *instance) {
    ...
    do {
        ...
        DBusObjectPathVTable fcitxIPCVTable = {
            NULL, &FcitxXkbDBusEventHandler, NULL, NULL, NULL, NULL};

        if (conn) {
            dbus_connection_register_object_path(conn, FCITX_XKB_PATH,
                                                 &fcitxIPCVTable, xkbdbus);
        }

        if (privconn) {
            dbus_connection_register_object_path(privconn, FCITX_XKB_PATH,
                                                 &fcitxIPCVTable, xkbdbus);
        }
        ...
        dbus_message_append_args(message, DBUS_TYPE_STRING, &helper,
                                 DBUS_TYPE_INVALID);

        DBusPendingCall *call = NULL;
        dbus_bool_t reply = dbus_connection_send_with_reply(
            xkbdbus->conn, message, &call, DBUS_TIMEOUT_USE_DEFAULT);
        if (reply == TRUE) {
            dbus_pending_call_set_notify(
                call, FcitxXkbDBusHelperServiceExistCallback, xkbdbus, NULL);
...
void FcitxXkbDBusDestroy(void *arg) {
    ...
    if (xkbdbus->privconn) {
        dbus_connection_unregister_object_path(xkbdbus->privconn,
                                               FCITX_XKB_PATH);
    }
...
#define GET_LANG                                                               \
    do {                                                                       \
        ...
        lang = NULL;                                                           \
        if (plang) {                                                           \
            FcitxIsoCodes639Entry *entry =                                     \
                FcitxIsoCodesGetEntry(isocodes, *plang);                       \
            if (entry) {                                                       \
    ...
    if (rules) {
        ...
        for (layoutInfo =
                 (FcitxXkbLayoutInfo *)utarray_front(rules->layoutInfos);
             layoutInfo != NULL;
             layoutInfo = (FcitxXkbLayoutInfo *)utarray_next(rules->layoutInfos,
                                                             layoutInfo)) {
            char *description =
                dgettext("xkeyboard-config", layoutInfo->description);
            GET_LANG;
            FcitxXkbDBusAppendLayout(&sub, layoutInfo->name, "", description,
                                     lang);
...
DBusHandlerResult FcitxXkbDBusEventHandler(DBusConnection *connection,
    ...
    if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE,
        ...
        dbus_message_append_args(reply, DBUS_TYPE_STRING, &introspection_xml,
                                 DBUS_TYPE_INVALID);
    } else if (dbus_message_is_method_call(message, FCITX_XKB_INTERFACE,
                                           "GetLayouts")) {
        reply = dbus_message_new_method_return(message);
        ...
        if (dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &im,
                                  DBUS_TYPE_STRING, &layout, DBUS_TYPE_STRING,
                                  &variant, DBUS_TYPE_INVALID)) {
        ...
        dbus_error_init(&error);
        char *layout, *variant;
        if (dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &layout,
        ...
        if (dbus_message_get_args(message, &error, DBUS_TYPE_STRING, &im,
            ...
            reply = dbus_message_new_method_return(message);
            dbus_message_append_args(reply, DBUS_TYPE_STRING, &layout,
                                     DBUS_TYPE_STRING, &variant,
                                     DBUS_TYPE_INVALID);
        } else {
            reply = FcitxXkbDBusUnknownMethod(message);
...
void FcitxXkbDBusHelperOwnerChanged(void *user_data, void *arg,
                                    const char *serviceName,
...
boolean FcitxXkbDBusLockGroupByHelper(FcitxXkbDBus *xkbdbus, int idx) {
    ...
    msg = dbus_message_new_method_call(GNOME_HELPER_NAME, GNOME_HELPER_PATH,
                                       GNOME_HELPER_INTERFACE, "LockXkbGroup");
    do {
        if (!msg) {
            break;
        }
        if (!dbus_message_append_args(msg, DBUS_TYPE_INT32, &idx,
                                      DBUS_TYPE_INVALID)) {
            break;
        }
