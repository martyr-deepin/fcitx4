...
void X11ProcessXFixesSelectionNotifyEvent(
    ...
    for (; notify;
         notify = fcitx_handler_table_next(x11priv->selectionNotify, notify)) {
        notify->cb(x11priv, notify_event->selection, notify_event->subtype,
                   notify);
    }
}
#endif

void X11SelectionNotifyHelper(FcitxX11 *x11priv, Atom selection, int subtype,
                              X11SelectionNotify *notify) {
#ifdef HAVE_XFIXES
...
int X11SelectionNotifyRegister(FcitxX11 *x11priv, const char *sel_str,
                               void *owner, X11SelectionNotifyCallback cb,
                               void *data, FcitxDestroyNotify destroy) {
#ifdef HAVE_XFIXES
    if (!cb)
...
int X11SelectionNotifyRegisterInternal(FcitxX11 *x11priv, Atom selection,
                                       void *owner,
    ...
    if (!(x11priv->hasXfixes && cb))
        return FCITX_OBJECT_POOL_INVALID_ID;
    // TODO catch bad atom?
    XFixesSelectSelectionInput(x11priv->dpy, x11priv->eventWindow, selection,
                               XFixesSetSelectionOwnerNotifyMask |
                                   XFixesSelectionWindowDestroyNotifyMask |
                                   XFixesSelectionClientCloseNotifyMask);
    X11SelectionNotify notify = {.owner = owner,
                                 .data = data,
                                 .cb = cb,
                                 .destroy = destroy,
                                 .func = func};
    return fcitx_handler_table_append(x11priv->selectionNotify, sizeof(Atom),
                                      &selection, &notify);
#else
    return FCITX_OBJECT_POOL_INVALID_ID;
#endif
...
static boolean X11ConvertSelectionHelper(FcitxX11 *x11priv, Atom selection,
                                         Atom target, int format, size_t nitems,
                                         const void *buff,
...
static boolean X11TextConvertSelectionHelper(FcitxX11 *x11priv, Atom selection,
                                             Atom target, int format,
...
static int X11RequestConvertSelectionInternal(
    ...
    XConvertSelection(x11priv->dpy, selection, target, prop,
                      x11priv->eventWindow, CurrentTime);
    X11ConvertSelection convert = {.owner = owner,
                                   .data = data,
                                   .target = target,
                                   .cb = cb,
                                   .destroy = destroy,
                                   .func = func};
    return fcitx_handler_table_prepend(x11priv->convertSelection, sizeof(Atom),
                                       &selection, &convert);
}

int X11RequestConvertSelection(FcitxX11 *x11priv, const char *sel_str,
                               const char *tgt_str, void *owner,
                               X11ConvertSelectionCallback cb, void *data,
    ...
    return X11RequestConvertSelectionInternal(
        x11priv, sel_str, XInternAtom(x11priv->dpy, sel_str, False), tgt_atom,
        owner, intern_cb, data, destroy, (FcitxCallBack)cb);
}

static unsigned char *X11GetWindowProperty(FcitxX11 *x11priv, Window win,
                                           Atom prop, Atom *ret_type,
                                           int *ret_format,
    ...
    if (prop == None)
        goto fail;
    res = XGetWindowProperty(x11priv->dpy, win, prop, 0, 256, False,
                             AnyPropertyType, ret_type, ret_format, nitems,
                             &bytes_left, &buff);
...
void X11ProcessSelectionNotifyEvent(FcitxX11 *x11priv,
    ...
    for (; (convert = fcitx_handler_table_get_by_id(table, id)); id = next_id) {
        ...
        if (convert->cb(x11priv, selection_event->selection,
                        selection_event->target, ret_format, nitems, buff,
                        convert)) {
