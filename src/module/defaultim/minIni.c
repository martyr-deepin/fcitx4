...
static TCHAR *ini_strncpy(TCHAR *dest, const TCHAR *source, size_t maxlen,
    ...
    case QUOTE_ENQUOTE:
        ...
        for (s = 0; source[s] != '\0' && d < maxlen - 2; s++, d++) {
            if (source[s] == '"') {
                if (d >= maxlen - 3)
                    break; /* no space to store the escape character plus the
                              one that follows it */
                dest[d++] = '\\';
            } /* if */
            dest[d] = source[s];
...
int ini_getkey(const TCHAR *Section, int idx, TCHAR *Buffer, int BufferSize,
    ...
    if (ini_openread(Filename, &fp)) {
        ok =
            getkeystring(&fp, Section, NULL, -1, idx, Buffer, BufferSize, NULL);
        (void)ini_close(&fp);
    } /* if */
    if (!ok)
        *Buffer = '\0';
...
static int cache_flush(TCHAR *buffer, int *size, INI_FILETYPE *rfp,
    ...
    while (pos < *size) {
        ...
        while (pos < *size && buffer[pos] != '\0')
            pos++; /* cannot use _tcslen() because buffer may not be
                      zero-terminated */
    }              /* while */
    if (buffer[0] != '\0') {
        ...
        if (pos == INI_BUFFERSIZE)
            pos--;
        buffer[pos] = '\0'; /* force zero-termination (may be left unterminated
...
int ini_puts(const TCHAR *Section, const TCHAR *Key, const TCHAR *Value,
     ...
     * and the setting already exists, there are two short-cuts to avoid
     * rewriting the INI file.
     */
    if (Key != NULL && Value != NULL) {
 ...
 */
    ...
    if (!ini_openwrite(LocalBuffer, &wfp))
        return 0;
    /* In the case of (advisory) file locks, ini_openwrite() may have been
     * blocked on the open, and after the block is lifted, the original file may
     * have been renamed, which is why the original file was closed and is now
     * reopened */
    if (!ini_openread(Filename, &rfp)) {
    ...
    if (len > 0) {
        do {
            if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
                ...
                if (Key != NULL && Value != NULL) {
                    if (!flag)
                        (void)ini_write(INI_LINETERM,
                                        &wfp); /* force a new line behind the
            ...
            /* Copy the line from source to dest, but not if this is the section
             * that we are looking for and this section must be removed
             */
            sp = skipleading(LocalBuffer);
            ep = _tcsrchr(sp, ']');
    ...
    cache_flush(LocalBuffer, &cachelen, &rfp, &wfp, &mark);
    /* when deleting a section, the section head that was just found has not
     * been copied to the output file, but because this line was not
     * "accumulated" in the cache, the position in the input file was reset to
     * the point just before the section; this must now be skipped (again)
     */
    if (Key == NULL) {
        (void)ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp);
        (void)ini_tell(&rfp, &mark);
    ...
    for (;;) {
        if (!ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp)) {
            ...
            if (Key != NULL && Value != NULL) {
                if (!flag)
                    (void)ini_write(INI_LINETERM,
                                    &wfp); /* force a new line behind the last
        ...
        if (ep == NULL)
            ep = _tcschr(sp, ':');
        match =
            (ep != NULL && len > 0 && (int)(skiptrailing(ep, sp) - sp) == len &&
             _tcsnicmp(sp, Key, len) == 0);
        if ((Key != NULL && match) || *sp == '[')
            break; /* found the key, or found a new section */
        /* copy other keys in the section */
        if (Key == NULL) {
            (void)ini_tell(&rfp, &mark); /* we are deleting the entire section,
                                            so update the read position */
        } else {
            if (!cache_accum(LocalBuffer, &cachelen, INI_BUFFERSIZE)) {
                cache_flush(LocalBuffer, &cachelen, &rfp, &wfp, &mark);
                (void)ini_read(LocalBuffer, INI_BUFFERSIZE, &rfp);
    ...
    /* the key was found, or we just dropped on the next section (meaning that
     * it wasn't found); in both cases we need to write the key, but in the
     * latter case, we also need to write the line starting the new section
