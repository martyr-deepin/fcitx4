...
static void FcitxDBusMenuEvent(FcitxNotificationItem* uosnotificationitem, DBusMessage* message);
static DBusMessage* FcitxDBusMenuGetGroupProperties(FcitxNotificationItem* uosnotificationitem, DBusMessage* message);
static DBusMessage* FcitxDBusMenuGetProperty(FcitxNotificationItem* uosnotificationitem, DBusMessage* message);
static DBusMessage* FcitxDBusMenuAboutToShow(FcitxNotificationItem* uosnotificationitem, DBusMessage* message);
static DBusMessage* FcitxDBusMenuGetLayout(FcitxNotificationItem* uosnotificationitem, DBusMessage* message);
static void FcitxDBusMenuFillLayooutItemWrap(FcitxNotificationItem* uosnotificationitem, int32_t id, int depth, FcitxStringHashSet* properties, DBusMessageIter* iter);
static void FcitxDBusMenuFillLayooutItem(FcitxNotificationItem* uosnotificationitem, int32_t id, int depth, FcitxStringHashSet* properties, DBusMessageIter* iter);
...
boolean FcitxDBusMenuCreate(FcitxNotificationItem* uosnotificationitem)
{
    DBusObjectPathVTable fcitxIPCVTable = {NULL, &FcitxDBusMenuEventHandler, NULL, NULL, NULL, NULL };
    if (dbus_connection_register_object_path(uosnotificationitem->conn, "/MenuBar", &fcitxIPCVTable, uosnotificationitem)) {
        return true;
    }
    return false;
}
...
DBusHandlerResult FcitxDBusMenuEventHandler(DBusConnection* connection, DBusMessage* message, void* user_data)
{
    ...
    if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE, "Introspect")) {
        ...
        FcitxDBusMenuEvent(uosnotificationitem, message);
        reply = dbus_message_new_method_return(message);
    } else if (dbus_message_is_method_call(message, DBUS_MENU_IFACE, "GetProperty")) {
        reply = FcitxDBusMenuGetProperty(uosnotificationitem, message);
    } else if (dbus_message_is_method_call(message, DBUS_MENU_IFACE, "GetLayout")) {
        reply = FcitxDBusMenuGetLayout(uosnotificationitem, message);
    } else if (dbus_message_is_method_call(message, DBUS_MENU_IFACE, "GetGroupProperties")) {
        reply = FcitxDBusMenuGetGroupProperties(uosnotificationitem, message);
    } else if (dbus_message_is_method_call(message, DBUS_MENU_IFACE, "AboutToShow")) {
        reply = FcitxDBusMenuAboutToShow(uosnotificationitem, message);
    } else if (dbus_message_is_method_call(message, DBUS_INTERFACE_PROPERTIES, "Get")) {
        reply = FcitxDBusPropertyGet(uosnotificationitem, dbusMenuPropertyTable, message);
    } else if (dbus_message_is_method_call(message, DBUS_INTERFACE_PROPERTIES, "Set")) {
        reply = FcitxDBusPropertySet(uosnotificationitem, dbusMenuPropertyTable, message);
    } else if (dbus_message_is_method_call(message, DBUS_INTERFACE_PROPERTIES, "GetAll")) {
        reply = FcitxDBusPropertyGetAll(uosnotificationitem, dbusMenuPropertyTable, message);
    }
...
void FcitxDBusMenuDoEvent(void* arg)
{
    ...
    FcitxInstance* instance = uosnotificationitem->owner;

    int32_t id = uosnotificationitem->pendingActionId;
    uosnotificationitem->pendingActionId = -1;
...
void FcitxDBusMenuEvent(FcitxNotificationItem* uosnotificationitem, DBusMessage* message)
{
    /* signature isvu */
    ...
    do {
        ...
        if (!FcitxInstanceCheckTimeoutByFunc(uosnotificationitem->owner, FcitxDBusMenuDoEvent)) {
            uosnotificationitem->pendingActionId = id;
            FcitxInstanceAddTimeout(uosnotificationitem->owner, 50, FcitxDBusMenuDoEvent, uosnotificationitem);
        }
    } while(0);
}
...
void FcitxDBusMenuFillProperty(FcitxNotificationItem* uosnotificationitem, int32_t id, FcitxStringHashSet* properties, DBusMessageIter* iter)
{
    FcitxInstance* instance = uosnotificationitem->owner;
    /* append a{sv} */
    DBusMessageIter sub;
...
void FcitxDBusMenuFillLayooutItemWrap(FcitxNotificationItem* uosnotificationitem, int32_t id, int depth, FcitxStringHashSet* properties, DBusMessageIter* iter)
{
    DBusMessageIter variant;
    dbus_message_iter_open_container(iter, DBUS_TYPE_VARIANT, "(ia{sv}av)", &variant);
    FcitxDBusMenuFillLayooutItem(uosnotificationitem, id, depth, properties, &variant);
    dbus_message_iter_close_container(iter, &variant);
}

void FcitxDBusMenuFillLayooutItem(FcitxNotificationItem* uosnotificationitem, int32_t id, int depth, FcitxStringHashSet* properties, DBusMessageIter* iter)
{
    FcitxInstance* instance = uosnotificationitem->owner;
    ...
    FcitxDBusMenuFillProperty(uosnotificationitem, id, properties, &sub);
    ...
    if (depth != 0) {
        uosnotificationitem->ids = MenuIdSetAdd(uosnotificationitem->ids, id);
        int32_t menu = ACTION_MENU(id);
        int32_t index = ACTION_INDEX(id);
        ...
        if (menu == 0) {
            if (index == 0) {
                FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(0,1), depth - 1, properties, &array);
                FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(0,2), depth - 1, properties, &array);
                boolean flag = false;
                ...
                for (i = 0, status = (FcitxUIStatus*) utarray_front(uistats);
                    ...
                    flag = true;
                    FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, STATUS_ID(0,i), depth - 1, properties, &array);
                }
                ...
                for (i = 0, compstatus = (FcitxUIComplexStatus*) utarray_front(uicompstats);
                    ...
                    flag = true;
                    FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, STATUS_ID(1,i), depth - 1, properties, &array);
                }

                if (flag)
                    FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(0,8), depth - 1, properties, &array);
                if (utarray_len(uimenus) > 0) {
                    ...
                    for (menupp = (FcitxUIMenu **) utarray_front(uimenus);
                        ...
                        do {
                            ...
                            FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(i,0), depth - 1, properties, &array);
                        } while(0);
                        i ++;
                    }
                    FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(0,3), depth - 1, properties, &array);
                }
                FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(0,5), depth - 1, properties, &array);
                FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(0,6), depth - 1, properties, &array);
                FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(0,7), depth - 1, properties, &array);
            }
        } else {
            if (index == 0) {
                FcitxUIMenu** menupp = (FcitxUIMenu**) utarray_eltptr(uimenus, menu - 1), *menup;
                if (menupp) {
                    ...
                    for (i = 0; i < len; i++) {
                        FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem, ACTION_ID(menu,i + 1), depth - 1, properties, &array);
                    }
                }
            }
        }
...
DBusMessage* FcitxDBusMenuGetLayout(FcitxNotificationItem* uosnotificationitem, DBusMessage* message)
{
    /* signature iias */
    DBusMessageIter args;
    dbus_message_iter_init(message, &args);
    ...
    do {
        ...
        dbus_message_iter_append_basic(&iter, DBUS_TYPE_UINT32, &uosnotificationitem->revision);
        FcitxDBusMenuFillLayooutItem(uosnotificationitem, id, recursionDepth, properties, &iter);
...
DBusMessage* FcitxDBusMenuGetProperty(FcitxNotificationItem* uosnotificationitem, DBusMessage* message)
{
...
DBusMessage* FcitxDBusMenuGetGroupProperties(FcitxNotificationItem* uosnotificationitem, DBusMessage* message)
{
    /* signature aias */
    DBusMessageIter args;
    dbus_message_iter_init(message, &args);
    ...
    do {
        ...
        for (; i < utarray_len(&ids); i ++) {
            ...
            FcitxDBusMenuFillProperty(uosnotificationitem, id, properties, &ssub);
...
DBusMessage* FcitxDBusMenuAboutToShow(FcitxNotificationItem* uosnotificationitem, DBusMessage* message)
{
    DBusMessage* reply = NULL;
    /* signature i out b */
    int32_t id;
    ...
    if (dbus_message_get_args(message, &err, DBUS_TYPE_INT32, &id, DBUS_TYPE_INVALID)) {
        ...
        if (id == 0) {
            ...
            needUpdate = !MenuIdSetContains(uosnotificationitem->ids, id);
        }
