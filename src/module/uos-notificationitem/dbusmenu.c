...
static void FcitxDBusMenuEvent(FcitxNotificationItem *uosnotificationitem,
                               DBusMessage *message);
static DBusMessage *
FcitxDBusMenuGetGroupProperties(FcitxNotificationItem *uosnotificationitem,
                                DBusMessage *message);
static DBusMessage *
FcitxDBusMenuGetProperty(FcitxNotificationItem *uosnotificationitem,
                         DBusMessage *message);
static DBusMessage *
FcitxDBusMenuAboutToShow(FcitxNotificationItem *uosnotificationitem,
                         DBusMessage *message);
static DBusMessage *
FcitxDBusMenuGetLayout(FcitxNotificationItem *uosnotificationitem,
                       DBusMessage *message);
static void FcitxDBusMenuFillLayooutItemWrap(
    FcitxNotificationItem *uosnotificationitem, int32_t id, int depth,
    FcitxStringHashSet *properties, DBusMessageIter *iter);
static void FcitxDBusMenuFillLayooutItem(
    FcitxNotificationItem *uosnotificationitem, int32_t id, int depth,
    FcitxStringHashSet *properties, DBusMessageIter *iter);
...
boolean FcitxDBusMenuCreate(FcitxNotificationItem *uosnotificationitem) {
    ...
    if (dbus_connection_register_object_path(uosnotificationitem->conn,
                                             "/MenuBar", &fcitxIPCVTable,
                                             uosnotificationitem)) {
        return true;
    }
    return false;
...
DBusHandlerResult FcitxDBusMenuEventHandler(DBusConnection *connection,
    ...
    if (dbus_message_is_method_call(message, DBUS_INTERFACE_INTROSPECTABLE,
        ...
        reply = FcitxDBusMenuAboutToShow(uosnotificationitem, message);
                                           ...
                                           "Get")) {
        reply = FcitxDBusPropertyGet(uosnotificationitem, dbusMenuPropertyTable,
                                     message);
    } else if (dbus_message_is_method_call(message, DBUS_INTERFACE_PROPERTIES,
                                           "Set")) {
        reply = FcitxDBusPropertySet(uosnotificationitem, dbusMenuPropertyTable,
                                     message);
    } else if (dbus_message_is_method_call(message, DBUS_INTERFACE_PROPERTIES,
                                           "GetAll")) {
        reply = FcitxDBusPropertyGetAll(uosnotificationitem,
                                        dbusMenuPropertyTable, message);
    }
...
void FcitxDBusMenuEvent(FcitxNotificationItem *uosnotificationitem,
                        DBusMessage *message) {
    /* signature isvu */
    ...
    do {
        ...
        if (!FcitxInstanceCheckTimeoutByFunc(uosnotificationitem->owner,
                                             FcitxDBusMenuDoEvent)) {
            uosnotificationitem->pendingActionId = id;
            FcitxInstanceAddTimeout(uosnotificationitem->owner, 50,
                                    FcitxDBusMenuDoEvent, uosnotificationitem);
        }
    } while (0);
}
...
void FcitxDBusMenuFillProperty(FcitxNotificationItem *uosnotificationitem,
...
void FcitxDBusMenuFillLayooutItemWrap(
    FcitxNotificationItem *uosnotificationitem, int32_t id, int depth,
    FcitxStringHashSet *properties, DBusMessageIter *iter) {
    DBusMessageIter variant;
    dbus_message_iter_open_container(iter, DBUS_TYPE_VARIANT, "(ia{sv}av)",
                                     &variant);
    FcitxDBusMenuFillLayooutItem(uosnotificationitem, id, depth, properties,
                                 &variant);
    dbus_message_iter_close_container(iter, &variant);
}

void FcitxDBusMenuFillLayooutItem(FcitxNotificationItem *uosnotificationitem,
    ...
    if (depth != 0) {
        ...
        if (menu == 0) {
            if (index == 0) {
                FcitxDBusMenuFillLayooutItemWrap(uosnotificationitem,
                ...
                if (utarray_len(uimenus) > 0) {
                    ...
                    for (menupp = (FcitxUIMenu **)utarray_front(uimenus);
                        ...
                        do {
                            ...
                            FcitxDBusMenuFillLayooutItemWrap(
                                uosnotificationitem, ACTION_ID(i, 0), depth - 1,
                                properties, &array);
                        } while (0);
                        i++;
                    }
            ...
            if (index == 0) {
                ...
                if (menupp) {
                    ...
                    for (i = 0; i < len; i++) {
                        FcitxDBusMenuFillLayooutItemWrap(
                            uosnotificationitem, ACTION_ID(menu, i + 1),
...
DBusMessage *FcitxDBusMenuGetLayout(FcitxNotificationItem *uosnotificationitem,
                                    DBusMessage *message) {
    /* signature iias */
    DBusMessageIter args;
    dbus_message_iter_init(message, &args);
    ...
    do {
        ...
        dbus_message_iter_append_basic(&iter, DBUS_TYPE_UINT32,
                                       &uosnotificationitem->revision);
        FcitxDBusMenuFillLayooutItem(uosnotificationitem, id, recursionDepth,
                                     properties, &iter);
...
FcitxDBusMenuGetGroupProperties(FcitxNotificationItem *uosnotificationitem,
    ...
    do {
        ...
        for (; i < utarray_len(&ids); i++) {
            ...
            FcitxDBusMenuFillProperty(uosnotificationitem, id, properties,
                                      &ssub);
