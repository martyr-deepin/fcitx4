...
typedef enum _FillRule { F_COPY = 0, F_RESIZE = 1 } FillRule;
...
void DrawInputBar(struct _InputWindow *inputWindow, boolean vertical,
                  int iCursorPos);
SkinImage *LoadImage(FcitxSkin *sc, const char *name, int fallback);
SkinImage *LoadImageWithText(struct _FcitxClassicUI *classicui, FcitxSkin *sc,
...
SkinImage *GetIMIcon(struct _FcitxClassicUI *classicui, FcitxSkin *sc,
...
void DrawResizableBackground(cairo_t *c, cairo_surface_t *background, int width,
                             ...
                             FillRule fillH);
#define fcitx_cairo_set_color(c, color)                                        \
