...
static int UntrapErrors(void) {
    XSetErrorHandler(hOldErrorHandler);
    return iTrappedErrorCode;
}

int TrayInitAtom(TrayWindow *tray) {
    char *atom_names[] = {NULL, "MANAGER", "_NET_SYSTEM_TRAY_OPCODE",
...
void TraySendOpcode(TrayWindow *tray, long message, long data1, long data2,
    ...
    XSync(dpy, False);
    if (UntrapErrors()) {
        FcitxLog(WARNING, _("X error %i on opcode send"), iTrappedErrorCode);
    }
}
...
XVisualInfo *TrayGetVisual(TrayWindow *tray) {
    ...
    if (tray->dockWindow != None) {
        ...
        int result = XGetWindowProperty(
            dpy, tray->dockWindow, tray->atoms[ATOM_VISUAL], 0, 1, False,
            XA_VISUALID, &actual_type, &actual_format, &nitems,
            &bytes_remaining, &data);
        VisualID vid = 0;
        if (result == Success && data && actual_type == XA_VISUALID &&
            actual_format == 32 && nitems == 1 && bytes_remaining == 0)
            vid = *(VisualID *)data;
        if (data)
            XFree(data);
        if (vid == 0)
