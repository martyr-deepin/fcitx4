...
static void XlibMenuCalculateContentSize(FcitxXlibWindow *window,
                                         unsigned int *width,
                                         unsigned int *height);
...
void XlibMenuInit(XlibMenu *menu) {
    ...
    FcitxXlibWindowInit(
        &menu->parent, MENU_WINDOW_WIDTH, MENU_WINDOW_HEIGHT, 0, 0,
        "Fcitx Menu Window", FCITX_WINDOW_MENU,
        ...
        XlibMenuMoveWindow, XlibMenuCalculateContentSize, XlibMenuPaint);

    int dpi = sc->skinFont.respectDPI ? classicui->dpi : 0;

    FcitxCairoTextContext *ctc = FcitxCairoTextContextCreate(NULL);
    FcitxCairoTextContextSet(ctc, classicui->menuFont,
                             sc->skinFont.menuFontSize, dpi);
    ...
    if (event->xany.window == window->wId) {
        ...
        case LeaveNotify: {
            ...
            if (!IsMouseInOtherMenu(menu, x, y) &&
                window->wId != window->owner->mainMenuWindow->parent.wId) {
        ...
        case MotionNotify: {
            int offseth = 0;
            int i = SelectShellIndex(menu, event->xmotion.x, event->xmotion.y,
                                     &offseth);
            ...
            if (!flag) {
                ...
                if (item && item->type == MENUTYPE_SUBMENU && item->subMenu) {
                    XlibMenu *subxlibmenu =
                        (XlibMenu *)
                            item->subMenu->uipriv[classicui->isfallback];
        ...
        case ButtonPress: {
            ...
            case Button1: {
                int offseth;
                int i = SelectShellIndex(menu, event->xmotion.x,
                                         event->xmotion.y, &offseth);
...
void CloseAllMenuWindow(FcitxClassicUI *classicui) {
    ...
    for (menupp = (FcitxUIMenu **)utarray_front(uimenus); menupp != NULL;
        ...
        XlibMenu *xlibMenu =
            (XlibMenu *)(*menupp)->uipriv[classicui->isfallback];
...
void CloseOtherSubMenuWindow(XlibMenu *xlibMenu, XlibMenu *subMenu) {
    ...
    for (menu = (FcitxMenuItem *)utarray_front(&xlibMenu->menushell->shell);
         menu != NULL; menu = (FcitxMenuItem *)utarray_next(
                           &xlibMenu->menushell->shell, menu)) {
        if (menu->type == MENUTYPE_SUBMENU && menu->subMenu &&
            menu->subMenu->uipriv[classicui->isfallback] != subMenu) {
            CloseAllSubMenuWindow(
                (XlibMenu *)menu->subMenu->uipriv[classicui->isfallback]);
        }
    }
}
...
void XlibMenuCalculateContentSize(FcitxXlibWindow *window, unsigned int *width,
                                  unsigned int *height) {
    ...
    for (i = 0; i < utarray_len(&menu->menushell->shell); i++) {
        if (GetMenuItem(menu->menushell, i)->type == MENUTYPE_SIMPLE ||
            GetMenuItem(menu->menushell, i)->type == MENUTYPE_SUBMENU)
            winheight += 6 + menu->fontheight;
        else if (GetMenuItem(menu->menushell, i)->type == MENUTYPE_DIVLINE)
            winheight += 5;

        int width = FcitxCairoTextContextStringWidth(
            ctc, GetMenuItem(menu->menushell, i)->tipstr);
        if (width > menuwidth)
            menuwidth = width;
    }
 ...
 */
void XlibMenuPaintText(XlibMenu *menu, cairo_t *c, FcitxCairoTextContext *ctc,
                       int shellindex, int line_y, int fontHeight) {
    FcitxXlibWindow *window = (FcitxXlibWindow *)menu;
    FcitxClassicUI *classicui = window->owner;
    FcitxSkin *sc = &classicui->skin;
    ...
    if (GetMenuItem(menu->menushell, shellindex)->isselect == 0) {
        FcitxCairoTextContextOutputString(
            ctc, GetMenuItem(menu->menushell, shellindex)->tipstr, 15, line_y,
            &sc->skinFont.menuFontColor[MENU_INACTIVE]);
    } else {
        ...
        FcitxCairoTextContextOutputString(
            ctc, GetMenuItem(menu->menushell, shellindex)->tipstr, 15, line_y,
            &sc->skinFont.menuFontColor[MENU_ACTIVE]);
    }
    cairo_restore(c);
}
...
void XlibMenuPaintArrow(XlibMenu *menu, cairo_t *cr, int line_y, int i) {
    ...
    cairo_line_to(cr, window->contentWidth - 1 - size,
                  line_y + size * 2 + offset);
...
void XlibMenuMoveWindow(FcitxXlibWindow *window) {
    ...
    if (menu->anchor == MA_MainWindow) {
        CalMenuWindowPosition(menu, classicui->iMainWindowOffsetX,
                              classicui->iMainWindowOffsetY,
                              classicui->mainWindow->parent.height);
    } else if (menu->anchor == MA_Menu) {
        XlibMenu *parent = menu->anchorMenu;
        menu->iPosX = parent->iPosX + parent->parent.contentX +
                      parent->parent.contentWidth - 4;
        menu->iPosY = parent->iPosY + menu->offseth - window->contentY;
        FcitxRect rect = GetScreenGeometry(classicui, menu->iPosX, menu->iPosY);

        if (menu->iPosX + window->width > rect.x2) {
            menu->iPosX =
                parent->iPosX - window->width + parent->parent.contentX + 4;
        }
        ...
        CalMenuWindowPosition(menu, menu->trayX, menu->trayY,
                              classicui->trayWindow->size);
    }
