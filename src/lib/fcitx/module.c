    ...
    for (addon = (FcitxAddon *)utarray_front(addons); addon != NULL;
        ...
        if (addon->bEnabled && addon->category == AC_MODULE) {
            ...
            case AT_SHAREDLIBRARY: {
                ...
                if (!handle) {
                    FcitxLog(ERROR, _("Module: open %s fail %s"), modulePath,
                             dlerror());
                    break;
                }
...
FcitxModuleFunction FcitxModuleFindFunction(FcitxAddon *addon, int func_id) {
    ...
    FcitxModuleFunction *func_p =
        fcitx_array_eltptr(&addon->functionList, func_id);
    if (func_p)
        return *func_p;
    return NULL;
}

FCITX_EXPORT_API void *FcitxModuleInvokeOnAddon(FcitxAddon *addon,
...
static void *_FcitxModuleInvokeFunction(FcitxAddon *addon, int functionId,
                                        FcitxModuleFunctionArg args) {
...
FCITX_EXPORT_API void *FcitxModuleInvokeFunction(FcitxAddon *addon,
                                                 int functionId,
                                                 FcitxModuleFunctionArg args) {
...
FCITX_EXPORT_API
void *FcitxModuleInvokeFunctionByName(FcitxInstance *instance, const char *name,
                                      int functionId,
                                      FcitxModuleFunctionArg args) {
...
FCITX_EXPORT_API void FcitxModuleAddFunction(FcitxAddon *addon,
                                             FcitxModuleFunction func) {
    void *temp = (void *)func;
