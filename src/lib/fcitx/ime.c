...
static const FcitxHotkey *switchKey1[] = {
    FCITX_RCTRL,       FCITX_RSHIFT,      FCITX_LSHIFT,
    FCITX_LCTRL,       FCITX_ALT_LSHIFT,  FCITX_ALT_RSHIFT,

    FCITX_RCTRL,       FCITX_RSHIFT,

    FCITX_LALT,        FCITX_RALT,        FCITX_LALT,

    FCITX_LSUPER,      FCITX_RSUPER,      FCITX_LSUPER,

    FCITX_CTRL_LSUPER, FCITX_CTRL_RSUPER, FCITX_SUPER_LCTRL,
    FCITX_SUPER_RCTRL, FCITX_NONE_KEY,
};

static const FcitxHotkey *switchKey2[] = {
    FCITX_NONE_KEY, FCITX_NONE_KEY, FCITX_NONE_KEY,
    FCITX_NONE_KEY, FCITX_NONE_KEY, FCITX_NONE_KEY,

    FCITX_LCTRL,    FCITX_LSHIFT,

    FCITX_NONE_KEY, FCITX_NONE_KEY, FCITX_RALT,

    FCITX_NONE_KEY, FCITX_NONE_KEY, FCITX_RSUPER,

    FCITX_NONE_KEY, FCITX_NONE_KEY, FCITX_NONE_KEY,
    FCITX_NONE_KEY, FCITX_NONE_KEY};
...
static void FcitxInstanceEnableIMInternal(FcitxInstance *instance,
                                          FcitxInputContext *ic,
                                          boolean keepState);
static void FcitxInstanceCloseIMInternal(FcitxInstance *instance,
                                         FcitxInputContext *ic);
static void FcitxInstanceChangeIMStateWithKey(FcitxInstance *instance,
                                              FcitxInputContext *ic,
                                              boolean withSwitchKey);
static void FcitxInstanceChangeIMStateInternal(FcitxInstance *instance,
                                               FcitxInputContext *ic,
...
FCITX_GETTER_VALUE(FcitxInputState, LastCommitString, strLastCommit,
                   const char *)
...
FCITX_GETTER_VALUE(FcitxInputState, CandidateList, candList,
...
FCITX_GETTER_VALUE(FcitxInputState, ClientPreedit, msgClientPreedit,
                   FcitxMessages *)
...
boolean FcitxInstanceCheckICFromSameApplication(FcitxInstance *instance,
    ...
    FcitxAddon **pfrontend =
        FcitxInstanceGetPFrontend(instance, ic->frontendid);
    if (pfrontend) {
...
void FcitxInstanceLoadIM(FcitxInstance *instance, FcitxAddon *addon) {
    ...
    if (addon->type == AT_SHAREDLIBRARY) {
        ...
        fclose(fp);
        handle = dlopen(modulePath,
                        RTLD_NOW | RTLD_NODELETE |
                            (addon->loadLocal ? RTLD_LOCAL : RTLD_GLOBAL));
        if (!handle) {
...
void FcitxInstanceRegisterEmptyIMEntry(FcitxInstance *instance,
                                       const char *name, const char *uniqueName,
                                       const char *iconName, int priority,
                                       const char *langCode,
                                       FcitxAddon *addon) {
    UT_array *imes = &instance->availimes;
    FcitxIM *entry =
        FcitxInstanceGetIMFromIMList(instance, IMAS_Disable, uniqueName);
    if (entry) {
        return;
    } else {
...
void FcitxInstanceRegisterIM(FcitxInstance *instance, void *imclass,
                             ...
                             const char *iconName, FcitxIMInit Init,
                             FcitxIMResetIM ResetIM, FcitxIMDoInput DoInput,
                             FcitxIMGetCandWords GetCandWords,
                             FcitxIMPhraseTips PhraseTips, FcitxIMSave Save,
                             FcitxIMReloadConfig ReloadConfig,
                             FcitxIMKeyBlocker KeyBlocker, int priority,
                             const char *langCode) {
    FcitxIMIFace iface;
    ...
    FcitxInstanceRegisterIMv2(instance, imclass, uniqueName, name, iconName,
                              iface, priority, langCode);
}

FCITX_EXPORT_API
void FcitxInstanceRegisterIMv2(FcitxInstance *instance, void *imclass,
                               const char *uniqueName, const char *name,
...
boolean FcitxInstanceLoadAllIM(FcitxInstance *instance) {
    ...
    while (curs) {
        FILE *fp =
            FcitxXDGGetFileWithPrefix("inputmethod", curs->name, "r", NULL);
        ...
        if (cfile) {
            ...
            FcitxAddon *addon =
                FcitxAddonsGetAddonByName(&instance->addons, entry->parent);

            if (addon && addon->bEnabled && addon->category == AC_INPUTMETHOD &&
                addon->registerMethod == IMRM_CONFIGFILE) {
                FcitxInstanceRegisterEmptyIMEntry(
                    instance, entry->name, entry->uniqueName, entry->iconName,
                    entry->priority, entry->langCode, addon);
            }
            FreeIMEntry(entry);
        }
        curs = curs->hh.next;
    ...
    FcitxAddon *addon;
    for (addon = (FcitxAddon *)utarray_front(addons); addon != NULL;
...
static void _NormalizeHotkeyForModifier(const FcitxHotkey *origin,
                                        FcitxHotkey *group1,
                                        FcitxHotkey *group2) {
    int i;
    memcpy(group1, origin, sizeof(FcitxHotkey[2]));
    for (i = 0; i < 2; i++) {
        if (FcitxHotkeyIsHotKeyModifierCombine(group1[i].sym,
                                               group1[i].state)) {
    ...
    for (i = 0; i < 2; i++) {
        if (FcitxHotkeyIsHotKeyModifierCombine(group2[i].sym,
                                               group2[i].state)) {
 ...
 * ctrl -> on release
 *
 * once here's a wrong implementation that using all key without unicode as
 * trigger on release which passes press to im. (to mozc)
 */
...
INPUT_RETURN_VALUE _DoSwitchIM(FcitxInstance *instance) {
    if (instance->config->bIMSwitchKey &&
        (instance->config->bIMSwitchIncludeInactive ||
         FcitxInstanceGetCurrentState(instance) == IS_ACTIVE)) {
        FcitxInstanceSwitchIMByIndex(
            instance, instance->config->bIMSwitchIncludeInactive ? -1 : -3);
        return IRV_DO_NOTHING;
    }
    return IRV_TO_PROCESS;
}
...
INPUT_RETURN_VALUE _DoSwitchIMReverse(FcitxInstance *instance) {
    if (instance->config->bIMSwitchKey &&
        (instance->config->bIMSwitchIncludeInactive ||
         FcitxInstanceGetCurrentState(instance) == IS_ACTIVE)) {
        FcitxInstanceSwitchIMByIndex(
            instance, instance->config->bIMSwitchIncludeInactive ? -2 : -4);
        return IRV_DO_NOTHING;
    }
    return IRV_TO_PROCESS;
}
    ...
    if (currentIM && currentIM->OnClose) {
        ...
        if (instance->config->bSendTextWhenSwitchEng) {
            if (instance->input->iCodeInputCount != 0) {
                strcpy(FcitxInputStateGetOutputString(instance->input),
                       FcitxInputStateGetRawInputBuffer(instance->input));
                retVal = IRV_ENG;
            }
        }
    }
...
INPUT_RETURN_VALUE _Do2ndSelect(FcitxInstance *instance) {
    ...
    if (input->bIsInRemind ||
        FcitxCandidateWordGetPageSize(input->candList) != 0) {
        if (!input->bIsInRemind) {
...
FCITX_EXPORT_API
INPUT_RETURN_VALUE FcitxInstanceProcessKey(FcitxInstance *instance,
...
#define CUSTOM_SWITCH_KEY 19
    if ((int)fc->iSwitchKey < CUSTOM_SWITCH_KEY) {
        ...
        _NormalizeHotkeyForModifier(fc->hkCustomSwitchKey, hkCustomSwitchKey1,
                                    hkCustomSwitchKey2);
    ...
    struct {
        ...
        {KR_2ND_SELECTKEY, fc->i2ndSelectKey, NULL, _Do2ndSelect,
         _Check2ndSelect}, // KR_2ND_SELECTKEY,
        {KR_3RD_SELECTKEY, fc->i3rdSelectKey, NULL, _Do3ndSelect,
         _Check3ndSelect}, // KR_3RD_SELECTKEY,
        {KR_SWITCH, hkSwitchKey1, hkSwitchKey2, _DoSwitch,
         _CheckSwitch}, // KR_SWITCH
        {KR_SWITCH_IM, hkSwitchNext1, hkSwitchNext2, _DoSwitchIM,
         _CheckSwitchIM}, //  KR_SWITCH_IM,
        {KR_SWITCH_IM_REVERSE, hkSwitchPrev1, hkSwitchPrev2, _DoSwitchIMReverse,
         _CheckSwitchIM}, // KR_SWITCH_IM_REVERSE,
        {KR_TRIGGER, hkTrigger1, hkTrigger2, _DoTrigger, NULL},
        {KR_ACTIVATE, hkActivate1, hkActivate2, _DoActivate, _CheckActivate},
        {KR_DEACTIVATE, hkInactivate1, hkInactivate2, _DoDeactivate,
         _CheckDeactivate}, //  KR_DEACTIVATE
    };
...
#define CHECK_HOTKEY(name)                                                     \
    ((name##1 ? FcitxHotkeyIsHotKey(sym, state, name##1) : false) ||           \
     (name##2 ? FcitxHotkeyIsHotKey(sym, state, name##2) : false))

    /*
     * for following reason, we cannot just process switch key, 2nd, 3rd key as
     * other simple hotkey because ctrl, shift, alt are compose key, so hotkey
     * like ctrl+a will also produce a key release event for ctrl key, so we
     * must make sure the key release right now is the key just pressed.
     */
    ...
    if (retVal == IRV_TO_PROCESS) {
        ...
        if (event == FCITX_PRESS_KEY) {
            ...
            do {
                ...
                int i;
                for (i = 0; i < FCITX_ARRAY_SIZE(keyHandle); i++) {
                    if (CHECK_HOTKEY(keyHandle[i].hk) &&
                        (!keyHandle[i].check || keyHandle[i].check(instance))) {
                        if (!triggerOnRelease) {
                            retVal = keyHandle[i].callback(instance);
                        }
                        input->keyReleased = keyHandle[i].kr;
    ...
    if (retVal == IRV_TO_PROCESS && event == FCITX_RELEASE_KEY) {
        FcitxInstanceProcessPreReleaseInputFilter(instance, sym, state,
                                                  &retVal);

        if (retVal == IRV_TO_PROCESS && currentIM &&
            currentIM->DoReleaseInput) {
            retVal = currentIM->DoReleaseInput(currentIM->klass, sym, state);
        }

        if (retVal == IRV_TO_PROCESS) {
            FcitxInstanceProcessPostReleaseInputFilter(instance, sym, state,
                                                       &retVal);
        }
    ...
    if (FcitxInstanceGetCurrentStatev2(instance) == IS_ACTIVE &&
        ...
        if (!input->bIsDoInputOnly && retVal == IRV_TO_PROCESS) {
            int index =
                FcitxCandidateWordCheckChooseKey(input->candList, sym, state);
            if (index >= 0)
                retVal =
                    FcitxCandidateWordChooseByIndex(input->candList, index);
        }
    }

    return FcitxInstanceDoInputCallback(instance, retVal, event, timestamp, sym,
                                        state);
}

FCITX_EXPORT_API
void FcitxInstanceChooseCandidateByIndex(FcitxInstance *instance, int index) {
    if (!(FcitxInstanceGetCurrentStatev2(instance) == IS_ACTIVE &&
          index < 10)) {
        return;
    }
...
FCITX_EXPORT_API
void FcitxInstanceChooseCandidateByTotalIndex(FcitxInstance *instance,
                                              int index) {
    if (!(FcitxInstanceGetCurrentStatev2(instance) == IS_ACTIVE)) {
        return;
    }
    FcitxInputState *input = instance->input;
    INPUT_RETURN_VALUE retVal =
        FcitxCandidateWordChooseByTotalIndex(input->candList, index);
    FcitxIM *currentIM = FcitxInstanceGetCurrentIM(instance);
    if (FcitxInstanceGetCurrentStatev2(instance) == IS_ACTIVE && currentIM &&
        (retVal & IRV_FLAG_UPDATE_CANDIDATE_WORDS)) {
        if (currentIM->GetCandWords) {
...
INPUT_RETURN_VALUE FcitxInstanceDoInputCallback(
    FcitxInstance *instance, INPUT_RETURN_VALUE retVal, FcitxKeyEventType event,
    long unsigned int timestamp, FcitxKeySym sym, unsigned int state) {
    FCITX_UNUSED(event);
    ...
    /*
     * since all candidate word are cached in candList, so we don't need to
     * trigger GetCandWords after go for another page, simply update input
     * window is ok.
     */
    if (FcitxInstanceGetCurrentStatev2(instance) == IS_ACTIVE &&
...
FCITX_EXPORT_API
void FcitxInstanceProcessInputReturnValue(FcitxInstance *instance,
                                          INPUT_RETURN_VALUE retVal) {
    FcitxIM *currentIM = FcitxInstanceGetCurrentIM(instance);
    FcitxInputState *input = instance->input;
    FcitxGlobalConfig *fc = instance->config;
    ...
    if (retVal & IRV_FLAG_PENDING_COMMIT_STRING) {
        FcitxInstanceCommitString(instance, instance->CurrentIC,
...
FCITX_EXPORT_API
void FcitxInstanceForwardKey(FcitxInstance *instance, FcitxInputContext *ic,
                             FcitxKeyEventType event, FcitxKeySym sym,
                             unsigned int state) {
    if (ic == NULL)
        return;
    FcitxAddon **pfrontend =
...
void FcitxInstanceSwitchIMByName(FcitxInstance *instance, const char *name) {
    ...
    if (im) {
        if (FcitxInstanceGetCurrentIC(instance)) {
            ...
            // Another possibility is to add it to
            // FcitxInstanceSwitchIMInternal, but that part of code is too hack
            // and some initialization may relies on it. This function is only
            // used in UI and DBus remote control, Thus it is safe to change it
            // here.
            FcitxIM *currentIM = FcitxInstanceGetCurrentIM(instance);
...
void FcitxInstanceSwitchIMByIndex(FcitxInstance *instance, int index) {
    ...
    if (index == 0)
        ...
        if (FcitxInstanceGetCurrentState(instance) != IS_ACTIVE) {
            FcitxInstanceEnableIM(instance, FcitxInstanceGetCurrentIC(instance),
                                  false);
        }
    }
    FcitxInstanceShowInputSpeed(instance, false);
...
void FcitxInstanceSwitchIMInternal(FcitxInstance *instance, int index,
    ...
    if (newIM && !newIM->initialized) {
        ...
        FcitxIM *im =
            FcitxInstanceGetIMFromIMList(instance, IMAS_Disable, name);
        if (!im->initialized) {
            im->initialized = true;
            unsigned int index = utarray_eltidx(&instance->availimes, im);
            utarray_erase(&instance->availimes, index, 1);
    ...
    if (newIM && updateGlobal) {
        if (fcitx_utils_strcmp0(instance->globalIMName, newIM->uniqueName) !=
            0) {
...
FCITX_EXPORT_API
void FcitxInstanceSendCloseEvent(struct _FcitxInstance *instance,
                                 FcitxIMCloseEventType closeEvent) {
    FcitxIM *currentIM = FcitxInstanceGetCurrentIM(instance);
    if (currentIM && currentIM->OnClose) {
...
FCITX_EXPORT_API
void FcitxInstanceReloadAddonConfig(FcitxInstance *instance,
    ...
    if (strcmp(addonname, "global") == 0) {
        ...
        do {
            ...
            FcitxAddon *addon =
                FcitxAddonsGetAddonByName(&instance->addons, addonname);
            ...
            case AC_FRONTEND:
                ...
                break;
            case AC_INPUTMETHOD:
                /* imclass and imclass2 are in same union, only check one of
                 * them */
                if (addon->imclass) {
                    for (im = (FcitxIM *)utarray_front(&instance->availimes);
                         im != NULL; im = (FcitxIM *)utarray_next(
                                         &instance->availimes, im)) {
...
void FcitxInstanceReloadConfig(FcitxInstance *instance) {
    ...
    for (addon = (FcitxAddon *)utarray_front(addons); addon != NULL;
         addon = (FcitxAddon *)utarray_next(addons, addon)) {
        if (addon->category == AC_MODULE && addon->bEnabled &&
            addon->addonInstance) {
            if (addon->module->ReloadConfig)
                addon->module->ReloadConfig(addon->addonInstance);
        }
    ...
    for (addon = (FcitxAddon *)utarray_front(addons); addon != NULL;
         addon = (FcitxAddon *)utarray_next(addons, addon)) {
        if (addon->category == AC_FRONTEND && addon->bEnabled &&
            addon->addonInstance) {
            if (addon->frontend->ReloadConfig)
                addon->frontend->ReloadConfig(addon->addonInstance);
        }
    ...
    for (addon = (FcitxAddon *)utarray_front(addons); addon != NULL;
         addon = (FcitxAddon *)utarray_next(addons, addon)) {
        if (addon->category == AC_INPUTMETHOD && addon->bEnabled &&
            addon->addonInstance && addon->isIMClass2) {
...
static inline void FcitxInstanceSetICStatus(FcitxInstance *instance,
                                            FcitxInputContext *ic,
...
FCITX_EXPORT_API
void FcitxInstanceSetLocalIMName(FcitxInstance *instance, FcitxInputContext *ic,
                                 const char *imname) {
    FcitxInputContext2 *ic2 = (FcitxInputContext2 *)ic;
    if (ic2->imname) {
        free(ic2->imname);
...
/* the "force" to this function is used when the list changed and index is not
 * valid */
boolean FcitxInstanceUpdateCurrentIM(FcitxInstance *instance, boolean force,
    ...
    FcitxInputContext2 *ic2 = (FcitxInputContext2 *)ic;
    int globalIndex =
        FcitxInstanceGetIMIndexByName(instance, instance->globalIMName);
    ...
    if (ic2 && ic2->imname) {
        FcitxIM *im =
            FcitxInstanceGetIMFromIMList(instance, IMAS_Enable, ic2->imname);
        if (!im) {
            free(ic2->imname);
            ic2->imname = NULL;
        }
    ...
    if (ic && ic->state != IS_ACTIVE) {
        ...
        if (ic2 && ic2->imname)
            targetIMIndex =
                FcitxInstanceGetIMIndexByName(instance, ic2->imname);
        else
    ...
    if (forceSwtich || targetIMIndex != instance->iIMIndex) {
        FcitxInstanceSwitchIMInternal(instance, targetIMIndex, skipZero,
                                      updateGlobal, userSwitchIM);
        return true;
    } else
        return false;
}
...
FCITX_EXPORT_API
void FcitxInstanceEnableIM(FcitxInstance *instance, FcitxInputContext *ic,
                           boolean keepState) {
    if (ic == NULL)
        return;
    ...
    case ShareState_PerProgram: {
        ...
        while (rec != NULL) {
            ...
            if (instance->config->shareState == ShareState_All)
                ...
                flag =
                    FcitxInstanceCheckICFromSameApplication(instance, rec, ic);
            }

            if (flag && (rec == ic || !(rec->contextCaps &
...
void FcitxInstanceEnableIMInternal(FcitxInstance *instance,
                                   FcitxInputContext *ic, boolean keepState) {
    if (ic == NULL)
        return;
    FcitxAddon **pfrontend =
        FcitxInstanceGetPFrontend(instance, ic->frontendid);
...
void FcitxInstanceCloseIMInternal(FcitxInstance *instance,
                                  FcitxInputContext *ic) {
    if (ic == NULL)
        return;
    FcitxAddon **pfrontend =
...
void FcitxInstanceChangeIMStateWithKey(FcitxInstance *instance,
                                       FcitxInputContext *ic,
                                       boolean withSwitchKey) {
    if (ic == NULL)
        return;
 ...
 */
FCITX_EXPORT_API
void FcitxInstanceChangeIMState(FcitxInstance *instance,
                                FcitxInputContext *ic) {
    FcitxInstanceChangeIMStateWithKey(instance, ic, false);
}

void FcitxInstanceChangeIMStateInternal(FcitxInstance *instance,
                                        FcitxInputContext *ic,
                                        FcitxContextState objectState,
                                        boolean withSwitchKey) {
    if (!ic)
...
void UpdateIMMenuItem(FcitxUIMenu *menu) {
    ...
    for (pim = (FcitxIM *)utarray_front(imes); pim != NULL;
         pim = (FcitxIM *)utarray_next(imes, pim))
        FcitxMenuAddMenuItem(&instance->imMenu, pim->strName, MENUTYPE_SIMPLE,
                             NULL);
...
void FcitxInstanceShowInputSpeed(FcitxInstance *instance, boolean force) {
    ...
    if (!force) {
        ...
        if (FcitxInstanceGetCurrentState(instance) != IS_ACTIVE &&
            instance->config->bShowInputWindowOnlyWhenActive)
            return;
    }
    ...
    if (im) {
        ...
        const char *subModeName =
            im->GetSubModeName ? im->GetSubModeName(im->klass) : NULL;
        if (subModeName) {
            FcitxMessagesAddMessageStringsAtLast(input->msgAuxUp, MSG_TIPS,
                                                 " - ", subModeName);
        }
    ...
    if (instance->config->bShowUserSpeed) {
        ...
        FcitxMessagesAddMessageAtLast(
            input->msgAuxDown, MSG_CODE, "%d",
            (int)(instance->iHZInputed * 60 / timePassed));
        FcitxMessagesAddMessageStringsAtLast(input->msgAuxDown, MSG_OTHER,
                                             _("/min  Time Used: "));
        FcitxMessagesAddMessageAtLast(input->msgAuxDown, MSG_CODE, "%d",
                                      (int)timePassed / 60);
        FcitxMessagesAddMessageStringsAtLast(input->msgAuxDown, MSG_OTHER,
                                             _("min Num of Characters: "));
        FcitxMessagesAddMessageAtLast(input->msgAuxDown, MSG_CODE, "%u",
                                      instance->iHZInputed);
    }
...
FCITX_EXPORT_API
int FcitxHotkeyCheckChooseKey(FcitxKeySym sym, unsigned int state,
                              const char *strChoose) {
    return FcitxHotkeyCheckChooseKeyAndModifier(sym, state, strChoose,
                                                FcitxKeyState_None);
}
...
FCITX_EXPORT_API
void FcitxInstanceNotifyUpdateSurroundingText(FcitxInstance *instance,
                                              FcitxInputContext *ic) {
    if (!ic)
        return;
    if (ic != instance->CurrentIC)
...
void FcitxInstanceUpdateIMList(FcitxInstance *instance) {
    ...
    UT_array *imList_bak =
        fcitx_utils_split_string(instance->profile->imList, ',');
    ...
    for (pstr = (char **)utarray_front(imList), index = 0,
        utarray_front(imList_bak);
         pstr != NULL; pstr = (char **)utarray_next(imList, pstr), index++,
        pstr_bak = (char **)utarray_next(imList_bak, pstr_bak)) {
        char *str = *pstr;
        char *pos = strchr(str, ':');
        if (pos) {
            ime = NULL;
            *pos = '\0';
            ...
            if (!ime) {
                ...
                if (!item) {
                    ...
                    item->status = status;
                    HASH_ADD_KEYPTR(hh, instance->unusedItem, item->name,
                                    strlen(item->name), item);
                }
    ...
    for (ime = (FcitxIM *)utarray_front(&instance->availimes); ime != NULL;
        ...
        if (!IMIsInIMNameList(imList, ime)) {
            ...
            if (ime->iPriority == 0)
                ...
                /*
                 * here, we use such logic, if user start fcitx for first time
                 * (or doing reset) then match everything by language, otherwise
                 * user might just install something, then add something else.
                 */
...
FCITX_EXPORT_API
FcitxIM *FcitxInstanceGetIMFromIMList(FcitxInstance *instance,
                                      FcitxIMAvailableStatus status,
                                      const char *name) {
    UT_array *imes;
    ...
    FcitxIM *ime = NULL;
    for (ime = (FcitxIM *)utarray_front(imes); ime != NULL;
...
boolean IMIsInIMNameList(UT_array *imList, FcitxIM *ime) {
    char **pstr;
    for (pstr = (char **)utarray_front(imList); pstr != NULL;
...
FCITX_EXPORT_API
INPUT_RETURN_VALUE FcitxStandardKeyBlocker(FcitxInputState *input,
                                           FcitxKeySym key,
                                           unsigned int state) {
