 ...
 * @file ui.h
 *
 * this file provides the UI component for Fcitx, fcitx doesn't provides any
 * platform specific UI element, all UI are implemented via Fcitx's addon.
 *
 * For input method and module, they can register UI component and get native UI
 * for free from Fcitx.
 *
 * There is two abstract UI element in Fcitx, one is Status, and the other is
 * Menu. Both of them require a unique name in Fcitx. Usually this name is
 * meaningful in order to increase readability.
 *
 * For status, due to backward compatible problem, there are two kinds of
 * status, simple and complex one. Status have three property, short
 * description, long description and icon name. The actual UI depends on
 * specific UI implementation, but usually, status are displayed as a button,
 * with an icon. Icon name can be absolute png file path. All description need
 * to be I18N'ed by addon themselves.
 *
 * For menu, each menu item contains a string, like status, it need to I18N'ed
 * before set. Though there is different type in menu item, DONOT use submenu or
 * divline, they are not supported by specific implementation, and suppose to be
 ...
 *  - Candidate words
 *
 * In spite of candidate words, other four fields are FcitxMessages pointer.
 * FcitxMessages is basically a fixed length string array. Each string inside
 * FcitxMessages can have a type.
 */
...
#define MESSAGE_MAX_LENGTH                                                     \
    (MESSAGE_MAX_CHARNUM * UTF8_MAX_LENGTH) /**< maximum byte per message */
...
typedef enum _FcitxMenuItemType {
    MENUTYPE_SIMPLE,
    MENUTYPE_SUBMENU, /**< unfortunately this is only used internal, don't use
                         it in your code */
    MENUTYPE_DIVLINE
} FcitxMenuItemType;
...
struct _FcitxUIMenu {
    ...
    char *name;
    /**
     * you might want to bind the menu on a status icon, but this is only a
     *hint, depends on the ui implementation
     **/
    char *candStatusBind;
    /**
...
typedef struct _FcitxUI {
    ...
    void (*RegisterComplexStatus)(
        void *, FcitxUIComplexStatus *); /**< register complex status */
    void (*UpdateComplexStatus)(
        void *arg, FcitxUIComplexStatus *); /**< register complext status */
...
void FcitxMessagesAddMessageVStringAtLast(FcitxMessages *message,
                                          ...
                                          const char **strs);
#define FcitxMessagesAddMessageStringsAtLast(message, type, strs...)           \
    do {                                                                       \
        const char *__msg_str_lst[] = {strs};                                  \
        size_t __msg_str_count = sizeof(__msg_str_lst) / sizeof(char *);       \
        FcitxMessagesAddMessageVStringAtLast(message, type, __msg_str_count,   \
                                             __msg_str_lst);                   \
    } while (0)
 ...
 * @since 4.2.7
 **/
void FcitxMessagesSetMessageTextVString(FcitxMessages *message, int position,
                                        size_t n, const char **strs);
#define FcitxMessagesSetMessageTextStrings(message, position, strs...)         \
    do {                                                                       \
        const char *__msg_str_lst[] = {strs};                                  \
        size_t __msg_str_count = sizeof(__msg_str_lst) / sizeof(char *);       \
        FcitxMessagesSetMessageTextVString(message, position, __msg_str_count, \
                                           __msg_str_lst);                     \
    } while (0)
...
#define FcitxMessagesSetMessageStrings(message, position, type, strs...)       \
    do {                                                                       \
        const char *__msg_str_lst[] = {strs};                                  \
        size_t __msg_str_count = sizeof(__msg_str_lst) / sizeof(char *);       \
 ...
 * @return void
 **/
void FcitxUIDisplayMessage(struct _FcitxInstance *instance, char *title,
                           char **msg, int length);
 ...
 **/
FcitxUIStatus *FcitxUIGetStatusByName(struct _FcitxInstance *instance,
                                      const char *name);
 ...
 * @since 4.2.1
 **/
FcitxUIMenu *FcitxUIGetMenuByStatusName(struct _FcitxInstance *instance,
                                        const char *name);
...
FcitxUIGetComplexStatusByName(struct _FcitxInstance *instance,
                              const char *name);
 ...
 * @return void
 **/
void FcitxUISetStatusVisable(struct _FcitxInstance *instance, const char *name,
                             boolean visible);
 ...
 * @since 4.2.1
 **/
void FcitxUISetStatusString(struct _FcitxInstance *instance, const char *name,
 ...
 * @return void
 **/
void FcitxUIGetMainWindowSize(struct _FcitxInstance *instance, int *x, int *y,
                              int *w, int *h);

/**
 * convert new messages to old up and down style messages, return the new cursos
 *pos
 *
 * @param instance fcitx instance
 ...
 * @return int
 **/
int FcitxUINewMessageToOldStyleMessage(struct _FcitxInstance *instance,
                                       FcitxMessages *msgUp,
