 ...
 *
 * The input method is key event centric application. When a key event comes to
 * fcitx, the process handling the keyboard event can be separated into 7
 * phases, PreInput, DoInput, Update Candidates, Prev/Next Page PostInput,
 * Hotkey, and key blocker.
 *
 * The input method engine process key event inside "DoInput".
 *
 * Each phase will change the INPUT_RETURN_VALUE, if INPUT_RETURN_VALUE is
 * non-zero (non IRV_TO_PROCESS), the following phases will not run.
 *
 * If a key event goes through all phase and still the state is IRV_TO_PROCESS,
 * it will be forwarded.
 *
 * When it comes to update candidates, if the flag contains
 * IRV_FLAG_UPDATE_CANDIDATE_WORDS, it will trigger the GetCandWords of input
 * method engine and clean up all strings in the input window, after that, it
 * will trigger update candidates hook.
 *
 * Key blocker is useful if you want to do something in the post input phase,
 * but you don't want forward key if they do nothing. There is an default
 * implemention inside fcitx, it will blocks key when raw input buffer is not
 * empty. Those keys contains direction key(left/right..), key will cause
 * something input (a,b,c...), key will cause cursor move (home/end...).
 *
 * DoInput, Update Candidates, Key Blocker are belongs to input method engine,
 * other can be registered by other addon.
 */
...
/** when input method priority is larger than 100, it will be disabled by
...
typedef enum _INPUT_RETURN_VALUE {
    ...
    IRV_FLAG_BLOCK_FOLLOWING_PROCESS =
        1 << 0, /* nothing to do, actually non-zero is blocking, but you need a
                   flag for do nothing */
    IRV_FLAG_FORWARD_KEY = 1 << 1, /* the key will be forwarded */
    IRV_FLAG_RESET_INPUT = 1 << 2, /* reset input */
    IRV_FLAG_PENDING_COMMIT_STRING =
        1 << 3, /* there is something in input strStringGet buffer, commit it */
    IRV_FLAG_UPDATE_INPUT_WINDOW =
        1 << 4, /* something updated in input window, let the UI update */
    ...
    IRV_COMMIT_STRING =
        IRV_FLAG_PENDING_COMMIT_STRING | IRV_FLAG_DO_PHRASE_TIPS,
    IRV_DO_NOTHING = IRV_FLAG_BLOCK_FOLLOWING_PROCESS,
    IRV_CLEAN = IRV_FLAG_RESET_INPUT,
    IRV_COMMIT_STRING_REMIND =
        IRV_FLAG_PENDING_COMMIT_STRING | IRV_FLAG_UPDATE_INPUT_WINDOW,
    IRV_DISPLAY_CANDWORDS =
        IRV_FLAG_UPDATE_INPUT_WINDOW | IRV_FLAG_UPDATE_CANDIDATE_WORDS,
    IRV_DONOT_PROCESS_CLEAN = IRV_FLAG_FORWARD_KEY | IRV_FLAG_RESET_INPUT,
    IRV_COMMIT_STRING_NEXT =
        IRV_FLAG_PENDING_COMMIT_STRING | IRV_FLAG_UPDATE_INPUT_WINDOW,
    IRV_DISPLAY_MESSAGE = IRV_FLAG_UPDATE_INPUT_WINDOW,
    IRV_ENG =
        IRV_FLAG_PENDING_COMMIT_STRING | IRV_FLAG_ENG | IRV_FLAG_RESET_INPUT,
    IRV_PUNC =
        IRV_FLAG_PENDING_COMMIT_STRING | IRV_FLAG_PUNC | IRV_FLAG_RESET_INPUT,
...
typedef struct _FcitxIMClass {
    void *(*Create)(struct _FcitxInstance
                        *instance); /**< interface for create a input method */
    void (*Destroy)(void *arg);     /**< interface for destroy all input method
...
typedef struct _FcitxIMClass2 {
    ...
    void (*Destroy)(void *arg);      /**< interface for destroy all input method
                                        created by this class */
    void (*ReloadConfig)(void *arg); /**< interface for destroy all input method
                                        created by this class */
...
typedef enum _FcitxIMCloseEventType {
     ...
     *
     * CET_ChangeByUser will not be emitted once CET_ChangeByInactivate is
     * emitted.
     */
     ...
     * CET_ChangeByInactivate, this behavior cannot be overrided), it give im a
     * chance to choose remember this word or not.
     *
    ...
    CET_SwitchIM,
    CET_ChangeByUser =
        CET_SwitchIM, // the old name is not accurate, but keep for compatible.
} FcitxIMCloseEventType;

typedef boolean (*FcitxIMInit)(void *arg); /**< FcitxIMInit */
typedef void (*FcitxIMResetIM)(void *arg); /**< FcitxIMResetIM */
typedef INPUT_RETURN_VALUE (*FcitxIMDoInput)(
    void *arg, FcitxKeySym, unsigned int); /**< FcitxIMDoInput */
typedef INPUT_RETURN_VALUE (*FcitxIMGetCandWords)(
    void *arg);                                  /**< FcitxIMGetCandWords */
...
typedef INPUT_RETURN_VALUE (*FcitxIMKeyBlocker)(
    void *arg, FcitxKeySym, unsigned int); /**< FcitxIMKeyBlocker */
typedef void (*FcitxIMUpdateSurroundingText)(
    void *arg); /**< FcitxIMKeyBlocker */
...
typedef struct _FcitxIMIFace {
    ...
    FcitxIMUpdateSurroundingText
        UpdateSurroundingText;     /**< surrounding text update trigger */
...
typedef struct _FcitxIM {
    ...
    FcitxIMUpdateSurroundingText
        UpdateSurroundingText; /**< called when surrounding text updated */
 ...
 * @return _FcitxIM*
 */
struct _FcitxIM *FcitxInstanceGetIM(struct _FcitxInstance *instance,
 ...
 * @since 4.2.7
 **/
struct _FcitxIM *FcitxInstanceGetIMByIndex(struct _FcitxInstance *instance,
                                           int index);
 ...
 * @since 4.2
 **/
int FcitxInstanceGetIMIndexByName(struct _FcitxInstance *instance,
                                  const char *imName);
 ...
 * @since 4.2.7
 **/
struct _FcitxIM *FcitxInstanceGetIMByName(struct _FcitxInstance *instance,
                                          const char *imName);
 ...
 * @return void
 **/
void FcitxInstanceEnableIM(struct _FcitxInstance *instance,
                           struct _FcitxInputContext *ic, boolean keepState);
 ...
 * @return void
 **/
void FcitxInstanceCloseIM(struct _FcitxInstance *instance,
                          struct _FcitxInputContext *ic);
 ...
 * @return void
 **/
void FcitxInstanceChangeIMState(struct _FcitxInstance *instance,
                                struct _FcitxInputContext *ic);
...
/**
 * send a close event, which can be used by standalone module which will do
 *something link reset first.
 *
 * @param instance fcitx instance
...
/**
 * Sometimes, we use INPUT_RETURN_VALUE not from ProcessKey, so use this
 *function to do the correct thing.
 *
 * @param instance fcitx instance
 * @param retVal input return val
 * @return void
 **/
void FcitxInstanceProcessInputReturnValue(struct _FcitxInstance *instance,
 ...
 * @return void
 **/
void FcitxInstanceRegisterIM(struct _FcitxInstance *instance, void *imclass,
...
/**
 * another half part for process key, will be called by
 *FcitxInstanceProcessKey()
 *
 * @param instance fcitx instance
 ...
 * @return INPUT_RETURN_VALUE
 **/
INPUT_RETURN_VALUE FcitxInstanceDoInputCallback(struct _FcitxInstance *instance,
                                                INPUT_RETURN_VALUE retVal,
                                                FcitxKeyEventType event,
                                                long unsigned int timestamp,
                                                FcitxKeySym sym,
 ...
 * @return INPUT_RETURN_VALUE
 **/
void FcitxInstanceChooseCandidateByIndex(struct _FcitxInstance *instance,
                                         int index);
 ...
 * @return INPUT_RETURN_VALUE
 **/
void FcitxInstanceChooseCandidateByTotalIndex(struct _FcitxInstance *instance,
                                              int index);
 ...
 * @return void
 **/
void FcitxInstanceForwardKey(struct _FcitxInstance *instance,
                             struct _FcitxInputContext *ic,
 ...
 * ~/.config/fcitx/profile, "addon" for addon info. "ui" for current user
 *interface Input method unique can be also used here.
 *
 * @param instance fcitx instance
 ...
 * @since 4.2.7
 **/
void FcitxInstanceReloadAddonConfig(struct _FcitxInstance *instance,
                                    const char *addon);
 ...
 * @return void
 **/
FCITX_DEPRECATED void FcitxInstanceSwitchIM(struct _FcitxInstance *instance,
                                            int index);

/**
 * switch to a input method by name, name need to be valid, otherwise it have no
 *effect And if the index is zero, the state will automatically change to
 *inactive
 *
 * @param instance fcitx instance
 * @param name ...
 ...
 * @since 4.2.4
 **/
void FcitxInstanceSwitchIMByName(struct _FcitxInstance *instance,
                                 const char *name);

/**
 * switch to a input method by index, index need to be valid, otherwise it have
 *no effect And if the object index is zero, the state will automatically change
 *to inactive -1 means scroll forward, and -2 means scroll backward. -3 means
 *scroll forward without first one, -4 mean scroll backward without first one.
 *
 *
 * @param instance fcitx instance
 * @param name ...
 ...
 * @return void
 **/
void FcitxInputStateSetIsDoInputOnly(FcitxInputState *input,
                                     boolean isDoInputOnly);
...
/**
 * get client cursor position, which is similar to cursor position, but used
 *with client preedit
 *
 * @param input input state
...
/**
 * set client cursor position, which is similar to cursor position, but used
 *with client preedit
 *
 * @param input input state
 ...
 * @return void
 **/
void FcitxInputStateSetLastIsSingleChar(FcitxInputState *input,
                                        int lastIsSingleChar);
 ...
 **/
FcitxIM *FcitxInstanceGetIMFromIMList(struct _FcitxInstance *instance,
                                      FcitxIMAvailableStatus imas,
                                      const char *name);
 ...
 * @return void
 **/
void FcitxInstanceNotifyUpdateSurroundingText(struct _FcitxInstance *instance,
                                              struct _FcitxInputContext *ic);

/**
 * an standard key blocker, block all the key that cause cursor move when raw
...
INPUT_RETURN_VALUE FcitxStandardKeyBlocker(FcitxInputState *input,
                                           FcitxKeySym key, unsigned int state);
 ...
 * @return void
 **/
void FcitxInstanceSetLocalIMName(struct _FcitxInstance *instance,
                                 struct _FcitxInputContext *ic,
                                 const char *imname);
 ...
 * @since 4.2.6
 */
void FcitxInstanceUnregisterIM(struct _FcitxInstance *instance,
                               const char *name);

/**
 * show current input method information, notably it will use GetSubModeName to
 * show the sub mode name
 *
