 ...
 **/
FcitxModuleFunction FcitxModuleFindFunction(FcitxAddon *addon, int func_id);

void *FcitxModuleInvokeOnAddon(FcitxAddon *addon, FcitxModuleFunction func,
                               FcitxModuleFunctionArg *args);
/**
 * invode inter module function with addon pointer, returns NULL when fails (the
 *function itself can also return NULL)
 *
 * @param addon addon
 * @param functionId function index
...
/**
 * invoke inter module function with addon name, returns NULL when fails (the
 *function itself can also return NULL)
 *
 * @param instance fcitx instance
...
#define InvokeFunction(INST, MODULE, FUNC, ARG)                                \
    ((MODULE##_##FUNC##_RETURNTYPE)FcitxModuleInvokeFunctionByName(            \
        INST, MODULE##_NAME, MODULE##_##FUNC, ARG))
...
#else
#define DECLARE_ADDFUNCTIONS(prefix)                                           \
    static inline FcitxAddon *Fcitx_##prefix##_GetAddon(                       \
...
#define DEFINE_GET_ADDON(name, prefix)                                         \
    static inline FcitxAddon *Fcitx##prefix##GetAddon(                         \
        FcitxInstance *instance) {                                             \
...
#define DEFINE_GET_AND_INVOKE_FUNC_WITH_ERROR(prefix, suffix, id, err_ret)     \
    DEFINE_GET_AND_INVOKE_FUNC_WITH_TYPE_AND_ERROR(                            \
        prefix, suffix, id, intptr_t, (intptr_t)err_ret)

#define DEFINE_GET_AND_INVOKE_FUNC_WITH_TYPE_AND_ERROR(prefix, suffix, id,     \
                                                       type, err_ret)          \
    static inline FcitxModuleFunction Fcitx##prefix##Find##suffix(             \
        FcitxAddon *addon) {                                                   \
    ...
    static inline void *Fcitx##prefix##Invoke##suffix(                         \
        FcitxInstance *instance, FcitxModuleFunctionArg args) {                \
...
#define FCITX_DEF_MODULE_ARGS_LONG(var, arg0, arg1, arg2, arg3, arg4, arg5,    \
                                   arg6, arg7, ARGV...)                        \
