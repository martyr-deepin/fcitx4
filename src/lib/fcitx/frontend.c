...
static boolean AppPreeditBlacklisted(FcitxInstance *instance,
...
FCITX_EXPORT_API void *FcitxInstanceGetICData(FcitxInstance *instance,
                                              FcitxInputContext *ic,
                                              int icdataid) {
...
static void FcitxInstanceSetICDataInternal(struct _FcitxInstance *instance,
                                           FcitxInputContext *ic, int icdataid,
                                           void *newdata, boolean copy) {
    FcitxInputContext2 *ic2 = (FcitxInputContext2 *)ic;
...
void FcitxInstanceSetICData(struct _FcitxInstance *instance,
    ...
    case ShareState_PerProgram: {
        ...
        while (rec != NULL) {
            ...
            if (flag)
                FcitxInstanceSetICDataInternal(instance, rec, icdataid, newdata,
                                               (rec != ic));
            rec = rec->next;
        }
    } break;
...
void FcitxInstanceCleanUpIC(FcitxInstance *instance) {
    ...
    while (rec) {
        FcitxAddon **pfrontend =
            FcitxInstanceGetPFrontend(instance, rec->frontendid);
        FcitxFrontend *frontend = (*pfrontend)->frontend;
        pid_t pid = 0;
        if (frontend->GetPid)
...
FCITX_EXPORT_API
FcitxInputContext *FcitxInstanceFindIC(FcitxInstance *instance, int frontendid,
                                       void *filter) {
    FcitxAddon **pfrontend = FcitxInstanceGetPFrontend(instance, frontendid);
    if (pfrontend == NULL)
        return NULL;
    ...
    FcitxInputContext *rec = instance->ic_list;
    while (rec != NULL) {
        if (rec->frontendid == frontendid &&
            frontend->CheckIC((*pfrontend)->addonInstance, rec, filter))
            return rec;
        rec = rec->next;
    }
    return NULL;
...
FCITX_EXPORT_API
void FcitxInstanceSetICStateFromSameApplication(FcitxInstance *instance,
                                                int frontendid,
                                                FcitxInputContext *ic) {
    FcitxAddon **pfrontend = FcitxInstanceGetPFrontend(instance, frontendid);
    if (pfrontend == NULL)
...
FCITX_EXPORT_API
void FcitxInstanceDestroyIC(FcitxInstance *instance, int frontendid,
                            void *filter) {
...
FCITX_EXPORT_API
boolean FcitxInstanceGetSurroundingText(FcitxInstance *instance,
    ...
    if (frontend->GetSurroundingPreedit) {
        return frontend->GetSurroundingPreedit((*pfrontend)->addonInstance, ic,
                                               str, cursor, anchor);
    }
    return false;
}
...
FCITX_EXPORT_API
void FcitxInstanceDeleteSurroundingText(FcitxInstance *instance,
    ...
    if (frontend->DeleteSurroundingText) {
        frontend->DeleteSurroundingText((*pfrontend)->addonInstance, ic, offset,
                                        size);
    }
}

FCITX_EXPORT_API
void FcitxInstanceUpdatePreedit(FcitxInstance *instance,
                                FcitxInputContext *ic) {
...
FCITX_EXPORT_API
void FcitxInstanceUpdateClientSideUI(FcitxInstance *instance,
                                     FcitxInputContext *ic) {
    if (ic == NULL)
        return;
...
FCITX_EXPORT_API
void FcitxInstanceSetWindowOffset(FcitxInstance *instance,
...
FCITX_EXPORT_API
void FcitxInstanceGetWindowPosition(FcitxInstance *instance,
                                    FcitxInputContext *ic, int *x, int *y) {
    if (ic == NULL)
        return;
    ...
    if (frontend->GetWindowRect) {
        frontend->GetWindowRect((*pfrontend)->addonInstance, ic, &rx, &ry, &rw,
                                &rh);
        *x = rx;
        *y = ry + rh;
    }
...
FCITX_EXPORT_API
void FcitxInstanceGetWindowRect(FcitxInstance *instance, FcitxInputContext *ic,
    ...
    for (addon = (FcitxAddon *)utarray_front(addons); addon != NULL;
        ...
        if (addon->bEnabled && addon->category == AC_FRONTEND) {
            ...
            case AT_SHAREDLIBRARY: {
                ...
                if (!handle) {
                    FcitxLog(ERROR, _("Frontend: open %s fail %s"), modulePath,
                             dlerror());
                    break;
                }
                ...
                if ((addon->addonInstance =
                         frontend->Create(instance, frontendindex)) == NULL) {
                    dlclose(handle);
                    break;
                }
