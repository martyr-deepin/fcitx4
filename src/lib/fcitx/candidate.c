...
static const UT_icd cand_icd = {sizeof(FcitxCandidateWord), NULL, NULL,
                                FcitxCandidateWordFree};
...
FCITX_EXPORT_API
void FcitxCandidateWordMove(FcitxCandidateWordList *candList, int from,
                            int to) {
...
FCITX_EXPORT_API void FcitxCandidateWordRemove(FcitxCandidateWordList *candList,
                                               FcitxCandidateWord *candWord) {
...
FcitxCandidateWordGetCurrentWindowNext(FcitxCandidateWordList *candList,
                                       FcitxCandidateWord *candWord) {
    FcitxCandidateWord *nextCandWord =
        (FcitxCandidateWord *)utarray_next(&candList->candWords, candWord);
    if (nextCandWord == NULL)
        return NULL;
    FcitxCandidateWord *startCandWord =
...
FcitxCandidateWordGetCurrentWindowPrev(FcitxCandidateWordList *candList,
    ...
    if (prevWord == NULL)
        return NULL;
    FcitxCandidateWord *startWord =
        FcitxCandidateWordGetCurrentWindow(candList);
    if (prevWord < startWord || prevWord >= startWord + candList->wordPerPage)
        return NULL;
    return prevWord;
}
...
FcitxCandidateWordChooseByIndex(FcitxCandidateWordList *candList, int index) {
    FcitxCandidateWord *candWord =
        FcitxCandidateWordGetByIndex(candList, index);
    if (candWord == NULL) {
...
FCITX_EXPORT_API
INPUT_RETURN_VALUE
FcitxCandidateWordChooseByTotalIndex(FcitxCandidateWordList *candList,
                                     int index) {
    FcitxCandidateWord *candWord =
...
FcitxCandidateWordPageCount(FcitxCandidateWordList *candList) {
    return (utarray_len(&candList->candWords) + candList->wordPerPage - 1) /
           candList->wordPerPage;
}
...
FCITX_EXPORT_API
void FcitxCandidateWordAppend(FcitxCandidateWordList *candList,
                              FcitxCandidateWord *candWord) {
...
FCITX_EXPORT_API
void FcitxCandidateWordSetChoose(FcitxCandidateWordList *candList,
                                 const char *strChoose) {
    FcitxCandidateWordSetChooseAndModifier(candList, strChoose,
                                           FcitxKeyState_None);
}

FCITX_EXPORT_API
void FcitxCandidateWordSetChooseAndModifier(FcitxCandidateWordList *candList,
                                            const char *strChoose,
                                            unsigned int state) {
...
FCITX_EXPORT_API
FcitxCandidateWord *
FcitxCandidateWordGetLast(FcitxCandidateWordList *candList) {
...
int FcitxCandidateWordCheckChooseKey(FcitxCandidateWordList *candList,
    ...
    return FcitxHotkeyCheckChooseKeyAndModifier(sym, state, candList->strChoose,
...
FcitxCandidateWordGetFocus(FcitxCandidateWordList *cand_list, boolean clear) {
    ...
    for (cand_word = FcitxCandidateWordGetCurrentWindow(cand_list); cand_word;
         cand_word =
             FcitxCandidateWordGetCurrentWindowNext(cand_list, cand_word)) {
