...
static const UT_icd ci_icd = {sizeof(FcitxContextCallbackInfo), NULL, NULL,
                              NULL};
static void FcitxInstanceSetContextInternal(FcitxInstance *instance,
                                            FcitxContext *context,
                                            const void *value);

FCITX_EXPORT_API
void FcitxInstanceRegisterWatchableContext(FcitxInstance *instance,
                                           const char *key,
                                           FcitxContextType type,
                                           unsigned int flag) {
    ...
    HASH_ADD_KEYPTR(hh, instance->context, context->name, strlen(context->name),
                    context);
}

FCITX_EXPORT_API
void FcitxInstanceSetContext(FcitxInstance *instance, const char *key,
                             const void *value) {
    FcitxContext *context;
...
void FcitxInstanceSetContextInternal(FcitxInstance *instance,
                                     FcitxContext *context, const void *value) {
    ...
    if (changed) {
        ...
        for (info =
                 (FcitxContextCallbackInfo *)utarray_front(context->callback);
             info != NULL; info = (FcitxContextCallbackInfo *)utarray_next(
...
FCITX_EXPORT_API
void FcitxInstanceWatchContext(FcitxInstance *instance, const char *key,
                               FcitxContextCallback callback, void *arg) {
    FcitxContext *context;
    HASH_FIND_STR(instance->context, key, context);
    if (context == NULL)
        return;
...
FCITX_EXPORT_API
const FcitxHotkey *FcitxInstanceGetContextHotkey(FcitxInstance *instance,
                                                 const char *key) {
    FcitxContext *context;
    HASH_FIND_STR(instance->context, key, context);
    if (context == NULL)
...
FCITX_EXPORT_API
const char *FcitxInstanceGetContextString(FcitxInstance *instance,
                                          const char *key) {
    FcitxContext *context;
    HASH_FIND_STR(instance->context, key, context);
    if (context == NULL)
...
FCITX_EXPORT_API
boolean FcitxInstanceGetContextBoolean(FcitxInstance *instance,
                                       const char *key) {
    FcitxContext *context;
    HASH_FIND_STR(instance->context, key, context);
    if (context == NULL)
