...
FCITX_EXPORT_API
boolean FcitxHotkeyIsKey(FcitxKeySym sym, unsigned int state,
                         FcitxKeySym symcmp, unsigned int statecmp) {
...
boolean FcitxHotkeyIsHotkeyCursorMove(FcitxKeySym sym, unsigned int state) {
    if ((sym == FcitxKey_Left || sym == FcitxKey_Right || sym == FcitxKey_Up ||
         sym == FcitxKey_Down || sym == FcitxKey_Page_Up ||
         sym == FcitxKey_Page_Down || sym == FcitxKey_Home ||
         sym == FcitxKey_End) &&
        (state == FcitxKeyState_Ctrl || state == FcitxKeyState_Ctrl_Shift ||
         state == FcitxKeyState_Shift || state == FcitxKeyState_None)
...
FCITX_EXPORT_API
boolean FcitxHotkeyIsHotKeyModifierCombine(FcitxKeySym sym,
                                           unsigned int state) {
    FCITX_UNUSED(state);
 ...
 */
FCITX_EXPORT_API
void FcitxHotkeyGetKey(FcitxKeySym keysym, unsigned int iKeyState,
                       FcitxKeySym *outk, unsigned int *outs) {
    /* key state != 0 */
    if (iKeyState) {
        if (iKeyState != FcitxKeyState_Shift &&
            FcitxHotkeyIsHotKeyLAZ(keysym, 0))
        ...
        if (FcitxHotkeyIsHotKeyLAZ(keysym, 0) ||
            FcitxHotkeyIsHotKeyUAZ(keysym, 0)) {
            if (iKeyState == FcitxKeyState_Shift)
                iKeyState &= ~FcitxKeyState_Shift;
        } else {
            if ((iKeyState & FcitxKeyState_Shift) &&
                (((FcitxHotkeyIsHotKeySimple(keysym, 0) ||
                   FcitxKeySymToUnicode(keysym) != 0) &&
...
char *FcitxHotkeyGetReadableKeyString(FcitxKeySym sym, unsigned int state) {
    ...
    if ((state & FcitxKeyState_Ctrl) && sym != FcitxKey_Control_R &&
        sym != FcitxKey_Control_L)
        len += strlen("Ctrl+");

    if ((state & FcitxKeyState_Alt) && sym != FcitxKey_Alt_R &&
        sym != FcitxKey_Alt_L)
        len += strlen("Alt+");

    if ((state & FcitxKeyState_Shift) && sym != FcitxKey_Shift_R &&
        sym != FcitxKey_Shift_L)
        len += strlen("Shift+");

    if ((state & FcitxKeyState_Super) && sym != FcitxKey_Super_L &&
...
FCITX_EXPORT_API
boolean FcitxHotkeyParseKey(const char *strKey, FcitxKeySym *sym,
                            unsigned int *state) {
    const char *p;
    int iKey;
    int iKeyState = 0;
...
#define PAD_TO_MAIN(keypad, keymain)                                           \
    case keypad:                                                               \
        return keymain
    switch (sym) {
