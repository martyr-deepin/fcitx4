 ...
 *
 * Fcitx configuration file can be easily mapped to corresponding user
 * interface, and you don't need to write any user interface at all.
 *
 ...
 * FilterIMName, instance) CONFIG_BINDING_REGISTER("Profile", "WidePunc",
 * bUseWidePunc) CONFIG_BINDING_REGISTER("Profile",
 * "PreeditStringInClientWindow", bUsePreedit)
 * CONFIG_BINDING_REGISTER_WITH_FILTER_ARG("Profile", "EnabledIMList", imList,
 * FilterIMList, instance) CONFIG_BINDING_END()
 * @endcode
 *
 * Then you will get following function:
 *
 * @code
 * void FcitxProfileConfigBind( FcitxProfile* config, FcitxConfigFile* cfile,
 * FcitxConfigFileDesc* cfdesc, FcitxInstance* instance )
 * @endcode
 *
 ...
 * @endcode
 *
 * The FcitxConfigFileDesc pointer is coresponding to the .desc file, which need
 * to be placed under share/fcitx/configdesc/
 *
 * You can use following macro to define a define to load FcitxConfigFileDesc*
 * pointer, second argument is the .desc file name.
 *
 * The FcitxConfigFileDesc pointer returned by this macro is a static variable,
 * so it should not be free'd, and will only load once.
 *
 * @code
...
typedef struct _FcitxConfigFileDesc
    FcitxConfigFileDesc; /**< FcitxConfigFileDesc */

typedef struct _FcitxConfigGroupDesc
    FcitxConfigGroupDesc; /**< FcitxConfigGroupDesc */

typedef struct _FcitxConfigOptionDesc
    FcitxConfigOptionDesc; /**< FcitxConfigOptionDesc */

typedef struct _FcitxConfigOptionDesc2
    FcitxConfigOptionDesc2; /**< FcitxConfigOptionDesc2 */

typedef struct _FcitxGenericConfig
    FcitxGenericConfig; /**< FcitxGenericConfig */

typedef struct _FcitxConfigOptionSubkey
    FcitxConfigOptionSubkey; /**< FcitxConfigOptionSubkey */

typedef union _FcitxConfigConstrain
    FcitxConfigConstrain /** < FcitxConfigConstrain */;

/**
 * sync filter function
 **/
typedef void (*FcitxSyncFilter)(FcitxGenericConfig *config,
...
/**
 * Config file contains multiple config groups, and the opposite config file
 *description
 **/
...
struct _FcitxConfigGroupDesc {
    ...
    FcitxConfigOptionDesc
        *optionsDesc;  /**< Hash table for option description */
    UT_hash_handle hh; /**< hash handle */
};
...
struct _FcitxConfigOption {
    ...
    FcitxConfigOptionSubkey
        *subkey;       /**< subkey which only used with I18NString */
    UT_hash_handle hh; /**< hash handle */
};
...
#define CONFIG_BINDING_DECLARE(config_type)                                    \
    void config_type##ConfigBind(config_type *config, FcitxConfigFile *cfile,  \
...
#define CONFIG_BINDING_DECLARE_WITH_ARG(config_type, arg...)                   \
    void config_type##ConfigBind(config_type *config, FcitxConfigFile *cfile,  \
                                 FcitxConfigFileDesc *cfdesc, arg);
...
#define CONFIG_DESC_DEFINE(funcname, path)                                     \
    FcitxConfigFileDesc *funcname() {                                          \
        ...
        if (!configDesc) {                                                     \
            ...
            if (tmpfp == NULL) {                                               \
                FcitxLog(ERROR,                                                \
                         "Load Config Description File %s Error, Please "      \
                         "Check your install.",                                \
                         path);                                                \
                return NULL;                                                   \
            }                                                                  \
...
#define CONFIG_DEFINE_LOAD_AND_SAVE(name, type, config_name)                   \
    ...
    void name##SaveConfig(type *_cfg) {                                        \
        ...
        FILE *fp = FcitxXDGGetFileUserWithPrefix(                              \
            "conf", config_name ".config", "w", NULL);                         \
    ...
    boolean name##LoadConfig(type *_cfg) {                                     \
        ...
        if (configDesc == NULL)                                                \
            return false;                                                      \
                                                                               \
        FILE *fp;                                                              \
        fp = FcitxXDGGetFileUserWithPrefix("conf", config_name ".config", "r", \
                                           NULL);                              \
        if (!fp) {                                                             \
            if (errno == ENOENT)                                               \
 ...
 * @return FcitxConfigFile*
 **/
FcitxConfigFile *FcitxConfigParseMultiConfigFile(char **filename, int len,
                                                 FcitxConfigFileDesc *cfdesc);
 ...
 * @return boolean
 **/
boolean FcitxConfigCheckConfigFile(FcitxConfigFile *configFile,
                                   FcitxConfigFileDesc *fileDesc);
 ...
 * @return boolean
 **/
boolean FcitxConfigSaveConfigFileFp(FILE *fp, FcitxGenericConfig *cfile,
                                    FcitxConfigFileDesc *cdesc);
...
/**
 * Get a option description from config file description, return NULL if not
 *found.
 *
 * @param cfdesc config file description
