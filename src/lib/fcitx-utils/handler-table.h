...
extern "C" {
#endif
typedef void (*FcitxFreeKeyFunc)(void *data, const void *key, size_t len,
                                 void *owner);
typedef void (*FcitxInitKeyFunc)(void *data, const void *key, size_t len,
                                 void *owner);
...
int fcitx_handler_table_append(FcitxHandlerTable *table, size_t keysize,
                               const void *key, const void *obj);
int fcitx_handler_table_prepend(FcitxHandlerTable *table, size_t keysize,
                                const void *key, const void *obj);
int fcitx_handler_key_append(FcitxHandlerTable *table, FcitxHandlerKey *key,
                             const void *obj);
int fcitx_handler_key_prepend(FcitxHandlerTable *table, FcitxHandlerKey *key,
                              const void *obj);
...
FcitxHandlerKey *fcitx_handler_table_find_key(FcitxHandlerTable *table,
...
fcitx_handler_table_find_strkey(FcitxHandlerTable *table, const char *key,
                                boolean create) {
    return fcitx_handler_table_find_key(table, strlen(key), (const void *)key,
                                        create);
}
...
void *fcitx_handler_key_get_data(FcitxHandlerTable *table,
                                 FcitxHandlerKey *key);
const void *fcitx_handler_key_get_key(FcitxHandlerTable *table,
                                      FcitxHandlerKey *key, size_t *len);
void *fcitx_handler_key_first(FcitxHandlerTable *table, FcitxHandlerKey *key);
void *fcitx_handler_key_last(FcitxHandlerTable *table, FcitxHandlerKey *key);
int fcitx_handler_key_first_id(FcitxHandlerTable *table, FcitxHandlerKey *key);
int fcitx_handler_key_last_id(FcitxHandlerTable *table, FcitxHandlerKey *key);
...
int fcitx_handler_table_first_id(FcitxHandlerTable *table, size_t keysize,
                                 const void *key);
int fcitx_handler_table_last_id(FcitxHandlerTable *table, size_t keysize,
                                const void *key);
...
static inline int fcitx_handler_table_append_strkey(FcitxHandlerTable *table,
                                                    const char *keystr,
                                                    const void *obj) {
    return fcitx_handler_table_append(table, strlen(keystr),
                                      (const void *)keystr, obj);
}
static inline int fcitx_handler_table_prepend_strkey(FcitxHandlerTable *table,
                                                     const char *keystr,
                                                     const void *obj) {
    return fcitx_handler_table_prepend(table, strlen(keystr),
                                       (const void *)keystr, obj);
}
static inline void *fcitx_handler_table_first_strkey(FcitxHandlerTable *table,
                                                     const char *keystr) {
    return fcitx_handler_table_first(table, strlen(keystr),
                                     (const void *)keystr);
}
static inline void *fcitx_handler_table_last_strkey(FcitxHandlerTable *table,
                                                    const char *keystr) {
    return fcitx_handler_table_last(table, strlen(keystr),
                                    (const void *)keystr);
}
static inline int fcitx_handler_table_first_id_strkey(FcitxHandlerTable *table,
                                                      const char *keystr) {
    return fcitx_handler_table_first_id(table, strlen(keystr),
                                        (const void *)keystr);
}
static inline int fcitx_handler_table_last_id_strkey(FcitxHandlerTable *table,
                                                     const char *keystr) {
...
fcitx_handler_table_remove_key_strkey(FcitxHandlerTable *table,
                                      const char *keystr) {
    fcitx_handler_table_remove_key(table, strlen(keystr), (const void *)keystr);
}
