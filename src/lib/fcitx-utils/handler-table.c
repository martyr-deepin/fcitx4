...
FCITX_EXPORT_API FcitxHandlerTable *fcitx_handler_table_new_with_keydata(
...
FCITX_EXPORT_API FcitxHandlerKey *(fcitx_handler_table_find_key)(
    FcitxHandlerTable *table, size_t keysize, const void *key, boolean create) {
    FcitxHandlerKey *key_struct = NULL;
    HASH_FIND(hh, table->keys, key, keysize, key_struct);
    if (key_struct || !create)
        return key_struct;
    key_struct =
        malloc(sizeof(FcitxHandlerKey) + keysize + table->key_vtable.size + 1);
    ...
    if (table->key_vtable.init) {
        table->key_vtable.init(key_struct + 1, key_ptr, keysize,
                               table->key_vtable.owner);
    }
    return key_struct;
}
...
FCITX_EXPORT_API int fcitx_handler_key_append(FcitxHandlerTable *table,
...
FCITX_EXPORT_API int fcitx_handler_table_append(FcitxHandlerTable *table,
...
FCITX_EXPORT_API int fcitx_handler_key_prepend(FcitxHandlerTable *table,
                                               FcitxHandlerKey *key_struct,
                                               const void *obj) {
    int new_id;
    new_id = fcitx_obj_pool_alloc_id(table->objs);
...
FCITX_EXPORT_API
boolean fcitx_handler_key_is_empty(FcitxHandlerTable *table,
                                   FcitxHandlerKey *key) {
    FCITX_UNUSED(table);
...
FCITX_EXPORT_API int fcitx_handler_table_prepend(FcitxHandlerTable *table,
                                                 size_t keysize,
                                                 const void *key,
                                                 const void *obj) {
    FcitxHandlerKey *key_struct;
...
FCITX_EXPORT_API void *fcitx_handler_table_get_by_id(FcitxHandlerTable *table,
                                                     int id) {
    if (id == FCITX_OBJECT_POOL_INVALID_ID)
        return NULL;
    FcitxHandlerObj *obj_struct;
...
FCITX_EXPORT_API
FcitxHandlerKey *fcitx_handler_table_get_key_by_id(FcitxHandlerTable *table,
                                                   int id) {
    if (id == FCITX_OBJECT_POOL_INVALID_ID)
        return NULL;
    FcitxHandlerObj *obj_struct;
...
FCITX_EXPORT_API void *fcitx_handler_key_get_data(FcitxHandlerTable *table,
                                                  FcitxHandlerKey *key) {
    FCITX_UNUSED(table);
...
FCITX_EXPORT_API const void *fcitx_handler_key_get_key(FcitxHandlerTable *table,
                                                       FcitxHandlerKey *key,
                                                       size_t *len) {
    if (fcitx_unlikely(!key)) {
        if (len)
...
FCITX_EXPORT_API int fcitx_handler_key_first_id(FcitxHandlerTable *table,
                                                FcitxHandlerKey *key) {
    if (fcitx_unlikely(!key))
...
FCITX_EXPORT_API int fcitx_handler_key_last_id(FcitxHandlerTable *table,
                                               FcitxHandlerKey *key) {
    if (fcitx_unlikely(!key))
        return FCITX_OBJECT_POOL_INVALID_ID;
    return key->last;
...
FCITX_EXPORT_API void *fcitx_handler_key_first(FcitxHandlerTable *table,
                                               FcitxHandlerKey *key) {
    int id = fcitx_handler_key_first_id(table, key);
    return fcitx_handler_table_get_by_id(table, id);
}

FCITX_EXPORT_API void *fcitx_handler_key_last(FcitxHandlerTable *table,
                                              FcitxHandlerKey *key) {
    int id = fcitx_handler_key_last_id(table, key);
    return fcitx_handler_table_get_by_id(table, id);
}

FCITX_EXPORT_API int fcitx_handler_table_first_id(FcitxHandlerTable *table,
                                                  size_t keysize,
                                                  const void *key) {
    FcitxHandlerKey *key_struct;
...
FCITX_EXPORT_API int fcitx_handler_table_last_id(FcitxHandlerTable *table,
                                                 size_t keysize,
                                                 const void *key) {
    FcitxHandlerKey *key_struct;
    key_struct = fcitx_handler_table_find_key(table, keysize, key, false);
...
FCITX_EXPORT_API int fcitx_handler_table_next_id(FcitxHandlerTable *table,
                                                 const void *obj) {
    FCITX_UNUSED(table);
...
FCITX_EXPORT_API int fcitx_handler_table_prev_id(FcitxHandlerTable *table,
                                                 const void *obj) {
    FCITX_UNUSED(table);
    const FcitxHandlerObj *obj_struct = obj - sizeof(FcitxHandlerObj);
...
FCITX_EXPORT_API void *fcitx_handler_table_first(FcitxHandlerTable *table,
                                                 size_t keysize,
                                                 const void *key) {
...
FCITX_EXPORT_API void *fcitx_handler_table_last(FcitxHandlerTable *table,
                                                size_t keysize,
                                                const void *key) {
...
FCITX_EXPORT_API void *fcitx_handler_table_next(FcitxHandlerTable *table,
                                                const void *obj) {
    int id = fcitx_handler_table_next_id(table, obj);
    return fcitx_handler_table_get_by_id(table, id);
}

FCITX_EXPORT_API void *fcitx_handler_table_prev(FcitxHandlerTable *table,
                                                const void *obj) {
    int id = fcitx_handler_table_prev_id(table, obj);
    return fcitx_handler_table_get_by_id(table, id);
}

FCITX_EXPORT_API void fcitx_handler_table_remove_by_id(FcitxHandlerTable *table,
                                                       int id) {
    if (id == FCITX_OBJECT_POOL_INVALID_ID)
        return;
    FcitxHandlerObj *obj_struct;
...
static void fcitx_handler_table_free_key(FcitxHandlerTable *table,
                                         FcitxHandlerKey *key_struct) {
    int id;
    int next_id;
    FcitxHandlerObj *obj_struct;
    for (id = key_struct->first; id != FCITX_OBJECT_POOL_INVALID_ID;
         id = next_id) {
        obj_struct = fcitx_handler_table_get_obj(table, id);
    ...
    if (table->key_vtable.free) {
        size_t len;
        const void *key_ptr =
            fcitx_handler_key_get_key(table, key_struct, &len);
        table->key_vtable.free(key_struct + 1, key_ptr, len,
                               table->key_vtable.owner);
    }
    free(key_struct);
}
...
FCITX_EXPORT_API void fcitx_handler_table_remove_key(FcitxHandlerTable *table,
                                                     size_t keysize,
                                                     const void *key) {
    FcitxHandlerKey *key_struct;
    key_struct = fcitx_handler_table_find_key(table, keysize, key, false);
...
FCITX_EXPORT_API void fcitx_handler_table_free(FcitxHandlerTable *table) {
    FcitxHandlerKey *key_struct;
