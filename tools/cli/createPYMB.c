...
boolean LoadPY(void) {
    ...
    for (i = 0; i < iPYFACount; i++) {
        ...
        PYFAList[i].pyBase =
            (_PyBase *)malloc(sizeof(_PyBase) * PYFAList[i].iHZCount);
        ...
        for (j = 0; j < PYFAList[i].iHZCount; j++) {
            ...
            PYFAList[i].pyBase[j].phrase =
                (_PyPhrase *)malloc(sizeof(_PyPhrase));
        ...
        for (iIndex = 0; iIndex < iPYFACount; iIndex++) {
            if (!strncmp(PYFAList[iIndex].strMap, strMap, 2)) {
                for (i = 0; i < PYFAList[iIndex].iHZCount; i++) {
                    if (!fcitx_utf8_strncmp(PYFAList[iIndex].pyBase[i].strHZ,
                                            strPhrase, 1)) {
                        t = PYFAList[iIndex].pyBase[i].phrase;

                        for (j = 0; j < PYFAList[iIndex].pyBase[i].iPhraseCount;
                             j++) {
                            tt = t;
                            t = t->next;

                            if (!strcmp(t->strMap, strMap + 2) &&
                                !strcmp(t->strPhrase,
                                        strPhrase +
                                            fcitx_utf8_char_len(strPhrase))) {
                                printf("\n\t%d: %s %s ----->deleted.\n", s2,
                                       strPY, strPhrase);
                                goto _next;
                            }
                        ...
                        phrase->strPhrase = (char *)malloc(
                            sizeof(char) *
                            (strlen(strPhrase) -
                             fcitx_utf8_char_len(strPhrase) + 1));
                        phrase->strMap = (char *)malloc(
                            sizeof(char) * ((strTemp.iHZCount - 1) * 2 + 1));
                        phrase->uIndex = uIndex++;
                        strcpy(phrase->strPhrase,
                               strPhrase + fcitx_utf8_char_len(strPhrase));
    ...
    for (i = 0; i < iPYFACount; i++) {
        for (j = 0; j < PYFAList[i].iHZCount; j++) {
            ...
            if (iIndex) {
                ...
                fwrite(PYFAList[i].pyBase[j].strHZ, sizeof(char) * clen, 1,
                       fp2);
                ...
                for (k = 0; k < PYFAList[i].pyBase[j].iPhraseCount; k++) {
                    ...
                    fwrite(t->strPhrase, sizeof(char), strlen(t->strPhrase),
                           fp2);
    ...
    while (!feof(fps)) {
        ...
        if (MapPY(&pyconfig, strPY, strMap, PY_PARSE_INPUT_SYSTEM)) {
            for (i = 0; i < iBaseCount; i++)
                if ((!strcmp(PYTable_template[i].strPY, strPY)) &&
                    PYTable_template[i].control == PYTABLE_NONE)
                    YY[i] += 1;
            ...
            if (fcitx_utf8_strlen(strHZ) > 1) {
                ...
                fprintf(stderr, "%s length is larger that 1, truncated to ",
                        strHZ);
                strHZ[charLen] = '\0';
                fprintf(stderr, "%s.\n", strHZ);
            }
...
void Usage() { printf("Usage: createPYMB <pyfile> <phrasefile>\n"); }
