...
void test_string_hash_set() {
    ...
    FcitxStringHashSet *sset =
        fcitx_utils_string_hash_set_parse("a,b,c,d", ',');
...
void test_string_map() {
    ...
    assert(!fcitx_string_map_get(map, "a", false));
    fcitx_string_map_from_string(map, joined, '\n');
    free(joined);
    assert(fcitx_string_map_get(map, "a", false));
    fcitx_string_map_remove(map, "a");
    assert(!fcitx_string_map_get(map, "a", false));
