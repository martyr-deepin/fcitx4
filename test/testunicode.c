...
int main(int argc, char *argv[]) {
    ...
    UT_array *approximateEquivalents =
        CharSelectDataApproximateEquivalents(charselect, 0x2003);
    ...
    char *label[] = {
        "Definition in English: ",      "Cantonese Pronunciation: ",
        "Mandarin Pronunciation: ",     "Tang Pronunciation: ",
        "Korean Pronunciation: ",       "Japanese On Pronunciation: ",
        "Japanese Kun Pronunciation: ",
    };
    for (i = 0; i < utarray_len(unihan); i++) {
        fprintf(stderr, "%s: %s\n", label[i],
                *(char **)utarray_eltptr(unihan, i));
    }
    fcitx_utils_free_string_list(unihan);

    UT_array *result = CharSelectDataFind(charselect, "f");
    utarray_foreach(c, result, uint16_t) { fprintf(stderr, "%x\n", *c); }
