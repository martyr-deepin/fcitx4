...
char *strs[] = {"jj0",      "kjk1",   "asdf2",       "jjjjj3",
                "kdjkfa4",  "asdf5",  "asdkf6",      "akskfd7",
                "aksdlfa8", "jaksd9", "jjjjjjakd10", "asdflasd11"};
...
static void test_free_handler_obj(void *obj) {
...
int main() {
    FcitxHandlerTable *table =
        fcitx_handler_table_new(sizeof(HandlerObj), test_free_handler_obj);
    HandlerObj obj;
    int i;
    for (i = 0; i < 6; i++) {
        obj.id = i;
    ...
    HandlerObj *p;
    assert(free_count == 0);
    for (p = fcitx_handler_table_first_strkey(table, KEY1), i = 0; p;
